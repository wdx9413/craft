...
export class VerificationPlane {
  ...
  record(args: JsonObject): JsonObject {
    ...
    const receiptId = String(args.receipt_id ?? `${verification.id}:${check.id}`); const identityDigest = digestJson(identity); const existing = this.store.find("verification_receipt", receiptId);
