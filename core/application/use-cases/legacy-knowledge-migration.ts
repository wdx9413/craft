...
export function installLegacyKnowledgeMigrationMethods(serviceClass: typeof CraftService): void {
  ...
  serviceClass.prototype.legacyKnowledgeMigrationCandidateImport = async function (args: JsonObject) {
    ...
    return this.legacyKnowledgeMigration.importCandidates(args, sourceId, (entry, _source, importedSourceId, candidateId) => {
      ...
      this.knowledgeClaimSave({ claim_id: claimId, title: entry.title, kind: ({ domain_rule: "fact", technical_decision: "decision", workflow: "rule", troubleshooting: "failure_mode", stable_project_fact: "fact" } as Record<string, string>)[entry.knowledge_type], content: `${entry.title}\n\n${entry.summary}`, source_id: importedSourceId, evidence_ids: [evidenceId], scope: `legacy:${entry.scope}:${entry.project ?? "shared"}`, tags: entry.tags });
      return { evidence_id: evidenceId, claim_id: claimId };
    });
  };
  serviceClass.prototype.legacyKnowledgeMigrationRebindProvenance = async function (args: JsonObject) {
    if (args.offline !== true) throw new Error("Legacy knowledge provenance rebind is offline-only");
    return this.legacyKnowledgeMigration.rebindProvenance(args);
  };
  serviceClass.prototype.knowledgeCandidateModelReview = function (args: JsonObject) {
    return this.legacyKnowledgeMigration.reviewCandidates(args);
  };
