...
export class WorkRuntimeModeKernel {
  ...
  configure(args: JsonObject): JsonObject {
    ...
    const existing = this.store.find("work_runtime_mode", profileId); const identityDigest = digestJson(identity);
  ...
  prepare(args: JsonObject): JsonObject {
    ...
    const existing = this.store.find("work_runtime_plan", planId); const identityDigest = digestJson(identity);
