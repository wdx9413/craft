...
export class UncertaintyPolicyKernel {
  ...
  save(args: JsonObject): JsonObject {
    ...
    const policyId = String(args.policy_id ?? `uncertainty_policy_${scope}_${identity.scope_id}`); const identityDigest = digestJson(identity); const existing = this.store.find("uncertainty_policy", policyId);
  ...
  resolve(args: JsonObject): JsonObject {
    ...
    const resolutionId = String(args.resolution_id ?? `uncertainty_resolution_${digestJson(identity).slice(-16)}`); const identityDigest = digestJson(identity); const existing = this.store.find("uncertainty_resolution", resolutionId);
  ...
  adjudicate(args: JsonObject): JsonObject {
    ...
    const adjudicationId = String(args.adjudication_id ?? `adjudication_${randomUUID().replaceAll("-", "")}`); const identityDigest = digestJson(identity); const existing = this.store.find("adjudication", adjudicationId);
