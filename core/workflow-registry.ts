...
const STATUSES: readonly string[] = ["active", "deprecated", "retired"];
const MAX_FILES = 500;
 ...
 */
export function normalizeWorkflowDefinition(raw: unknown): JsonObject {
  ...
  if (!WORKFLOW_ID.test(id)) throw new Error(`Unsupported workflow id: ${id}`);
  ...
  if (!STATUSES.includes(status)) throw new Error(`Unsupported workflow status: ${status}`);
  ...
  const inputs = rawInputs.map((entry, index) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) throw new Error(`Workflow input at index ${index} must be an object`);
    const name = text((entry as JsonObject).name, `workflow input name at index ${index}`);
    return { ...(entry as JsonObject), name };
  });
  const rawSteps = input.steps ?? [];
  if (input.steps !== undefined && !Array.isArray(rawSteps)) throw new Error("Workflow steps must be an array");
...
export function describeWorkflow(path: string, definition: JsonObject): WorkflowDescriptor {
  const steps = definition.steps as JsonObject[];
...
function walk(root: string, current: string, found: string[], limit: number): void {
  for (const entry of readdirSync(current, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
    const absolute = join(current, entry.name);
    const stat = lstatSync(absolute);
    // A symlinked directory could point anywhere, and the registry never follows links.
    if (stat.isSymbolicLink()) continue;
    if (stat.isDirectory()) { walk(root, absolute, found, limit); continue; }
    if (!entry.name.endsWith(WORKFLOW_FILE_SUFFIX)) continue;
    found.push(relative(root, absolute).replaceAll("\\", "/"));
    if (found.length > limit) throw new Error(`Workflow registry exceeds ${limit} files`);
  }
}

/** Pure filter that decides whether a path is a workflow file (and not a symlink or directory). */
export function isWorkflowFile(name: string, stat: { isSymbolicLink(): boolean; isDirectory(): boolean }): boolean {
 ...
 */
export function describeJsonFailure(path: string, error: unknown): string {
 ...
 */
export function discoverWorkflows(root: string, options: { limit?: number } = {}): WorkflowDescriptor[] {
  const base = resolve(root);
  if (!existsSync(base)) throw new Error("Workflow registry root does not exist");
  const limit = options.limit ?? MAX_FILES;
  if (!Number.isInteger(limit) || limit < 1) throw new Error("Workflow registry limit must be a positive integer");
  const found: string[] = [];
  walk(base, base, found, limit);
  const descriptors = found.map((path) => {
    let parsed: unknown;
    try { parsed = JSON.parse(readFileSync(join(base, path), "utf8")); }
    catch (error) { throw new Error(describeJsonFailure(path, error)); }
  ...
  if (new Set(keys).size !== keys.length) throw new Error("Workflow registry contains a duplicate workflow id and version");
  return descriptors;
}

/** Catalog drift: what is new, what changed, and what disappeared since the last scan. */
export function diffWorkflowCatalog(previous: readonly WorkflowDescriptor[], current: readonly WorkflowDescriptor[]): JsonObject {
  const key = (item: WorkflowDescriptor): string => `${item.workflow_id}@${item.version}`;
...
function resolvePolicy(options: { policy?: Partial<RetirementPolicy> }): RetirementPolicy {
  const policy = { ...DEFAULT_RETIREMENT_POLICY, ...(options.policy ?? {}) };
  if (!Number.isInteger(policy.min_uses) || policy.min_uses < 0) throw new Error("Retirement min_uses must be a non-negative integer");
  if (!Number.isInteger(policy.stale_days) || policy.stale_days < 1) throw new Error("Retirement stale_days must be a positive integer");
 ...
 */
export function planWorkflowRetirement(usage: readonly WorkflowUsage[], options: { now: string; policy?: Partial<RetirementPolicy> }): RetirementDecision[] {
  ...
  return usage.map((item) => {
    ...
    let recommendation: RetirementDecision["recommendation"];
    if (uses >= policy.min_uses && successRate !== null && successRate < policy.min_success_rate) {
      recommendation = "retire"; reasons.push("low_success_rate");
    } else if (stale && uses < policy.min_uses) {
      recommendation = "retire"; reasons.push("never_proven_and_stale");
    } else if (stale) {
      recommendation = "deprecate"; reasons.push("stale");
    } else {
      recommendation = "keep"; reasons.push("in_use");
    }
