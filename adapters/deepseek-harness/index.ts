...
function callCraft(command: string, packageSpec: string, args: ToolArgs, dataDir?: string): Promise<unknown> {
  return new Promise((resolve, reject) => {
    if (!args.tool?.trim()) { reject(new Error("tool must not be empty")); return; }
    let toolArguments: unknown;
    try { toolArguments = JSON.parse(args.arguments_json || "{}"); }
    catch { reject(new Error("arguments_json must be valid JSON")); return; }
    if (!toolArguments || typeof toolArguments !== "object" || Array.isArray(toolArguments)) {
      reject(new Error("arguments_json must contain a JSON object")); return;
    }
    ...
    let stdout = ""; let stderr = ""; let settled = false; let outputExceeded = false;
    const timer = setTimeout(() => {
      if (!settled) { child.kill(); settled = true; reject(new Error("Craft MCP call timed out after 60 seconds")); }
    }, 60_000);
    ...
    child.stdout.on("data", (chunk: string) => {
      stdout += chunk;
      if (stdout.length > 5_000_000) { outputExceeded = true; child.kill(); }
    });
    child.stderr.on("data", (chunk: string) => { stderr = (stderr + chunk).slice(-20_000); });
    child.on("error", (error) => rejectOnce(error));
    child.on("close", (code) => {
      if (outputExceeded) { rejectOnce(new Error("Craft MCP output exceeded 5 MB")); return; }
      if (code !== 0 && !stdout) { rejectOnce(new Error(stderr || `craft-mcp exited ${code}`)); return; }
      let response: Record<string, any> | undefined;
      try { response = stdout.split(/\r?\n/).filter(Boolean).map((line) => JSON.parse(line))
        .find((item) => item.id === 2); }
      catch { rejectOnce(new Error(stderr || "Craft returned invalid MCP output")); return; }
      if (!response) { rejectOnce(new Error(stderr || "Craft returned no tool response")); return; }
      if (response.error) { rejectOnce(new Error(response.error.message)); return; }
      if (response.result?.isError) {
        rejectOnce(new Error(response.result.content?.[0]?.text || "Craft tool call failed")); return;
      }
      resolveOnce(response.result?.structuredContent ?? response.result);
    });
    child.stdin.write(`${JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2025-11-25", capabilities: {}, clientInfo: { name: "dsh-craft-adapter", version: "0.12.10" } } })}\n`);
...
export function apply(ctx: Context, config: AdapterConfig = {}): void {
  ctx.tools.register(defineTool({
    ...
    execute: (args: ToolArgs) => callCraft(config.npxCommand || "npx",
      config.packageSpec || "craft-agent-harness@0.12.10", args, config.dataDir),
  }));
}
