...
function normalizeCapabilities(value) {
    ...
    const filesystem = text(input.filesystem, "capabilities.filesystem");
    const network = text(input.network, "capabilities.network");
    if (!FILESYSTEM.has(filesystem) || !NETWORK.has(network))
        throw new Error("Sandbox filesystem or network mode is unsupported");
    const features = strings(input.features ?? [], "capabilities.features").sort();
    if (features.some((feature) => !FEATURES.has(feature)))
        throw new Error("Sandbox capability feature is unsupported");
    const networkAllowlist = strings(input.network_allowlist ?? [], "capabilities.network_allowlist").sort();
    if ((network === "allowlist") !== (networkAllowlist.length > 0))
        throw new Error("Network allowlist is required only for allowlist mode");
...
export class SandboxKernel {
    ...
    profileVerify(args) {
        ...
        if (profile.lifecycle !== "declared")
            throw new Error("Only a declared Sandbox Profile can be verified");
        ...
        if (!evidenceIds.length)
            throw new Error("Sandbox verification requires evidence_ids");
        ...
        if (!matches)
            return { profile, assessment, verified: false };
    ...
    plan(args) {
        ...
        const profile = this.store.get("sandbox_profile", text(args.profile_id, "profile_id"), args.profile_version === undefined ? undefined : Number(args.profile_version));
        if (profile.lifecycle !== "verified")
            throw new Error("Sandbox execution requires a verified profile version");
        const requirements = object(args.requirements, "requirements");
        ...
        if (!FILESYSTEM.has(requiredFilesystem) || !NETWORK.has(requiredNetwork))
            throw new Error("Sandbox requirement mode is unsupported");
        const requiredFeatures = strings(requirements.features ?? [], "requirements.features");
        if (requiredFeatures.some((feature) => !FEATURES.has(feature)))
            throw new Error("Sandbox requirement feature is unsupported");
        ...
        if (missing.length)
            return { compatible: false, profile, missing, ticket: null };
        if (args.dry_run === true)
            return { compatible: true, profile, missing: [], ticket: null, dry_run: true };
    ...
    receipt(args) {
        ...
        if (existing) {
            if (existing.request_fingerprint !== fingerprint)
                throw new Error("Sandbox receipt idempotency conflict");
            return { receipt: existing, idempotent: true };
        }
        if (ticket.status !== "issued")
            throw new Error("Sandbox ticket is not active");
        ...
        if (!new Set(["passed", "failed", "cancelled", "timed_out"]).has(status))
            throw new Error("Sandbox receipt status is unsupported");
        const observed = normalizeCapabilities(args.observed_capabilities);
        const boundaryMatches = digest(observed) === ticket.profile_digest;
        if (status === "passed" && !boundaryMatches)
            throw new Error("A passed Sandbox receipt must prove the exact profile boundaries");
        const evidenceIds = strings(args.evidence_ids, "evidence_ids");
        if (!evidenceIds.length)
            throw new Error("Sandbox receipt requires evidence_ids");
        for (const evidenceId of evidenceIds)
            this.store.get("evidence", evidenceId);
