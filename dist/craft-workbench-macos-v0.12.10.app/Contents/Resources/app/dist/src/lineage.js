...
function key(ref) { return `${ref.kind}:${ref.id}:v${ref.version}:${ref.locator ?? ""}`; }
...
export class LineageKernel {
    ...
    reference(value, name, workspaceId, taskId) {
        ...
        const version = integer(input.version, `${name}.version`, 0);
        const record = this.store.get(kind, id, version);
        if (kind === "work_object" && record.workspace_id !== workspaceId)
            throw new Error(`${name} work object belongs to another workspace`);
        if (taskId && record.task_id && record.task_id !== taskId)
            throw new Error(`${name} belongs to another task`);
    ...
    record(args) {
        ...
        if (!inputs.length || new Set(inputs.map(key)).size !== inputs.length)
            throw new Error("inputs must contain unique source references");
        if (inputs.some((input) => key(input) === key(output)))
            throw new Error("Lineage output cannot directly depend on itself");
        ...
        if (!evidenceIds.length || new Set(evidenceIds).size !== evidenceIds.length)
            throw new Error("evidence_ids must contain unique Evidence");
        for (const evidenceId of evidenceIds)
            this.store.get("evidence", evidenceId);
        const actorType = text(args.actor_type, "actor_type");
        if (!ACTORS.has(actorType))
            throw new Error("actor_type is unsupported");
        ...
        const existingOutput = this.store.list("lineage_edge", 10_000, (edge) => edge.workspace_id === workspace.id && edge.output_key === outputKey)[0];
        ...
        if (existingOutput) {
            if (existingOutput.statement_digest !== digest(identity))
                throw new Error("Lineage output already has a different derivation");
        ...
        while (pending.length) {
            ...
            for (const edge of adjacency)
                if (edge.input_keys.includes(current))
                    pending.push(String(edge.output_key));
        }
        if (inputs.some((input) => reachable.has(key(input))))
            throw new Error("Lineage references would form a cycle");
    ...
    trace(args) {
        ...
        if (!new Set(["upstream", "downstream"]).has(direction))
            throw new Error("Lineage direction is unsupported");
        const start = this.reference(args.entity, "entity", String(workspace.id), null);
        const startKey = key(start);
        const maxDepth = integer(args.max_depth, "max_depth", 8, 1, 20);
        const edges = this.store.list("lineage_edge", 10_000, (edge) => edge.workspace_id === workspace.id);
        const selected = [];
        ...
        for (let depth = 0; depth < maxDepth && frontier.size; depth += 1) {
            ...
            for (const edge of edges) {
                const matches = direction === "upstream" ? frontier.has(String(edge.output_key))
                    : edge.input_keys.some((input) => frontier.has(input));
                if (!matches || seenEdges.has(String(edge.id)))
                    continue;
    ...
    verify(args) {
        const lineage = this.store.get("lineage_edge", text(args.lineage_id, "lineage_id"));
        const issues = [];
        const references = [["output", lineage.output], ...(lineage.inputs.map((item) => ["input", item]))];
        if (lineage.transform)
            references.push(["transform", lineage.transform]);
        for (const [role, ref] of references) {
            ...
            if (!exact)
                issues.push({ role, key: key(ref), issue: "missing_exact_version" });
            else if (latest && Number(latest.version) !== Number(ref.version))
                issues.push({ role, key: key(ref), issue: "newer_version_exists", latest_version: latest.version });
        }
        for (const evidenceId of lineage.evidence_ids)
            if (!this.store.find("evidence", evidenceId))
                issues.push({ role: "evidence", id: evidenceId, issue: "missing" });
