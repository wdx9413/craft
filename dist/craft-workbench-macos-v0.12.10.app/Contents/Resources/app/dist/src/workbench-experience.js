...
function limit(value) { const n = value === undefined ? 100 : Number(value); if (!Number.isInteger(n) || n < 1 || n > 1000)
    throw new Error("limit must be an integer between 1 and 1000"); return n; }
...
export class WorkbenchExperienceKernel {
    ...
    query(args = {}) {
        ...
        const taskIds = new Set(sessions.map((item) => String(item.task_id)));
        if (taskId)
            taskIds.add(taskId);
        const launches = this.store.list("work_launch", 10_000, (item) => taskIds.has(String(item.task_id))).slice(0, n);
        const traces = this.store.list("trace", 10_000, (item) => taskIds.has(String(item.task_id))).slice(0, n);
    ...
    get(args) { return this.query({ ...args, session_id: text(args.session_id, "session_id") }); }
    replayPlan(args) {
        const traceId = text(args.trace_id, "trace_id");
        const trace = this.store.get("trace", traceId);
        const events = this.store.list("trace_event", 10_000, (item) => item.trace_id === traceId).sort((a, b) => Number(a.sequence) - Number(b.sequence));
    ...
    review(args) { const result = this.query(args); return { ...result, review_digest: digest(result), recommendations: [result.next_action], evidence_required: true }; }
    ref(item) { return { id: item.id, version: item.version, status: item.status ?? null, digest: item.content_digest ?? item.identity_digest ?? item.summary_digest ?? null }; }
}
//# sourceMappingURL=workbench-experience.js.map