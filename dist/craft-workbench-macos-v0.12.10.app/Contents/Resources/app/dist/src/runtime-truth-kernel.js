...
import { compactConversation, createWorkNote, exportOtlp, standardizeTrace, toOtlpTrace } from "./runtime-truth.js";
...
export class RuntimeTruthKernel {
    ...
    otlp(args) {
        const trace = standardizeTrace(object(args.trace ?? args, "trace"));
        const events = args.events === undefined ? [] : Array.isArray(args.events) ? args.events : (() => { throw new Error("events must be an array"); })();
    ...
    async export(args) {
        ...
        const injected = args.fetch_impl;
        const transport = typeof injected === "function" ? injected : async (url, init) => { const response = await fetch(url, init); return { status: response.status, body: await response.text() }; };
        const result = await exportOtlp(endpoint, mapped.payload, transport);
    ...
    compact(args) {
        ...
        const result = compactConversation(messages, args.max_chars === undefined ? undefined : Number(args.max_chars));
    ...
    workNote(args) {
        ...
        if (existing)
            return { note: existing, idempotent: true };
        return { note: this.store.create("work_note", id, note), idempotent: false };
    }
}
//# sourceMappingURL=runtime-truth-kernel.js.map