...
function list(value, name) { if (value === undefined)
...
export class OsSecurityKernel {
    ...
    plan(args) {
        const platform = text(args.platform ?? process.platform, "platform");
        if (!PLATFORMS.has(platform))
            throw new Error("Unsupported platform");
        const workspace = text(args.workspace, "workspace");
        const network = text(args.network ?? "denied", "network");
        if (!NETWORK.has(network))
            throw new Error("Unsupported network policy");
        const filesystem = text(args.filesystem ?? "read_only", "filesystem");
        if (!FILESYSTEM.has(filesystem))
        ...
        const boundaryDigest = digest(boundary);
        const planId = String(args.plan_id ?? `os_security_${randomUUID().replaceAll("-", "")}`);
        const existing = this.store.find("os_security_plan", planId);
        if (existing) {
            if (existing.boundary_digest !== boundaryDigest)
                throw new Error("OS security plan idempotency conflict");
    ...
    verify(args) {
        const plan = this.store.get("os_security_plan", text(args.plan_id, "plan_id"));
        const observed = (args.observed && typeof args.observed === "object" && !Array.isArray(args.observed)) ? args.observed : {};
        ...
        const compatible = observedDigest === plan.boundary_digest && evidenceIds.length > 0;
        const receiptId = String(args.receipt_id ?? `os_security_receipt_${plan.id}`);
        const existing = this.store.find("os_security_receipt", receiptId);
        if (existing)
            return { receipt: existing, compatible: existing.compatible === true, idempotent: true };
        ...
        if (compatible)
            this.store.save("os_security_plan", String(plan.id), { ...plan, status: "verified", verified_receipt_id: receipt.id });
        return { receipt, compatible, idempotent: false };
    }
}
//# sourceMappingURL=os-security.js.map