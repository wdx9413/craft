...
export class HostBridgeKernel {
    ...
    prepare(args) {
        ...
        if (!["prepared", "awaiting_approval", "running"].includes(String(launch.status)))
            throw new Error("Host Bridge Launch is not executable");
        ...
        const invocationId = String(args.invocation_id ?? `host_bridge_${fabric.id}`);
        const existing = this.store.find("host_bridge_invocation", invocationId);
        const identityDigest = digest(identity);
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Host Bridge invocation idempotency conflict");
            return { invocation: existing, idempotent: true };
        }
    ...
    start(args) {
        ...
        const run = this.store.get("host_run", text(args.run_id, "run_id"));
        const receipt = this.store.get("host_activation_receipt", text(args.activation_receipt_id, "activation_receipt_id"));
        if (invocation.status === "running") {
            if (invocation.run_id !== run.id || invocation.activation_receipt_id !== receipt.id)
                throw new Error("Host Bridge start idempotency conflict");
    ...
    finish(args) {
        ...
        if (!TERMINAL.has(String(run.status)))
            throw new Error("Host Bridge can finish only after a terminal Host receipt");
        if (invocation.run_id !== run.id)
            throw new Error("Host Run does not belong to Host Bridge invocation");
        if (TERMINAL.has(String(invocation.status)))
            return { invocation, idempotent: true };
        if (invocation.status !== "running")
            throw new Error("Host Bridge invocation was not started");
