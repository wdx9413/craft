...
export class CampaignRunnerKernel {
    ...
    create(args) {
        ...
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Campaign Runner idempotency conflict");
            return { runner: existing, idempotent: true };
        }
    ...
    claim(args) {
        ...
        if (runner.lifecycle === "completed")
            throw new Error("Campaign Runner is completed");
        const slots = this.slots(runner);
        const issued = new Set(runner.issued_slot_ids);
        const next = slots.find((slot) => slot.status === "pending" && !issued.has(String(slot.id)));
        if (!next)
            return { runner, dispatch: null, next_action: "advance_campaign" };
        ...
        const dispatchId = String(args.dispatch_id ?? `campaign_dispatch_${runner.id}_${next.id}`);
        const dispatchDigest = digest(identity);
    ...
    bind(args) {
        ...
        const taskRunId = text(args.task_run_id, "task_run_id");
        if (dispatch.status === "bound" && dispatch.task_run_id === taskRunId)
            return { runner, dispatch, slot: this.store.get("eval_campaign_slot", String(dispatch.slot_id)), idempotent: true };
        if (dispatch.status !== "issued")
            throw new Error("Campaign Runner dispatch is not issuable");
        ...
        const boundIds = [...new Set([...runner.bound_slot_ids, String(slot.id)])].sort();
        const saved = this.store.save("campaign_runner", String(runner.id), { ...payload(runner), bound_slot_ids: boundIds });
    ...
    advance(args) {
        ...
        const lifecycle = ["eligible", "rejected", "inconclusive"].includes(status) ? "completed" : "collecting";
        const saved = this.store.save("campaign_runner", String(runner.id), { ...payload(runner), lifecycle, campaign_lifecycle: status, evaluation_run_id: result.evaluation?.id ?? null });
    ...
    get(args) { const runner = this.store.get("campaign_runner", text(args.runner_id, "runner_id")); return { runner, campaign: this.campaigns.get({ campaign_id: runner.campaign_id }), dispatches: this.store.list("campaign_runner_dispatch", 10_000, (item) => item.runner_id === runner.id), timeline: this.store.events(`campaign-runner:${runner.id}`) }; }
    slots(runner) { return this.store.list("eval_campaign_slot", 10_000, (item) => item.campaign_id === runner.campaign_id).sort((left, right) => String(left.id).localeCompare(String(right.id))); }
}
