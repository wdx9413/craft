...
export function estimateTokens(value) {
    if (typeof value !== "string")
        throw new Error("Token estimation requires a string");
...
export function estimatePromptTokens(texts) {
    if (!Array.isArray(texts))
        throw new Error("Token estimation requires an array of strings");
    return texts.reduce((total, item) => total + estimateTokens(item), 0);
}
export function createBudgetState(limit, reserve = 0) {
    if (!Number.isInteger(limit) || limit < 1)
        throw new Error("Token budget limit must be a positive integer");
...
export function remainingTokens(state) {
 ...
 */
export function spendTokens(state, amount) {
    if (!Number.isInteger(amount) || amount < 0)
        throw new Error("Token spend must be a non-negative integer");
...
export function truncateToBudget(value, maxTokens) {
    if (typeof value !== "string")
        throw new Error("Token truncation requires a string");
    if (!Number.isInteger(maxTokens) || maxTokens < 1)
        throw new Error("Token cap must be a positive integer");
    const estimated = estimateTokens(value);
    if (estimated <= maxTokens)
        return { text: value, truncated: false, estimated_tokens: estimated };
...
export function classifyComplexity(signals) {
    ...
    if (signals.requires_multi_step_reasoning || signals.requires_external_write
        || steps >= FRONTIER_STEPS || paths >= FRONTIER_PATHS)
        return "frontier";
    if (contextChars >= STANDARD_CONTEXT_CHARS || steps >= STANDARD_STEPS || paths >= STANDARD_PATHS)
        return "standard";
    return "small";
}
/**
...
export function routeModel(request) {
    ...
    if (start === -1)
        throw new Error(`Unsupported model tier: ${String(request.tier)}`);
