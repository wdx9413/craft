...
export class MemoryConsolidationKernel {
    ...
    consolidate(args) {
        ...
        if (!memoryIds.length)
            throw new Error("memory_ids must contain at least one id");
        ...
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Semantic memory idempotency conflict");
            return { memory: existing, idempotent: true };
        }
        for (const memory of memories)
            this.store.save("episodic_memory", String(memory.id), { ...payload(memory), consolidated: true, consolidated_into: semanticId });
    ...
    resolve(args) {
        ...
        if (!(new Set(["active", "superseded", "rejected"]).has(status)))
            throw new Error("Unsupported semantic memory status");
