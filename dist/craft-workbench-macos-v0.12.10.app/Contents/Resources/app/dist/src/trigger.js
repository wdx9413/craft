...
function fields(value) {
    ...
    if (!Array.isArray(value) || value.length > 32)
        throw new Error("allowed_fields must be an array with at most 32 fields");
    ...
    if (new Set(result).size !== result.length || result.some((field) => !/^[a-zA-Z0-9_-]{1,128}$/u.test(field) ||
        /(?:api[_-]?key|authorization|cookie|password|secret|token)/iu.test(field)))
        throw new Error("allowed_fields contains duplicate, invalid, or sensitive fields");
    return result;
}
...
export class TriggerKernel {
    ...
    subscriptionSave(args) {
        const subscriptionId = id(args.subscription_id, "subscription_id");
        ...
        if (handle.status !== "active")
            throw new Error("Webhook credential handle is not active");
        ...
        if (!new Set(["active", "paused"]).has(next.status))
            throw new Error("Trigger subscription status is unsupported");
        const existing = this.store.find("trigger_subscription", subscriptionId);
        if (!existing)
            return { subscription: this.store.create("trigger_subscription", subscriptionId, next) };
        if (Number(args.expected_version) !== Number(existing.version))
            throw new Error("Trigger subscription version conflict");
    ...
    webhookIngest(args) {
        ...
        if (subscription.status !== "active")
            throw new Error("Trigger subscription is not active");
        const eventId = id(args.event_id, "event_id");
        const rawBody = text(args.raw_body, "raw_body");
        if (Buffer.byteLength(rawBody) > 1024 * 1024)
            throw new Error("Webhook body exceeds 1 MiB");
        const timestamp = text(args.timestamp, "timestamp");
        const signedAt = Date.parse(timestamp);
        const now = instant(args.now, "now");
        if (Number.isNaN(signedAt) || Math.abs(now - signedAt) > Number(subscription.max_age_seconds) * 1000)
            throw new Error("Webhook timestamp is invalid or outside the replay window");
        ...
        if (!/^[a-f0-9]{64}$/u.test(supplied) || !timingSafeEqual(Buffer.from(expected), Buffer.from(supplied)))
            throw new Error("Webhook signature is invalid");
        const bodyDigest = `sha256:${createHash("sha256").update(rawBody).digest("hex")}`;
        const recordId = `${subscription.id}_${eventId}`;
        const existing = this.store.find("trigger_event", recordId);
        if (existing) {
            if (existing.body_digest !== bodyDigest || existing.timestamp !== timestamp)
                throw new Error("Webhook event idempotency conflict");
            return { trigger_event: existing, dispatch: existing.dispatch, idempotent: true };
        }
        let parsed;
        try {
            parsed = object(JSON.parse(rawBody), "webhook body");
        }
        catch {
            throw new Error("Webhook body must be a JSON object");
        }
        if (Object.entries(subscription.filters).some(([key, value]) => parsed[key] !== value)) {
        ...
        const projected = Object.fromEntries(subscription.allowed_fields.filter((field) => Object.hasOwn(parsed, field))
