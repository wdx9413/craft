...
export class A2ATransportKernel {
    ...
    async taskGet(args, fetchImpl = fetch) {
        const endpoint = text(args.endpoint, "endpoint");
        if (!endpoint.startsWith("https://"))
            throw new Error("A2A endpoint must use HTTPS");
        const taskId = text(args.task_id, "task_id");
        const response = await fetchImpl(`${endpoint.replace(/\/$/u, "")}/tasks/${encodeURIComponent(taskId)}`, { method: "GET", headers: { accept: "application/json" } });
        if (response.status < 200 || response.status >= 300)
            throw new Error(`A2A task HTTP ${response.status}`);
        const body = await response.json();
        if (!body || typeof body !== "object" || Array.isArray(body))
            throw new Error("A2A task response must be an object");
        const value = body;
        const status = text(value.status, "status");
        if (!new Set(["submitted", "working", "input-required", "completed", "failed", "canceled", "rejected"]).has(status))
            throw new Error("A2A task status is unsupported");
        return { task_id: taskId, status, artifact_refs: Array.isArray(value.artifact_refs) ? value.artifact_refs : [], raw_content: false, response_digest: digest(value) };
    }
    async taskCancel(args, fetchImpl = fetch) {
        const endpoint = text(args.endpoint, "endpoint");
        if (!endpoint.startsWith("https://"))
            throw new Error("A2A endpoint must use HTTPS");
        const taskId = text(args.task_id, "task_id");
        const response = await fetchImpl(`${endpoint.replace(/\/$/u, "")}/tasks/${encodeURIComponent(taskId)}:cancel`, { method: "POST", headers: { "content-type": "application/json", accept: "application/json" }, body: JSON.stringify({ task_id: taskId, reason_digest: digest(args.reason ?? "user_requested"), execution_authority: false }) });
        if (response.status < 200 || response.status >= 300)
            throw new Error(`A2A cancel HTTP ${response.status}`);
        const body = await response.json();
        if (!body || typeof body !== "object" || Array.isArray(body))
            throw new Error("A2A cancel response must be an object");
        const value = body;
        return { task_id: taskId, status: text(value.status, "status"), canceled: true, raw_content: false, response_digest: digest(value) };
    }
}
