...
function deliveryRate(value) { return value && ["accepted", "ready_for_delivery"].includes(String(value.status)) ? 1 : 0; }
...
export class EvalCampaignReportKernel {
    ...
    report(args) {
        ...
        for (const slot of slots)
            pairs.set(`${slot.case_id}:${slot.trial}`, [...(pairs.get(`${slot.case_id}:${slot.trial}`) ?? []), slot]);
        ...
        const reportId = String(args.report_id ?? `eval_campaign_report_${digest(identity).slice(-16)}`);
        const existing = this.store.find("eval_campaign_report", reportId);
        const reportDigest = digest(identity);
        if (existing) {
            if (existing.report_digest !== reportDigest)
                throw new Error("Eval Campaign report idempotency conflict");
    ...
    sample(key, pair) {
        const baseline = pair.find((item) => item.arm === "baseline");
        const candidate = pair.find((item) => item.arm === "candidate");
        if (!baseline || !candidate)
            throw new Error("Eval Campaign report pair is incomplete");
        const baselineDelivery = this.delivery(baseline);
        const candidateDelivery = this.delivery(candidate);
        if (!baselineDelivery || !candidateDelivery)
            return { key, status: "awaiting_delivery", baseline_delivery_id: baselineDelivery?.id ?? null, baseline_delivery_version: baselineDelivery?.version ?? null, candidate_delivery_id: candidateDelivery?.id ?? null, candidate_delivery_version: candidateDelivery?.version ?? null };
        const left = deliveryRate(baselineDelivery);
        const right = deliveryRate(candidateDelivery);
    ...
    delivery(slot) { if (!slot.task_run_id)
        return null; const run = this.store.get("task_run", String(slot.task_run_id)); return this.store.list("work_delivery", 10_000, (item) => item.launch_id === run.launch_id).at(-1) ?? null; }
}
...
//# sourceMappingURL=eval-campaign-report.js.map