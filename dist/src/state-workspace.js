...
function fileDigest(path) { return `sha256:${createHash("sha256").update(readFileSync(path)).digest("hex")}`; }
function relativePath(value, name) { const item = text(value, name).replaceAll("\\", "/"); if (isAbsolute(item) || item.split("/").includes(".."))
    throw new Error(`${name} must be relative to the workspace`); return item.replace(/^\.\//, "") || "."; }
...
function files(root, path) {
    const target = nested(root, path);
    if (!existsSync(target))
        return [];
    const stat = lstatSync(target);
    if (stat.isSymbolicLink())
        throw new Error(`state adapters do not follow symbolic links: ${path}`);
    if (kind(stat, path) === "file")
        return [{ path, digest: fileDigest(target), size_bytes: stat.size }];
...
export class StateWorkspaceKernel {
    ...
    observe(args) {
        ...
        if (!["file_tree", "file_artifact"].includes(adapter))
            throw new Error("State Workspace adapter is unsupported");
        const root = text(workspace.root_path, "workspace.root_path");
        const paths = adapter === "file_tree"
            ? workspace.include_paths.map((item) => relativePath(item, "workspace.include_paths")).sort()
            : (args.paths ?? []).map((item) => relativePath(item, "paths")).sort();
        if (!paths.length)
            throw new Error("file_artifact observation requires at least one declared path");
        if (new Set(paths).size !== paths.length)
            throw new Error("State Workspace paths must be unique");
        ...
        if (new Set(entries.map((item) => item.path)).size !== entries.length)
            throw new Error("State Workspace paths must not overlap");
        const artifactIds = args.artifact_ids === undefined ? [] : args.artifact_ids.map((item) => text(item, "artifact_ids"));
        ...
        const snapshotId = String(args.snapshot_id ?? `state_snapshot_${workspace.id}_${digest(identity).slice(-16)}`);
        const existing = this.store.find("state_snapshot", snapshotId);
        const snapshotDigest = digest(identity);
        if (existing) {
            if (existing.snapshot_digest !== snapshotDigest)
                throw new Error("State Snapshot idempotency conflict");
    ...
    compare(args) {
        const before = this.store.get("state_snapshot", text(args.before_snapshot_id, "before_snapshot_id"));
        const after = this.store.get("state_snapshot", text(args.after_snapshot_id, "after_snapshot_id"));
        if (before.workspace_id !== after.workspace_id)
            throw new Error("State Snapshots must belong to one workspace");
...
//# sourceMappingURL=state-workspace.js.map