...
function numberLimit(limits, name, fallback, minimum, maximum) {
    ...
    if (!Number.isFinite(value) || value < minimum || value > maximum)
        throw new Error(`Docker ${name} limit is unsupported`);
    return value;
}
function scrub(value) {
    return value.replace(/(api[_-]?key|authorization|cookie|password|secret|token)\s*[:=]\s*[^\s]+/giu, "$1=[REDACTED]");
}
export function dockerRequestDigest(image, requestedCommand) {
...
export function runDocker(argv, timeoutMs, outputLimit = OUTPUT_LIMIT, spawnProcess = spawn) {
...
function dockerFlags(capabilities, workspace) {
    ...
    if (Object.keys(limits).some((name) => !known.has(name)))
        throw new Error("Docker Adapter received an unsupported resource limit");
    ...
    if (workspace)
        flags.push("--mount", `type=bind,src=${workspace},dst=/workspace`, "--workdir", "/workspace");
    return { flags, timeoutMs };
}
export class DockerSandboxAdapter {
    runner;
    constructor(runner = runDocker) { this.runner = runner; }
    async probe(imageValue, capabilities) {
        ...
        for (const argv of [["version", "--format", "{{.Server.Version}}"], ["image", "inspect", image],
            ...
            if (result.code !== 0 || result.timed_out || result.output_limited)
                throw new Error("Docker Sandbox probe failed closed");
        }
    ...
    async conformance(probeIdValue, imageValue, capabilities, runtimeRootValue) {
        ...
        const workspace = resolve(runtimeRoot, containerName(probeId));
        ...
        for (const [check, script, expectation] of checks) {
            ...
            if (expectation === "timeout")
                await this.runner(["rm", "-f", name], configured.timeoutMs, OUTPUT_LIMIT);
        }
        ...
        if (results.some((result) => result.passed !== true))
            throw new Error("Docker Sandbox conformance failed closed");
    ...
    async execute(args) {
        const image = text(args.image, "image");
