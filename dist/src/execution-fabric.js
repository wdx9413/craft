...
function payload(record) { const { id: _id, version: _version, created_at: _createdAt, updated_at: _updatedAt, ...rest } = record; return rest; }
/**
...
export class ExecutionFabricKernel {
    ...
    create(args) {
        ...
        if (loop.task_id !== manifest.task_id || contract.activation_profile === null ||
            contract.activation_profile.id !== manifest.profile_id || Number(contract.activation_profile.version) !== Number(manifest.profile_version)) {
        ...
        const fabricId = String(args.fabric_id ?? `execution_fabric_${loop.id}`);
        const existing = this.store.find("execution_fabric", fabricId);
        const identityDigest = digest(identity);
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Execution Fabric idempotency conflict");
            return { fabric: existing, idempotent: true };
        }
    ...
    advance(args) {
        ...
        if (receipt.work_loop_id !== fabric.work_loop_id)
            throw new Error("Work Loop receipt does not belong to Execution Fabric");
        ...
        if (activationReceipt && activationReceipt.manifest_id !== fabric.manifest_id)
            throw new Error("Host Activation receipt does not belong to Execution Fabric");
        ...
        const advanceId = String(args.advance_id ?? `execution_fabric_advance_${fabric.id}_${receipt.id}_${receipt.version}_${activationReceipt?.id ?? "none"}`);
        const existing = this.store.find("execution_fabric_advance", advanceId);
        const identityDigest = digest(identity);
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Execution Fabric advance idempotency conflict");
            return { fabric, advance: existing, idempotent: true };
        }
...
//# sourceMappingURL=execution-fabric.js.map