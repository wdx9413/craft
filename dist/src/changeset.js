...
function patches(value) {
    ...
    return value.map((item, index) => {
        ...
        if (!new Set(["set", "remove"]).has(op))
            throw new Error(`patches[${index}].op is unsupported`);
        const baseVersion = Number(raw.base_version);
        if (!Number.isInteger(baseVersion) || baseVersion < 1)
            throw new Error(`patches[${index}].base_version must be a positive integer`);
        pointer(raw.path);
        if (op === "set" && !Object.hasOwn(raw, "value"))
            throw new Error(`patches[${index}].value is required for set`);
        return { object_id: id(raw.object_id, `patches[${index}].object_id`, "work_object"), base_version: baseVersion,
            op: op, path: String(raw.path), ...(op === "set" ? { value: raw.value } : {}) };
    });
}
...
function apply(root, patch) {
    ...
    for (const part of parts.slice(0, -1)) {
        ...
        if (existing === undefined)
            ...
            throw new Error(`patch path crosses a non-object value: ${patch.path}`);
        parent = parent[part];
    }
...
export class ChangeSetKernel {
    store;
    workbench;
    constructor(store, workbench) { this.store = store; this.workbench = workbench; }
    create(args) {
        ...
        for (const patch of normalized) {
            ...
            if (object.workspace_id !== workspaceId)
                throw new Error("Patched objects must belong to the workspace");
            if (!this.store.find("work_object", patch.object_id, patch.base_version))
                throw new Error("Patch base object version does not exist");
        }
    ...
    apply(args) {
        ...
        if (changeSet.status === "applied")
            return { change_set: changeSet, idempotent: true };
        if (changeSet.status !== "draft")
            throw new Error("Only a draft ChangeSet can be applied");
        ...
        if (!Number.isInteger(expected) || expected !== Number(workspace.state_revision))
            throw new Error("Workspace state changed; refresh before applying ChangeSet");
        const preview = this.preview({ change_set_id: changeSet.id });
        if (preview.conflicts.length)
            throw new Error("ChangeSet has field conflicts that require resolution");
        const grouped = new Map();
        for (const patch of changeSet.patches)
            grouped.set(patch.object_id, [...(grouped.get(patch.object_id) ?? []), patch]);
        ...
        for (const objectId of impact.impacted_object_ids) {
            ...
            for (const patch of grouped.get(objectId) ?? [])
                data = apply(data, patch);
