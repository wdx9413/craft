...
function items(value) { if (!Array.isArray(value) || !value.length || value.some((item) => item === null || typeof item !== "object" || Array.isArray(item)))
    throw new Error("items must be a non-empty object array"); return value; }
function positiveInteger(value, name, fallback) { const parsed = value === undefined ? fallback : Number(value); if (!Number.isInteger(parsed) || parsed < 1 || parsed > 100)
...
export class DeliveryEvaluationKernel {
    ...
    run(args) {
        const environment = text(args.environment_fingerprint, "environment_fingerprint");
        const budget = text(args.budget_fingerprint, "budget_fingerprint");
        const minTrials = positiveInteger(args.min_trials, "min_trials", 2);
        const comparisons = items(args.items).map((item) => this.compare({ case_id: text(item.case_id, "items.case_id"), baseline_delivery_id: text(item.baseline_delivery_id, "items.baseline_delivery_id"), candidate_delivery_id: text(item.candidate_delivery_id, "items.candidate_delivery_id"), environment_fingerprint: environment, budget_fingerprint: budget }).comparison);
        ...
        const runId = String(args.run_id ?? `delivery_evaluation_run_${digest(identity).slice(7, 23)}`);
        const existing = this.store.find("delivery_evaluation_run", runId);
        const runDigest = digest(identity);
        if (existing) {
            if (existing.run_digest !== runDigest)
                throw new Error("Delivery evaluation run idempotency conflict");
            return { run: existing, comparisons, idempotent: true };
        }
