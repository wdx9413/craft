...
function inferMetric(goal, requested) {
    if (requested !== undefined) {
        const metric = text(requested, "metric").toLowerCase();
        if (!["methods", "functions", "lines", "statements", "branches", "all"].includes(metric))
            throw new Error("metric is unsupported");
        return metric;
    }
    if (/(?:方法|函数|method|function)/iu.test(goal))
        return "methods";
    if (/(?:行|line)/iu.test(goal))
        return "lines";
    if (/(?:分支|branch)/iu.test(goal))
        return "branches";
    return null;
}
...
function inferScope(goal, requested) {
    if (requested !== undefined) {
        ...
        if (!["changed", "workspace", "specified"].includes(scope))
            throw new Error("scope is unsupported");
        return scope;
    }
    return /(?:增量|变更|diff|incremental|changed)/iu.test(goal) ? "changed" : "workspace";
}
function inferTaskType(goal) {
    if (/(?:覆盖率|coverage)/iu.test(goal))
        return "coverage_verification";
    if (/(?:测试|test)/iu.test(goal))
        return "test_verification";
    if (/(?:研究|research|分析|analy[sz]e)/iu.test(goal))
        return "research";
    if (/(?:写|生成|create|write|build|制作)/iu.test(goal))
        return "creation";
    return "governed_work";
}
function clarification(metric, scope, goal) {
    const questions = [];
    if (/(?:覆盖率|coverage)/iu.test(goal) && metric === null)
        questions.push({ id: "coverage_metric", question: "覆盖率按方法、行、分支还是全部指标验收？", required: true, options: ["methods", "lines", "branches", "all"] });
    if (scope === "specified")
        questions.push({ id: "coverage_scope", question: "请提供要验收的具体文件或方法范围。", required: true });
    return questions;
}
export class IntentCompilerKernel {
    ...
    compile(args) {
        ...
        const route = questions.length ? "clarification" : (args.require_governance === true || goal.length > 24 || /(?:please|请|帮我|实现|完成|修复|build|run|test|测试|覆盖率|研究|生成|写)/iu.test(goal)) ? "governed" : "simple";
        ...
        if (existing) {
            if (existing.request_digest !== requestDigest)
                throw new Error("Task intent idempotency conflict");
    ...
    acceptanceCompile(args) {
        const intent = this.store.get("task_intent", text(args.intent_id, "intent_id"));
        if (intent.route === "clarification")
            throw new Error("Task intent requires clarification before acceptance compilation");
        if (!intent.acceptance_required)
            throw new Error("Simple intents do not require an acceptance contract");
        ...
        if (existing) {
            if (existing.definition_digest !== definition.definition_digest)
                throw new Error("Acceptance contract idempotency conflict");
    ...
    get(args) { return { intent: this.store.get("task_intent", text(args.intent_id, "intent_id")) }; }
    acceptanceGet(args) { return { acceptance: this.store.get("acceptance_contract", text(args.acceptance_id, "acceptance_id")) }; }
}
