...
export class EvaluationOperationsKernel {
    store;
    campaigns;
    constructor(store, campaigns) { this.store = store; this.campaigns = campaigns; }
    programSave(args) {
        ...
        if (developmentCaseIds.some((id) => heldOutCaseIds.includes(id)))
            throw new Error("Evaluation Program case partitions must not overlap");
        ...
        if (!new Set(["active", "paused"]).has(definition.lifecycle))
            throw new Error("Evaluation Program lifecycle is unsupported");
        const programId = text(args.program_id, "program_id");
        ...
        if (existing) {
            if (existing.definition_digest !== definitionDigest)
                throw new Error("Evaluation Program idempotency conflict");
    ...
    due(args) {
        ...
        if (program.lifecycle !== "active")
            return { program, due: false, reason: "program_paused" };
    ...
    plan(args) {
        ...
        if (due.due !== true)
            throw new Error(`Evaluation Program is not due: ${String(due.reason)}`);
        ...
        if (!new Set(["development", "held_out"]).has(partition))
            throw new Error("Evaluation Program partition is unsupported");
        if (partition === "held_out" && text(args.independent_approval_ref, "independent_approval_ref") === program.owner_ref)
            throw new Error("Held-out planning requires an independent approval reference");
        const baselineHarness = text(args.baseline_harness, "baseline_harness");
        const candidateHarness = text(args.candidate_harness, "candidate_harness");
        if (baselineHarness === candidateHarness)
            throw new Error("Evaluation Program requires distinct baseline and candidate Harnesses");
        ...
        const trials = integer(args.trials_per_case, "trials_per_case", 2, 1, 20);
        const environmentDigest = digest(environment);
        const budgetDigest = digest(budget);
        const campaign = partition === "held_out" ? this.campaigns.create({ campaign_id: String(args.campaign_id ?? `evaluation_program_campaign_${program.id}_${partition}_${now}`),
            ...
            acceptance_ref: text(args.acceptance_ref, "acceptance_ref"), environment, budget }).campaign : null;
        ...
        const runId = String(args.program_run_id ?? `evaluation_program_run_${program.id}_${digest(runIdentity).slice(-16)}`);
        const existing = this.store.find("evaluation_program_run", runId);
        const runDigest = digest(runIdentity);
        if (existing) {
            if (existing.run_digest !== runDigest)
                throw new Error("Evaluation Program Run idempotency conflict");
    ...
    validateCases(ids, partition) {
        for (const id of ids) {
            const item = this.store.get("delivery_evaluation_case", id);
            if (item.partition !== partition || item.sanitized !== true)
                throw new Error(`Evaluation Program requires sanitized ${partition} Cases`);
            if (partition === "held_out" && !item.approved_by)
                throw new Error("Held-out Evaluation Case is missing independent approval");
        }
    }
}
//# sourceMappingURL=evaluation-operations.js.map