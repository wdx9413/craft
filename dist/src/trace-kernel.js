...
function optional(value) { return value === undefined || value === null ? null : text(value, "reference"); }
function object(value, name, fallback = {}) {
    if (value === undefined)
        return fallback;
...
function safeData(value, name) {
    ...
    if (/["']?(?:api[_-]?key|authorization|cookie|password|secret|token)["']?\s*[:=]/iu.test(JSON.stringify(result)))
...
export class TraceKernel {
    ...
    append(args) {
        ...
        if (TERMINAL.has(String(trace.status)))
            throw new Error("Cannot append to a terminal Trace");
        ...
        if (!Number.isInteger(sequence) || (!existingRequested && sequence !== Number(trace.next_sequence) + 1))
            throw new Error("Trace event sequence must be the next contiguous integer");
        const trust = String(args.trust ?? (args.source === "human" ? "human" : "untrusted"));
        if (!TRUST.has(trust))
            throw new Error("Trace event trust is unsupported");
    ...
    feedback(args) {
        const traceId = text(args.trace_id, "trace_id");
        this.store.get("trace", traceId);
        const signal = text(args.signal, "signal");
        if (!SIGNALS.has(signal))
            throw new Error("Trace feedback signal is unsupported");
        const summary = text(args.summary, "summary");
        const feedbackId = String(args.feedback_id ?? `feedback_${traceId}_${digest({ signal, summary }).slice(-16)}`);
        const existing = this.store.find("trace_feedback", feedbackId);
        if (existing) {
            if (existing.signal !== signal || existing.summary !== summary)
                throw new Error("Trace feedback idempotency conflict");
    ...
    finalize(args) {
        ...
        if (!TERMINAL.has(status))
            throw new Error("Trace final status is unsupported");
        if (TERMINAL.has(String(trace.status)))
    ...
    replayBundle(args) {
        const result = this.get(args);
        const trace = result.trace;
        const required = ["model_fingerprint", "environment_fingerprint", "capability_fingerprint", "policy_fingerprint"];
        const replayable = required.every((key) => typeof trace[key] === "string" && String(trace[key]).length > 0) && result.events.every((event) => event.event_kind === "trace.finalized" || (event.data_digest && event.action_contract !== null));
        return { schema: TRACE_SCHEMA_VERSION, trace_id: trace.id, replayable, raw_content: false, trace, events: result.events, feedback: result.feedback, replay: { required_fingerprints: required, state_refs: result.events.map((event) => ({ sequence: event.sequence, before: event.workspace_before, after: event.workspace_after })) } };
    }
    compileCase(args) {
        const result = this.get({ trace_id: args.trace_id });
        const trace = result.trace;
        if (!TERMINAL.has(String(trace.status)))
            throw new Error("Trace Case requires a terminal Trace");
        const partition = String(args.partition ?? "development");
        if (!(new Set(["development", "held_out"]).has(partition)))
            throw new Error("Trace Case partition is unsupported");
        const approvedBy = args.approved_by === undefined ? null : text(args.approved_by, "approved_by");
        if (partition === "held_out" && approvedBy === null)
            throw new Error("held_out Trace Case requires approved_by");
        const caseId = String(args.case_id ?? `trace_case_${trace.id}`);
        const eventIds = result.events.map((event) => String(event.id));
        const identity = { source_trace_id: trace.id, trace_version: trace.version, event_ids: eventIds, partition, acceptance_contract_ref: optional(args.acceptance_contract_ref), summary: text(args.summary, "summary") };
        const caseDigest = digest(identity);
        const existing = this.store.find("trace_case", caseId);
        if (existing) {
            if (existing.case_digest !== caseDigest)
                throw new Error("Trace Case idempotency conflict");
    ...
    retentionPlan(args) {
        ...
        const maxEvents = Number(args.max_events ?? 100_000);
        if (!Number.isInteger(maxDays) || maxDays < 1)
            throw new Error("max_days must be a positive integer");
        if (!Number.isInteger(maxEvents) || maxEvents < 1)
            throw new Error("max_events must be a positive integer");
        const existing = this.store.find("trace_policy", policyId);
        const identity = { max_days: maxDays, max_events: maxEvents, pii_mode: String(args.pii_mode ?? "digest_only") };
        if (existing) {
            if (existing.identity_digest !== digest(identity))
                throw new Error("Trace policy idempotency conflict");
    ...
    appendTrial(args) {
        ...
        const traceId = `trial:${trialId}`;
        if (!this.store.find("trace", traceId))
            this.start({ trace_id: traceId, task_id: String(this.store.get("trial", trialId).task_id), trial_id: trialId, environment_fingerprint: args.environment_fingerprint });
...
//# sourceMappingURL=trace-kernel.js.map