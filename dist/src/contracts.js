...
function strings(value, name, minimum = 1) { if (!Array.isArray(value) || value.length < minimum)
...
export class ContractInferenceKernel {
    ...
    observe(args) {
        ...
        const adapter = text(args.adapter_kind, "adapter_kind");
        if (!ADAPTERS.has(adapter))
            throw new Error("Contract adapter_kind is unsupported");
        const effect = text(args.observed_effect, "observed_effect");
        if (!EFFECTS.has(effect))
            throw new Error("Contract observed_effect is unsupported");
        const outcome = text(args.outcome, "outcome");
        if (!new Set(["succeeded", "failed"]).has(outcome))
            throw new Error("Contract observation outcome is unsupported");
        ...
        const fingerprint = digest({ ...identity, outcome, evidence_ids: evidenceIds });
        const existing = this.store.find("contract_observation", observationId);
        if (existing) {
            if (existing.fingerprint !== fingerprint)
                throw new Error("Contract observation idempotency conflict");
    ...
    infer(args) {
        const ids = strings(args.observation_ids, "observation_ids", 2);
        const observations = ids.map((id) => this.store.get("contract_observation", id));
        const first = observations[0];
        const same = ["task_id", "adapter_kind", "endpoint", "operation", "input_schema_digest", "output_schema_digest", "observed_effect"];
        if (observations.some((item) => same.some((key) => item[key] !== first[key])))
            throw new Error("Contract observations are not structurally consistent");
        if (observations.some((item) => item.outcome !== "succeeded"))
            throw new Error("Contract inference requires successful observations");
        const candidateId = String(args.candidate_id ?? `contract_candidate_${randomUUID().replaceAll("-", "")}`);
        const candidateFingerprint = digest({ observation_ids: [...ids].sort() });
        const existing = this.store.find("contract_candidate", candidateId);
        if (existing) {
            if (existing.candidate_fingerprint !== candidateFingerprint)
                throw new Error("Contract candidate idempotency conflict");
    ...
    review(args) {
        ...
        if (candidate.status !== "candidate")
            throw new Error("Contract candidate is not awaiting review");
        const decision = text(args.decision, "decision");
        if (!new Set(["accept", "reject"]).has(decision))
            throw new Error("Contract review decision is unsupported");
        const corrected = args.corrected_contract === undefined ? null : object(args.corrected_contract, "corrected_contract");
        if (decision === "reject" && corrected)
            throw new Error("Rejected contracts cannot include corrections");
        ...
        if (decision === "accept" && !EFFECTS.has(String(contract.effect)))
            throw new Error("Reviewed contract effect is unsupported");
    ...
    verify(args) {
        ...
        if (candidate.status !== "reviewed")
            throw new Error("Contract candidate is not reviewed");
        ...
        if (sandbox.status !== "verified")
            throw new Error("Contract verification requires a verified Sandbox Profile");
        const trial = this.store.get("trial", text(args.trial_id, "trial_id"));
        if (trial.task_id !== candidate.task_id)
            throw new Error("Contract verification Trial belongs to another task");
        ...
        if (outcome.verdict !== "passed" || !Array.isArray(outcome.evidence_ids) || !outcome.evidence_ids.length)
            throw new Error("Contract verification requires a passed evidence-backed Trial");
    ...
    diff(args) {
        ...
        if (baseline.adapter_kind !== candidate.adapter_kind || baseline.endpoint !== candidate.endpoint || baseline.operation !== candidate.operation)
            throw new Error("Contract diff subjects are unrelated");
        const left = object(baseline.reviewed_contract, "baseline reviewed_contract");
        const right = object(candidate.reviewed_contract, "candidate reviewed_contract");
        const changes = [];
        const add = (field, breaking) => { if (canonical(left[field]) !== canonical(right[field]))
            changes.push({ field, breaking }); };
        add("input_schema", true);
        add("output_schema", true);
        add("effect", true);
        add("credential_handles_required", true);
        add("idempotency", left.idempotency === true && right.idempotency !== true);
        add("compensation", left.compensation === true && right.compensation !== true);
    ...
    publish(args) {
        ...
        if (contract.status !== "verified")
            throw new Error("Only a verified Contract can be published");
        const publisher = text(args.publisher, "publisher");
        if (publisher === contract.reviewer)
            throw new Error("Contract publication requires an independent publisher");
        ...
        if (!new Set(["read_only", "local_write", "external_write", "destructive"]).has(effect))
            throw new Error("Contract publication effect is not executable");
        ...
        if (existing) {
            if (existing.fingerprint !== fingerprint)
                throw new Error("Contract publication idempotency conflict");
    ...
    rollback(args) {
        const publication = this.store.get("contract_publication", text(args.publication_id, "publication_id"));
        if (publication.status !== "active")
            throw new Error("Contract publication is not active");
        const asset = this.store.get("capability_asset", String(publication.asset_id));
        if (Number(asset.version) !== Number(publication.asset_version))
            throw new Error("Contract publication no longer owns the active capability version");
