...
const schemaFor = (name) => {
    ...
    if (["limit", "version", "capacity", "max_concurrency", "size_bytes", "subject_version", "expected_version", "max_candidates",
        ...
        return { type: "integer" };
    if (["score", "value", "threshold", "confidence", "min_pass_rate_delta", "max_cost_regression_ratio", "max_duration_regression_ratio", "max_budget_ratio", "baseline", "candidate", "minimum_agreement", "max_budget_usd", "input_per_million", "output_per_million"].includes(name))
        return { type: "number" };
...
export const TOOLS = [
    ...
    tool("craft_logical_activation_resolve", "Revalidate and load digest-pinned local capability documents as bounded read-only context. It never executes their instructions.", ["plan_id"], false, ["resolution_id", "audit_id", "max_chars"]),
    ...
    tool("craft_runtime_dispatch", "Lease only pending policy-approved runtime operations to a host.", ["run_id", "claimed_by"], false, ["capacity", "kinds", "effects", "authorization_requests", "notification_refs", "now"]),
    ...
    tool("craft_runtime_adapter_dispatch", "Lease only operations declared by one Runtime Adapter contract.", ["runtime_adapter_id", "run_id"], false, ["runtime_adapter_version", "capacity", "authorization_requests", "notification_refs", "now"]),
    ...
    tool("craft_capability_context_dispatch_prepare", "Prepare a Codex or Claude dispatch with a digest-pinned logical capability plan as bounded read-only prompt context.", ["host", "task_id", "workspace", "prompt", "plan_id"], false, ["dispatch_id", "sandbox", "model", "max_turns", "max_budget_usd", "timeout_ms", "output_limit", "max_chars"]),
    tool("craft_capability_context_dispatch_execute", "Revalidate a capability-bound dispatch immediately before execution; it fails closed if its local capability context drifted.", ["host", "dispatch_id", "prompt"], false, ["authorization_request_id", "notification_ref", "now"]),
    ...
    tool("craft_work_object_put", "Create or revise a typed workspace object with explicit dependencies and optimistic concurrency.", ["workspace_id", "object_type", "name"], false, ["object_id", "data", "depends_on", "source_paths", "artifact_ids", "status", "evidence_ids", "expected_state_revision"]),
    tool("craft_work_object_list", "List the human/Agent shared objects in one workspace.", ["workspace_id"], true, ["status", "limit"]),
    tool("craft_workspace_impact", "Preview the transitive object impact of changed objects or source paths without mutating state.", ["workspace_id"], true, ["object_ids", "affected_paths"]),
    tool("craft_workspace_change_apply", "Record an intervention and mark its transitive dependents for review with revision conflict protection.", ["workspace_id", "summary"], false, ["change_id", "object_ids", "affected_paths", "source", "expected_state_revision"]),
    tool("craft_memory_remember", "Store a scoped, attributable fact, preference, decision, or experience.", ["kind", "scope", "content", "source"], false, ["memory_id", "task_id", "workspace_id", "applies_to", "valid_until", "evidence_ids", "supersedes_id"]),
    ...
    tool("craft_context_assemble", "Build a bounded context capsule from current workspace objects and valid scoped memories.", ["query"], true, ["task_id", "workspace_id", "limit", "max_chars"]),
    tool("craft_context_profile_save", "Save a versioned, scoped context selection policy; it does not grant execution authority.", ["name"], false, ["profile_id", "task_id", "workspace_id", "memory_kinds", "object_types", "required_memory_ids", "required_object_ids", "max_items", "max_chars"]),
    ...
    tool("craft_context_profile_assemble", "Assemble a bounded, attributable context using one exact Context Profile.", ["profile_id", "query"], true, ["profile_version", "task_id", "workspace_id"]),
    tool("craft_task_graph_create", "Create a domain-neutral task dependency graph with optional exact Context Profile bindings; it does not dispatch Agents.", ["name", "nodes"], false, ["graph_id", "task_id", "workspace_id"]),
    ...
    tool("craft_change_set_create", "Create a durable field-level ActionPatch set against exact work-object versions.", ["workspace_id", "summary", "author", "patches"], false, ["change_set_id", "intent"]),
    ...
    tool("craft_change_set_apply", "Atomically apply a conflict-free ChangeSet and invalidate only dependent work objects.", ["change_set_id"], false, ["expected_state_revision"]),
    ...
    tool("craft_budget_reserve", "Reserve resource capacity before exploration or parallel execution with an idempotent reservation ID.", ["budget_id", "resources"], false, ["reservation_id", "purpose", "trial_id"]),
    ...
    tool("craft_durable_wait_create", "Persist an approval, event, or time wait so the execution environment can be released.", ["task_id", "condition"], false, ["wait_id", "workspace_id", "event_key", "resume_after", "snapshot_refs", "policy_fingerprint", "trial_id"]),
    tool("craft_durable_wait_resume", "Resume a durable wait only after its signal, workspace revision, and policy are revalidated.", ["wait_id", "signal"], false, ["now", "signal_key", "policy_fingerprint"]),
    tool("craft_external_event_ingest", "Idempotently store an untrusted external signal and resume only exact matching durable waits; it never grants execution authority.", ["event_id", "event_key"], false, ["task_id", "source", "observed_at", "payload", "policy_fingerprint"]),
    tool("craft_durable_wait_sweep", "Resume due time waits in a bounded batch and revalidate each supplied current policy fingerprint.", [], false, ["now", "limit", "policy_fingerprints"]),
    tool("craft_fallback_contract_save", "Attach explicit drift triggers and a bounded fallback strategy to an exact subject version.", ["subject_type", "subject_id", "subject_version", "triggers", "strategy"], false, ["contract_id", "max_attempts", "budget_id", "estimated_resources"]),
    tool("craft_fallback_evaluate", "Evaluate a runtime drift signal, reserve fallback budget, or stop at the configured boundary.", ["contract_id", "trigger"], false, ["event_id", "task_id", "trial_id", "start_trial", "case_id", "environment", "trial_budget", "observation"]),
    tool("craft_fallback_complete", "Close a running fallback with an immutable Trial Outcome, evidence references, and actual budget settlement.", ["event_id", "verdict", "summary"], false, ["trial_id", "failure_type", "scores", "costs", "actual_resources", "evidence_ids", "source"]),
    tool("craft_credential_handle_register", "Register only a reference to a host-managed environment secret; Craft never reads or returns its value.", ["provider", "secret_ref"], false, ["handle_id", "description", "header_name", "prefix"]),
    ...
    tool("craft_credential_lease_issue", "Issue a short-lived task-bound credential lease for exact HTTPS hosts and actions.", ["task_id", "handle_id", "allowed_hosts", "allowed_actions"], false, ["lease_id", "approval_required_actions", "ttl_seconds", "now"]),
    tool("craft_credential_lease_revoke", "Revoke a credential lease idempotently.", ["lease_id", "reason"], false),
    tool("craft_egress_authorize", "Authorize one exact outbound request without exposing its secret; the trusted broker performs injection.", ["lease_id", "url", "action", "request_digest"], false, ["receipt_id", "approval_ref", "now", "untrusted_input"]),
    tool("craft_egress_request_digest", "Compute the canonical method, URL, caller-header, and body digest for an outbound request.", ["method", "url"], true, ["headers", "body"]),
    tool("craft_egress_execute", "Execute one exact authorization through the trusted HTTPS broker; secrets are injected only at the pinned connection boundary.", ["authorization_id", "method"], false, ["headers", "body", "timeout_ms", "output_limit"]),
    tool("craft_sandbox_egress_deliver", "Keep a Sandbox offline while the trusted broker delivers one sanitized, untrusted response into its private inbox.", ["binding_id", "ticket_id", "authorization_id", "method"], false, ["headers", "body", "output_name", "timeout_ms", "output_limit"]),
    tool("craft_external_effect_prepare", "Prepare an approved external write with exact request, idempotency, and optional compensation contracts.", ["task_id", "provider", "action", "target", "effect", "request_digest", "idempotency_key", "approval_ref"], false, ["effect_id", "trial_id", "compensation"]),
    ...
    tool("craft_external_effect_report", "Record a succeeded, failed, or indeterminate remote effect result with Evidence.", ["effect_id", "receipt_id", "status", "evidence_ids"], false, ["remote_operation_id", "response_digest"]),
    tool("craft_external_effect_resolve", "Resolve an indeterminate remote effect only through an evidence-backed human decision.", ["effect_id", "resolution", "resolver_type", "approval_ref", "evidence_ids"], false, ["remote_operation_id"]),
    tool("craft_effect_reconciliation_execute", "Run one pre-authorized read-only GET and reconcile an ambiguous effect from an explicit HTTP-status contract.", ["effect_id", "authorization_id", "succeeded_http_statuses", "failed_http_statuses"], false, ["reconciliation_id", "headers", "timeout_ms", "output_limit"]),
    tool("craft_effect_compensation_issue", "Issue the declared compensation for a succeeded external effect after explicit approval.", ["effect_id", "approval_ref"], false, ["compensation_id"]),
    tool("craft_effect_compensation_report", "Record compensation success, failure, or ambiguity with Evidence.", ["compensation_id", "status", "evidence_ids"], false, ["response_digest"]),
    tool("craft_effect_compensation_execute", "Execute an exact pre-authorized compensation and preserve definitive, failed, or ambiguous outcomes.", ["effect_id", "compensation_id", "authorization_id", "approval_ref", "method", "succeeded_http_statuses", "failed_http_statuses"], false, ["headers", "body", "timeout_ms", "output_limit"]),
    tool("craft_effect_saga_create", "Bind ordered same-task external effects and derive forward or reverse compensation work.", ["task_id", "effect_ids"], false, ["saga_id", "trial_id"]),
    tool("craft_effect_saga_get", "Read a Saga's current effects, derived state, and next safe action.", ["saga_id"], true),
    tool("craft_recovery_queue_refresh", "Project durable waits and uncertain external effects into a bounded persistent recovery queue.", [], false, ["now", "limit"]),
    ...
    tool("craft_recovery_work_claim", "Lease the highest-priority compatible recovery item without executing it.", ["worker_id", "actions"], false, ["now", "lease_seconds"]),
    tool("craft_recovery_work_report", "Close a leased recovery item with Evidence, rejecting stale source state.", ["item_id", "lease_token", "outcome", "summary"], false, ["evidence_ids", "now"]),
    tool("craft_recovery_lease_recover", "Return expired recovery leases to the open queue in a bounded sweep.", [], false, ["now", "limit"]),
    tool("craft_trigger_subscription_save", "Create or update a versioned signed-webhook subscription with exact filters, projection, throttle, and optional budget.", ["subscription_id", "task_id", "name", "event_key", "handle_id"], false, ["filters", "allowed_fields", "max_age_seconds", "throttle_seconds", "budget_id", "per_event_resources", "status", "expected_version"]),
    tool("craft_webhook_trigger_ingest", "Verify and deduplicate one signed webhook without persisting its raw body, then deliver a zero-authority projected event.", ["subscription_id", "event_id", "timestamp", "signature", "raw_body"], false, ["now"]),
    tool("craft_speculative_policy_save", "Version a read-only proactive preparation policy with a hard budget, TTL, and candidate cap.", ["policy_id", "task_id", "name", "event_key", "operation", "budget_id", "estimated_resources"], false, ["ttl_seconds", "max_candidates", "status", "expected_version"]),
    tool("craft_speculative_enqueue", "Create an input-pinned, budget-reserved, disposable candidate from one delivered signed event.", ["policy_id", "policy_version", "trigger_event_id"], false, ["now"]),
    tool("craft_speculative_claim", "Lease one compatible read-only speculative job; the dispatch has no external-write authority.", ["worker_id", "operations"], false, ["lease_ttl_seconds", "now"]),
    tool("craft_speculative_submit", "Submit an evidence-backed candidate or failure and settle its reserved resources.", ["candidate_id", "lease_id", "worker_id", "verdict", "summary"], false, ["artifact_ids", "evidence_ids", "actual_resources", "now"]),
    tool("craft_speculative_decide", "Accept or reject a ready candidate and capture human edits as an implicit preference signal.", ["candidate_id", "decision", "reviewer"], false, ["final_artifact_ids", "correction", "now"]),
    tool("craft_speculative_expire", "Expire stale disposable candidates in a bounded sweep.", [], false, ["now", "limit"]),
    tool("craft_lineage_record", "Record one immutable evidence-backed derivation between exact object, artifact, trial, memory, capability, or effect versions.", ["workspace_id", "output", "sources", "actor_type", "evidence_ids", "summary"], false, ["lineage_id", "task_id", "transform"]),
    tool("craft_lineage_trace", "Traverse bounded upstream sources or downstream consumers from one exact entity version.", ["workspace_id", "entity"], true, ["direction", "max_depth"]),
    ...
    tool("craft_dehydration_capture", "Freeze the minimal versioned task, workspace, wait, runtime, budget, and recovery references needed for a long-task handoff.", ["task_id"], false, ["snapshot_id", "workspace_id", "wait_id", "runtime_run_id", "budget_ids", "recovery_item_ids", "ttl_seconds", "now"]),
    ...
    tool("craft_hydration_claim", "Lease one revalidated snapshot to a Host without exposing raw conversation context or credentials.", ["snapshot_id", "state_fingerprint", "host_id", "claim_key"], false, ["lease_ttl_seconds", "now"]),
    tool("craft_hydration_report", "Complete or abandon an exact hydration lease; completion requires Evidence.", ["snapshot_id", "lease_id", "host_id", "outcome", "summary"], false, ["evidence_ids", "now"]),
    tool("craft_hydration_lease_recover", "Return expired hydration leases to the frozen pool in a bounded sweep.", [], false, ["now", "limit"]),
    tool("craft_autonomy_policy_save", "Version an exact-task autonomy policy for automatic, notify-only, human approval, or multi-signature execution.", ["policy_id", "task_id", "name", "rules"], false, ["default_ttl_seconds", "status", "expected_version"]),
    tool("craft_autonomy_request", "Request a short-lived authorization bound to one policy version, task, action, target, and request digest.", ["policy_id", "policy_version", "task_id", "action", "target", "request_digest", "requested_by"], false, ["request_id", "ttl_seconds", "now"]),
    tool("craft_autonomy_decide", "Record one unique human approval or denial against a pending authorization request.", ["request_id", "actor", "decision", "approval_ref"], false, ["now"]),
    tool("craft_autonomy_consume", "Atomically consume an authorized request exactly once after matching its task, action, target, and digest.", ["request_id", "task_id", "action", "target", "request_digest", "idempotency_key"], false, ["notification_ref", "now"]),
    tool("craft_contract_observation_record", "Record a schema-only, evidence-backed API, MCP, or Computer Use observation without raw payloads.", ["task_id", "adapter_kind", "endpoint", "operation", "input_schema", "output_schema", "observed_effect", "outcome", "evidence_ids"], false, ["observation_id", "idempotency_observed", "compensation_observed", "credential_handles_required"]),
    ...
    tool("craft_contract_candidate_review", "Accept, correct, or reject an exact inferred contract without granting execution authority.", ["candidate_id", "decision", "reviewer", "review_ref"], false, ["corrected_contract"]),
    tool("craft_contract_candidate_verify", "Verify a reviewed contract against a verified Sandbox Profile and passed evidence-backed Trial; keep activation separate.", ["candidate_id", "sandbox_profile_id", "sandbox_profile_version", "trial_id"]),
    tool("craft_contract_diff", "Compare exact reviewed Contract versions and classify equivalent, compatible, or breaking changes.", ["baseline_id", "baseline_version", "candidate_id", "candidate_version"], true),
    tool("craft_contract_publish", "Publish one exact verified Contract as a versioned Capability Adapter after independent human approval.", ["candidate_id", "candidate_version", "asset_id", "name", "publisher", "approval_ref"], false, ["publication_id"]),
    tool("craft_contract_publication_rollback", "Retire only the capability version still owned by an active Contract publication.", ["publication_id", "reason", "actor", "rollback_ref"]),
    tool("craft_capability_canary_start", "Start a bounded sticky baseline/candidate rollout for one active Contract publication.", ["publication_id", "baseline_asset_id", "baseline_asset_version", "allocation_percent", "min_samples_per_arm", "thresholds"], false, ["canary_id"]),
    ...
    tool("craft_capability_canary_observe", "Record one evidence-backed quality, cost, latency, and correction sample.", ["canary_id", "sample_id", "arm", "outcome", "cost", "latency_ms", "evidence_ids"], false, ["corrected"]),
    ...
    tool("craft_capability_bundle_propose", "Propose an evidence-backed, secret-free bundle for personal, team, or organization sharing without copying raw trajectories.", ["asset_id", "asset_version", "audience", "manifest", "evidence_ids"], false, ["bundle_id"]),
    tool("craft_capability_bundle_review", "Approve, redact, or reject an exact Capability bundle without granting execution authority.", ["bundle_id", "decision", "reviewer", "review_ref"], false, ["reviewed_manifest"]),
    tool("craft_capability_bundle_publish", "Publish a reviewed bundle only after independent approval and pin its exact Capability Asset version.", ["bundle_id", "bundle_version", "publisher", "approval_ref"], false, ["release_id"]),
    tool("craft_capability_release_subscribe", "Pin a consumer to one exact active Capability release; activation remains governed by normal policy.", ["release_id", "release_version", "consumer"], false, ["subscription_id"]),
    ...
    tool("craft_capability_release_revoke", "Revoke a shared Capability release and fail closed for every pinned consumer.", ["release_id", "actor", "revocation_ref", "reason"]),
    tool("craft_hub_source_register", "Trust one HTTPS Capability Hub catalog source by an exact Ed25519 public key; no network request is performed.", ["endpoint", "publisher", "public_key_pem"], false, ["source_id"]),
    tool("craft_hub_catalog_ingest", "Verify and atomically ingest one signed, monotonic catalog page of at most 1000 metadata-only entries.", ["source_id", "revision", "previous_digest", "issued_at", "entries", "signature"], false, ["receipt_id"]),
    ...
    tool("craft_capability_materialize_stage", "Verify a selected Hub package digest and write bounded files into the local quarantine cache; never execute them.", ["source_id", "entry_id", "files"], false, ["materialization_id"]),
    tool("craft_capability_materialize_review", "Approve or reject a quarantined package after findings review; high-risk packages require a security approval reference.", ["materialization_id", "decision", "reviewer", "review_ref"], false, ["evidence_ids", "security_approval_ref"]),
    tool("craft_capability_materialize_activate", "Register an approved exact package only as a non-executable candidate Capability for later evaluation and promotion.", ["materialization_id", "asset_id", "asset_type", "effect"], false, ["name"]),
    tool("craft_capability_certification_assess", "Certify one exact materialized candidate only after held-out Evaluation, per-Trial Sandbox receipts, evidence-backed program Grades, and Signoff.", ["materialization_id", "materialization_version", "sandbox_profile_id", "sandbox_profile_version", "evaluation_run_id", "sandbox_receipts", "signoff_id", "certifier", "certification_ref"], false, ["certification_id"]),
    tool("craft_capability_certification_promote", "Atomically promote an unchanged certified candidate to verified after independent approval; this still grants no execution authority.", ["certification_id", "certification_version", "promoter", "approval_ref"]),
    tool("craft_supply_chain_advisory_record", "Record an evidence-backed Hub security advisory against one catalog entry or Capability asset.", ["source_id", "severity", "summary", "evidence_ids"], false, ["advisory_id", "entry_id", "asset_id"]),
    tool("craft_supply_chain_advisory_resolve", "Resolve an advisory with new Evidence without erasing its audit history.", ["advisory_id", "resolution", "resolver", "evidence_ids"]),
    tool("craft_supply_chain_reconcile", "Propagate source disablement, withdrawal, digest drift, and security advisories into blocked assets, invalid Activation Profiles, and recoverable recertification work.", ["source_id"]),
    tool("craft_untrusted_content_register", "Register external content by locator and digest without copying its raw body into Craft.", ["task_id", "source_type", "locator", "content_digest"], false, ["content_id", "media_type"]),
    tool("craft_untrusted_extraction_record", "Store data-only structured extraction from an execution-isolated parser and quarantine detected instructions.", ["content_id", "extractor", "schema_id", "structured_data"], false, ["extraction_id", "detected_instructions", "citations", "field_sources"]),
    tool("craft_decision_projection_release", "Release only reviewed top-level fields into decision context; projections never carry execution authority.", ["extraction_id", "reviewer_type", "reviewer_id", "verdict", "allowed_fields"], false, ["projection_id", "approval_ref"]),
    tool("craft_decision_projection_explain", "Explain field-level source, parser, review, and content-digest lineage without exposing unreleased fields.", ["projection_id"], true, ["field"]),
    tool("craft_untrusted_content_parse", "Verify and parse registered JSON or text in memory with deterministic selectors; raw content is never persisted.", ["content_id", "raw_content", "format", "schema_id", "selectors"], false, ["extraction_id"]),
    tool("craft_parser_security_evaluate", "Run a bounded ephemeral parser security suite and persist only digests, labels, metrics, and results.", ["evaluation_id", "suite_id", "suite_version", "cases"]),
    tool("craft_untrusted_content_parse_process", "Parse registered JSON or text in a memory-limited, environment-scrubbed child process with timeout and a durable receipt.", ["content_id", "raw_content", "format", "schema_id", "selectors"], false, ["extraction_id", "receipt_id", "timeout_ms"]),
    tool("craft_sandbox_profile_save", "Declare a versioned Sandbox Adapter contract without claiming it is verified.", ["name", "backend", "adapter_id", "capabilities"], false, ["profile_id"]),
    tool("craft_sandbox_profile_verify", "Verify one exact Sandbox Profile against observed capabilities and Evidence.", ["profile_id", "profile_version", "observed_capabilities", "evidence_ids", "verifier"], false, ["assessment_id"]),
    tool("craft_sandbox_plan", "Issue an exact-version execution ticket only when a verified Sandbox Profile satisfies every requirement.", ["task_id", "profile_id", "requirements", "request_digest"], false, ["profile_version", "ticket_id"]),
    tool("craft_sandbox_receipt", "Close a Sandbox ticket with exact adapter, profile, observed-boundary, and Evidence checks.", ["ticket_id", "receipt_id", "adapter_id", "profile_version", "status", "observed_capabilities", "evidence_ids"]),
    tool("craft_docker_sandbox_request_digest", "Compute the canonical request digest required before issuing a Docker Sandbox ticket.", ["image", "argv"], true),
    tool("craft_docker_sandbox_probe", "Diagnose Docker availability, image presence, and hardened startup without verifying the declared Profile.", ["profile_id", "profile_version", "image"]),
    tool("craft_docker_sandbox_conformance", "Run black-box Docker boundary checks; only a passing suite can verify a declared Profile.", ["profile_id", "profile_version", "image"], false, ["probe_id"]),
    tool("craft_docker_sandbox_execute", "Execute one exact Docker Sandbox ticket with no shell, denied network, bounded resources, and a verified receipt.", ["ticket_id", "receipt_id", "image", "argv"]),
