...
const schemaFor = (name) => {
    ...
    if (["inputs", "metadata", "policy", "dimensions", "environment", "budget", "data", "scores",
        "costs", "metrics", "configuration"].includes(name))
        return { type: "object" };
    if (["completed", "pending", "decisions", "artifacts", "steps", "cases", "capabilities",
        ...
        "trial_ids", "requirements", "grade_ids", "pattern_ids", "failure_modes"].includes(name))
        return { type: "array" };
    return { type: "string" };
};
...
export const TOOLS = [
    ...
    tool("craft_default_route_update", "Record one required safe-plan stage with real evidence; the final stage records the route Outcome.", ["route_id", "stage_id", "summary"], false, ["artifact_ids", "evidence_ids", "verdict"]),
