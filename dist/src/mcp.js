...
export const TOOLS = [
    ...
    tool("craft_default_route_find", "Find one uniquely matching active default route for a natural-language continuation; never guess on a tie.", ["query"], true, ["project_id"]),
    tool("craft_project_policy_save", "Save a versioned project policy that can require evidence-backed route receipts.", ["project_id", "name"], false, ["policy_id", "enforcement", "receipt_requirements"]),
    ...
    tool("craft_route_receipt_record", "Register a structured command receipt for the current safe route stage; secrets are rejected.", ["route_id", "stage_id", "kind", "status", "command", "summary"], false, ["receipt_id", "uri", "host_adapter_id"]),
    tool("craft_host_adapter_save", "Save a versioned Host Adapter contract; it grants only listed route operations.", ["name", "host", "allowed_operations"], false, ["host_adapter_id"]),
    ...
    tool("craft_workflow_trial_run", "Execute a Workflow and automatically capture its Trial, Trace, Artifact, Evidence, and Outcome.", ["task_id", "workflow_id", "project_root"], false, ["trial_id", "case_id", "version", "inputs",
    ...
    tool("craft_experience_mine", "Extract repeated evidence-backed success strategies and failure modes as proposal-only candidates; it never publishes changes.", ["subject_type", "subject_id", "subject_version"]),
    ...
    tool("craft_experience_shadow_experiment_evaluate", "Run a planned mining candidate only through read-only held-out Workflow evaluation and Promotion; it prepares, but never grants, Signoff or publication.", ["experiment_id", "suite_id", "project_root", "baseline_workflow_id", "baseline_workflow_version", "candidate_workflow_id", "candidate_workflow_version", "signoff_policy_id", "signoff_policy_version"], false, ["shadow_evaluation_id", "suite_version", "trials_per_case", "environment", "budget", "min_trials", "min_pass_rate_delta", "cost_metric", "max_cost_regression_ratio", "max_duration_regression_ratio"]),
    ...
    tool("craft_skill_proposal_transition", "Move a Skill candidate through the existing held-out Evaluation and Signoff Gate.", ["proposal_id", "target", "reason"], false, ["evaluation_run_id", "signoff_id"]),
    tool("craft_skill_proposal_rollback", "Restore a previously verified Skill candidate version as the latest version.", ["proposal_id", "target_version", "reason"]),
    tool("craft_skill_proposal_publish", "Write only a verified Skill candidate to an existing SKILL.md with explicit authorization, digest protection, and backup.", ["proposal_id", "source_id", "target_path", "expected_digest", "allow_external_write"], false, ["publication_id"]),
    ...
    tool("craft_skill_publication_rollback", "Restore a published SKILL.md only when its digest still matches the published candidate.", ["publication_id", "expected_digest", "allow_external_write"]),
    ...
    tool("craft_signoff_policy_save", "Save a versioned policy for evaluation and grader requirements.", ["name"], false, ["policy_id", "description", "requirements", "require_held_out", "require_outcome_passed"]),
    ...
    tool("craft_signoff_evaluate", "Evaluate an immutable signoff decision from an Evaluation Run and explicit Grades.", ["policy_id", "evaluation_run_id"], false, ["signoff_id", "policy_version", "grade_ids"]),
