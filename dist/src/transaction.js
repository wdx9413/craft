...
export class TransactionCoordinator {
    ...
    begin(args) {
        if (String(args.effect ?? "local_write") !== "local_write")
            throw new Error("workspace transactions support local_write only");
        ...
        const checkpoint = this.workspace.checkpoint({ workspace_id: workspaceId, checkpoint_id: `${transactionId}_baseline`,
            label: `transaction baseline: ${text(args.label, "label")}` }).checkpoint;
    ...
    commit(args) {
        ...
        if (transaction.status === "committed") {
            if (transaction.committed_checkpoint_id === checkpointId)
                return { transaction };
            throw new Error("workspace transaction is already committed with another checkpoint");
        }
        if (transaction.status !== "prepared")
            throw new Error(`workspace transaction cannot commit from ${transaction.status}`);
        const checkpoint = this.store.get("workspace_checkpoint", checkpointId);
        if (checkpoint.workspace_id !== transaction.workspace_id)
            throw new Error("workspace checkpoint does not belong to transaction workspace");
    ...
    rollback(args) {
        if (args.approved !== true)
            throw new Error("workspace transaction rollback requires approved=true");
        const transaction = this.store.get("workspace_transaction", id(args.transaction_id, "transaction_id", "workspace_transaction"));
        if (!new Set(["prepared", "committed"]).has(String(transaction.status)))
            throw new Error(`workspace transaction cannot roll back from ${transaction.status}`);
