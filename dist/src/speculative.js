...
function resources(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("estimated_resources must be an object");
...
export class SpeculativeKernel {
    ...
    policySave(args) {
        ...
        if (!OPERATIONS.has(operation))
            throw new Error("Speculative operation is not read-only or supported");
        const budget = this.store.get("budget_account", text(args.budget_id, "budget_id"));
        if (budget.status !== "active")
            throw new Error("Speculative budget must be active");
        ...
        if (!new Set(["active", "paused"]).has(status))
            throw new Error("Speculative policy status is unsupported");
        ...
        if (!existing)
            return { policy: this.store.create("speculative_policy", policyId, next) };
        if (Number(args.expected_version) !== Number(existing.version))
            throw new Error("Speculative policy version conflict");
    ...
    enqueue(args) {
        ...
        if (policy.status !== "active")
            throw new Error("Speculative policy is not active");
        ...
        if (existing) {
            if (existing.input_fingerprint !== inputFingerprint)
                throw new Error("Speculative candidate idempotency conflict");
        ...
        if (active.length >= Number(policy.max_candidates))
            throw new Error("Speculative policy candidate limit reached");
        ...
        if (reservation.status !== "reserved" || reservation.budget_id !== policy.budget_id ||
            JSON.stringify(reservation.resources) !== JSON.stringify(policy.estimated_resources))
            throw new Error("Speculative reservation does not match policy");
    ...
    claim(args) {
        ...
        const allowed = strings(args.operations, "operations", 1);
        if (allowed.some((item) => !OPERATIONS.has(item)))
            throw new Error("Worker declares an unsupported speculative operation");
        const now = instant(args.now, "now");
        const candidate = this.store.list("speculative_candidate", 10_000, (item) => item.status === "queued" && Date.parse(String(item.expires_at)) > now && allowed.includes(String(item.operation)))[0];
        if (!candidate)
            return { candidate: null };
        const lease = `lease_${randomUUID().replaceAll("-", "")}`;
    ...
    submit(args) {
        ...
        if (candidate.status !== "leased" || candidate.lease_id !== args.lease_id || candidate.claimed_by !== args.worker_id)
            throw new Error("Speculative candidate lease does not match");
        const now = instant(args.now, "now");
        if (now > Date.parse(String(candidate.lease_expires_at)))
            throw new Error("Speculative candidate lease expired");
        const verdict = text(args.verdict, "verdict");
        if (!new Set(["ready", "failed"]).has(verdict))
            throw new Error("Speculative verdict is unsupported");
        const artifactIds = strings(args.artifact_ids ?? [], "artifact_ids");
        const evidenceIds = strings(args.evidence_ids ?? [], "evidence_ids");
        if (verdict === "ready" && (!artifactIds.length || !evidenceIds.length))
            throw new Error("A ready speculative candidate requires Artifact and Evidence");
    ...
    decide(args) {
        ...
        if (candidate.status !== "ready")
            throw new Error("Only a ready speculative candidate can be decided");
        const decision = text(args.decision, "decision");
        if (!new Set(["accepted", "rejected"]).has(decision))
            throw new Error("Speculative decision is unsupported");
        ...
        const correction = args.correction === undefined ? null : text(args.correction, "correction");
    ...
    expire(args) {
        ...
        const expired = [];
        for (const candidate of this.store.list("speculative_candidate", 10_000, (item) => new Set(["queued", "leased", "ready"]).has(String(item.status)) && Date.parse(String(item.expires_at)) <= now).slice(0, limit)) {
