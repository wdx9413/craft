...
function endpointFor(kind, endpoint, name) {
    ...
    if (kind === "mcp_http") {
        ...
        try {
            ...
            throw new Error("mcp_http endpoint must be a valid HTTPS URL");
        }
        if (url.protocol !== "https:")
            throw new Error("mcp_http endpoint must use HTTPS");
    }
    return result;
}
...
export class CapabilityConnectorKernel {
    ...
    register(args) {
        ...
        if (!CONNECTOR_KINDS.has(kind))
            throw new Error("Unsupported capability connector kind");
        const approved = args.approved === true;
        if (EXTERNAL_CONNECTORS.has(kind) && !approved)
            throw new Error("External connector requires explicit user approval");
    ...
    discover(args) {
        ...
        if (!entries.length || entries.length > 100)
            throw new Error("assets must contain between 1 and 100 entries");
        const seen = new Set();
    ...
    approve(args) {
        ...
        if (source.status !== "discovered")
            throw new Error("Connector asset is not awaiting approval");
    ...
    ticketIssue(args) {
        ...
        const connector = this.store.get("capability_connector", String(source.connector_id));
        this.assertActive(connector);
        if (source.status !== "approved" || source.capability_asset_id === undefined)
            throw new Error("Connector asset is not approved");
        const asset = this.store.get("capability_asset", String(source.capability_asset_id), Number(source.capability_asset_version));
        if (!profile.asset_ids.includes(String(asset.id)))
            throw new Error("Capability asset is not in the activation profile");
        if (Number(profile.asset_versions[String(asset.id)]) !== Number(asset.version))
            throw new Error("Activation Profile pins a different asset version");
        if (asset.trust !== "trusted" && asset.trust !== "verified")
            throw new Error("Capability asset is not trusted for execution");
        if (asset.health !== "healthy" || asset.requires_credential || !(profile.allowed_effects.includes(String(asset.effect)))) {
            throw new Error("Capability asset is not eligible for this Activation Profile");
        }
        if (connector.kind === "serena_mcp" && asset.effect !== "read_only")
            throw new Error("Serena connector only permits read-only calls");
        const expiresAt = expiry(args.expires_at);
        const operation = assertSafe(text(args.operation, "operation"), "operation");
    ...
    ticketConsume(args) {
        ...
        if (ticket.profile_id !== profileId)
            throw new Error("Connector ticket profile does not match");
        if (ticket.status !== "issued")
            throw new Error("Connector ticket was already consumed");
        if (Date.parse(String(ticket.expires_at)) < Date.now())
            throw new Error("Connector ticket has expired");
        const connector = this.store.get("capability_connector", String(ticket.connector_id));
        this.assertActive(connector);
        const source = this.store.get("capability_connector_asset", String(ticket.connector_asset_id));
        if (source.status !== "approved" || Number(source.version) !== Number(ticket.connector_asset_version))
            throw new Error("Connector asset changed after ticket issue");
        const call = this.store.get("capability_call", String(ticket.call_id));
        if (call.status !== "issued" || call.profile_id !== profileId)
            throw new Error("Underlying capability call is unavailable");
    ...
    connector(connectorId) { return this.store.get("capability_connector", text(connectorId, "connector_id")); }
    assertActive(connector) {
        if (connector.status !== "active" || !["trusted", "verified"].includes(String(connector.trust)))
            throw new Error("Capability connector is not active and trusted");
    }
    discoverOne(connector, entry, seen) {
        const name = text(entry.name, "asset name");
        const assetType = text(entry.asset_type, "asset_type");
        const effect = text(entry.effect, "effect");
        if (!ASSET_TYPES.has(assetType) || !EFFECTS.has(effect))
            throw new Error("Connector asset type or effect is unsupported");
        if (connector.kind === "serena_mcp" && effect !== "read_only")
            throw new Error("Serena connector only permits read-only assets");
        const logicalId = text(entry.logical_id, "logical_id");
        if (seen.has(logicalId))
            throw new Error("Connector discovery logical_id must be unique");
        seen.add(logicalId);
...
//# sourceMappingURL=capability-connector.js.map