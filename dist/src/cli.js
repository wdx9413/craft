...
async function runStandalone(args, paths) {
    ...
    try {
        ...
        service.verifiedWorkAuthorize({ work_id: verified.work.id, authorization_ref: "local-read" });
        ...
        service.verifiedWorkAction({ work_id: verified.work.id, action_contract: { operation: "model_loop", dispatch_id: dispatch.id }, idempotency_key: "host-receipt", input_digest: digest(prompt), result_digest: digest(receipt) });
        service.verifiedWorkReobserve({ work_id: verified.work.id, observed_digest: digest(paths.root), expected_digest: digest(paths.root) });
        const gate = service.acceptanceGatePrepare({ gate_id: `standalone-gate-${dispatch.id}`, task_id: task.id, work_id: verified.work.id, acceptance_ref: `acceptance:${task.id}`, required_artifact_ids: [artifact.id], required_evidence_ids: [evidence.id] });
        ...
        return { version: VERSION, project_id: effectiveProjectId, session: completedSession.session, task, ...executed, acceptance: gate, outcome: null };
    }
    finally {
        store.close();
    }
