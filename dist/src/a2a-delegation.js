...
function confirmedEvidence(store, ids) {
    for (const id of ids)
        if (store.get("evidence", id).confidence !== "confirmed")
            throw new Error("A2A trust requires confirmed Evidence");
}
/**
...
export class A2ADelegationKernel {
    ...
    trustApprove(args) {
        ...
        if (provider.lifecycle !== "verified")
            throw new Error("A2A trust requires a verified enterprise identity provider");
        const evidenceIds = strings(args.evidence_ids, "evidence_ids", 1);
        confirmedEvidence(this.store, evidenceIds);
        const allowedEffects = strings(args.allowed_effects, "allowed_effects", 1);
        if (allowedEffects.some((effect) => effect !== "read_only"))
            throw new Error("A2A trust is read-only until a separately certified remote effect adapter exists");
        ...
        const trustId = text(args.trust_id, "trust_id");
        ...
        if (existing) {
            if (existing.definition_digest !== definitionDigest)
                throw new Error("A2A Agent trust idempotency conflict");
    ...
    sessionCreate(args) {
        ...
        const trust = this.store.get("a2a_agent_trust", text(args.trust_id, "trust_id"));
        if (trust.status !== "active")
            throw new Error("A2A Agent trust is not active");
        ...
        if (evaluation.status !== "eligible_for_signoff")
            throw new Error("A2A collaboration requires an eligible single-Agent evaluation baseline");
        const evidenceId = text(args.justification_evidence_id, "justification_evidence_id");
        confirmedEvidence(this.store, [evidenceId]);
        const budget = object(args.budget, "budget");
        const sessionIdentity = { task_id: task.id, trust_id: trust.id, trust_version: trust.version,
        ...
        const sessionDigest = digest(sessionIdentity);
        if (existing) {
            if (existing.session_digest !== sessionDigest)
                throw new Error("A2A collaboration session idempotency conflict");
    ...
    delegationPrepare(args) {
        const session = this.store.get("a2a_collaboration_session", text(args.session_id, "session_id"));
        if (session.lifecycle !== "active")
            throw new Error("A2A collaboration session is not active");
        const artifactIds = strings(args.artifact_ids ?? [], "artifact_ids", 0);
        const evidenceIds = strings(args.evidence_ids, "evidence_ids", 1);
        ...
        const delegationId = text(args.delegation_id, "delegation_id");
        const existing = this.store.find("a2a_delegation", delegationId);
        const delegationDigest = digest(intent);
        if (existing) {
            if (existing.delegation_digest !== delegationDigest)
                throw new Error("A2A delegation idempotency conflict");
            return { delegation: existing, idempotent: true };
        }
        if (Number(session.delegated_count) >= Number(session.max_delegations))
            throw new Error("A2A collaboration session reached its delegation limit");
    ...
    dispatch(args) {
        ...
        if (delegation.status === "issued" || delegation.status === "completed")
            return { delegation, envelope: this.envelope(delegation), idempotent: true };
        if (delegation.status !== "prepared" || now >= Date.parse(String(delegation.expires_at)))
            throw new Error("A2A delegation is expired or not dispatchable");
        const receipt = this.store.get("evidence", text(args.transport_evidence_id, "transport_evidence_id"));
        if (receipt.confidence !== "confirmed")
            throw new Error("A2A transport dispatch requires confirmed Evidence");
    ...
    report(args) {
        ...
        if (!new Set(["passed", "failed", "cancelled"]).has(verdict))
            throw new Error("A2A delegation verdict is unsupported");
        if (delegation.status === "completed") {
            const receipt = this.store.get("a2a_delegation_receipt", String(delegation.result_receipt_id));
            if (args.receipt_id !== undefined && text(args.receipt_id, "receipt_id") !== receipt.id)
                throw new Error("A2A completed delegation receipt does not match");
            return { delegation, receipt, idempotent: true };
        }
        if (delegation.status !== "issued")
            throw new Error("A2A delegation has not been issued");
        const evidenceIds = strings(args.evidence_ids, "evidence_ids", 1);
        confirmedEvidence(this.store, evidenceIds);
        const result = object(args.result, "result");
        if (Object.keys(result).some((key) => /(?:secret|token|cookie|password|authorization)/iu.test(key)))
            throw new Error("A2A result must not contain credential fields");
...
//# sourceMappingURL=a2a-delegation.js.map