...
function bodyObject(body) { if (!body)
    return {}; if (Buffer.byteLength(body) > MAX_BODY)
    throw new Error("Request body exceeds 64 KiB"); const value = JSON.parse(body); if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("Request body must be a JSON object"); return value; }
export class WorkbenchWebApp {
    service;
    token;
    origin;
    constructor(service, token, origin) { this.service = service; this.token = token; this.origin = origin; }
    handle(request) {
        ...
        if (path.startsWith("/api/") && !authorized(request.token, this.token))
            return json(401, { error: "Workbench token required" });
        try {
            if (request.method === "GET" && path === "/api/home")
                return json(200, this.service.homeView({}));
            if (request.method === "GET" && path === "/api/host-runs")
                return json(200, this.service.homeHostRuns({}));
            if (request.method === "GET" && path === "/api/domain-kits")
                return json(200, this.service.domainKitInstallBuiltins());
            if (request.method === "POST" && path.startsWith("/api/domain-kits/") && path.endsWith("/apply"))
                return json(201, this.service.domainKitApply({ ...bodyObject(request.body), kit_id: decodeURIComponent(path.slice(17, -6)) }));
            if (request.method === "GET" && path === "/api/knowledge")
                return json(200, this.service.knowledgeWorkbenchView({ limit: url.searchParams.get("limit") ?? undefined }));
            if (request.method === "GET" && path.startsWith("/api/knowledge/pages/"))
                return json(200, this.service.wikiPageGet({ page_id: decodeURIComponent(path.slice(21)), version: url.searchParams.get("version") ?? undefined }));
            if (request.method === "POST" && path.startsWith("/api/knowledge/pages/") && path.endsWith("/refresh"))
                return json(200, this.service.wikiPageRefresh({ page_id: decodeURIComponent(path.slice(21, -8)) }));
            if (request.method === "POST" && path.startsWith("/api/knowledge/claims/") && path.endsWith("/review"))
                return json(200, this.service.knowledgeClaimReview({ ...bodyObject(request.body), claim_id: decodeURIComponent(path.slice(22, -7)) }));
            if (request.method === "GET" && path.startsWith("/api/knowledge/bundles/") && path.endsWith("/preview"))
                return json(200, this.service.knowledgeContextBundlePreview({ bundle_id: decodeURIComponent(path.slice(23, -8)), bundle_version: url.searchParams.get("version") ?? undefined }));
            if (request.method === "POST" && path === "/api/knowledge-work-launches")
                return json(201, this.service.knowledgeWorkbenchWorkLaunchPrepare(bodyObject(request.body)));
            if (request.method === "GET" && path.startsWith("/api/knowledge-work-launches/"))
                return json(200, this.service.workLaunchGet({ launch_id: decodeURIComponent(path.slice(29)), after_sequence: url.searchParams.get("after") ?? undefined }));
            if (request.method === "POST" && path.startsWith("/api/knowledge-work-launches/") && path.endsWith("/decide"))
                return json(200, this.service.knowledgeWorkbenchWorkLaunchDecide({ ...bodyObject(request.body), launch_id: decodeURIComponent(path.slice(29, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/knowledge-work-launches/") && path.endsWith("/retry"))
                return json(201, this.service.knowledgeWorkbenchWorkLaunchRetry({ ...bodyObject(request.body), launch_id: decodeURIComponent(path.slice(29, -6)) }));
            if (request.method === "POST" && path === "/api/work-launches")
                return json(201, this.service.workLaunchPrepare(bodyObject(request.body)));
            if (request.method === "GET" && path.startsWith("/api/work-launches/"))
                return json(200, this.service.workLaunchGet({ launch_id: decodeURIComponent(path.slice(19)), after_sequence: url.searchParams.get("after") ?? undefined }));
            if (request.method === "POST" && path.startsWith("/api/work-launches/") && path.endsWith("/decide"))
                return json(200, this.service.workLaunchDecide({ ...bodyObject(request.body), launch_id: decodeURIComponent(path.slice(19, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/work-launches/") && path.endsWith("/retry"))
                return json(201, this.service.workLaunchRetry({ ...bodyObject(request.body), launch_id: decodeURIComponent(path.slice(19, -6)) }));
            if (request.method === "GET" && path === "/api/guided-work")
                return json(200, this.service.guidedWorkList({ limit: url.searchParams.get("limit") ?? undefined }));
            if (request.method === "POST" && path === "/api/guided-work")
                return json(201, this.service.guidedWorkCreate(bodyObject(request.body)));
            if (request.method === "GET" && path.startsWith("/api/guided-work/"))
                return json(200, this.service.guidedWorkGet({ brief_id: decodeURIComponent(path.slice(17)), version: url.searchParams.get("version") ?? undefined }));
            if (request.method === "POST" && path.startsWith("/api/guided-work/") && path.endsWith("/decide"))
                return json(200, this.service.guidedWorkDecide({ ...bodyObject(request.body), brief_id: decodeURIComponent(path.slice(17, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/guided-work/") && path.endsWith("/launch"))
                return json(201, this.service.guidedWorkLaunchPrepare({ ...bodyObject(request.body), brief_id: decodeURIComponent(path.slice(17, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/acceptance-plans/") && path.endsWith("/human-review"))
                return json(201, this.service.acceptanceHumanReview({ ...bodyObject(request.body), plan_id: decodeURIComponent(path.slice(22, -13)) }));
            if (request.method === "POST" && path.startsWith("/api/acceptance-plans/") && path.endsWith("/file-evaluation"))
                return json(201, this.service.acceptanceFileEvaluationPrepare({ ...bodyObject(request.body), plan_id: decodeURIComponent(path.slice(22, -16)) }));
            if (request.method === "GET" && path.startsWith("/api/host-runs/"))
                return json(200, this.service.homeHostRun({ run_id: decodeURIComponent(path.slice(15)), after_sequence: url.searchParams.get("after") ?? undefined, limit: 50 }));
            if (request.method === "POST" && path.startsWith("/api/host-runs/") && path.endsWith("/cancel"))
                return json(200, this.service.hostRunCancel({ ...bodyObject(request.body), run_id: decodeURIComponent(path.slice(15, -7)) }));
            if (request.method === "GET" && path.startsWith("/api/tasks/"))
                return json(200, this.service.homeTask({ task_id: decodeURIComponent(path.slice(11)) }));
            if (request.method === "POST" && path === "/api/tasks")
                return json(201, this.service.taskOpen(bodyObject(request.body)));
            if (request.method === "POST" && path === "/api/task-controls")
                return json(201, this.service.taskControlSave(bodyObject(request.body)));
            if (request.method === "GET" && path.startsWith("/api/task-controls/"))
                return json(200, this.service.taskControlGet({ contract_id: decodeURIComponent(path.slice(19)) }));
            if (request.method === "POST" && path.startsWith("/api/task-controls/") && path.endsWith("/refresh"))
                return json(200, this.service.taskControlRefresh({ contract_id: decodeURIComponent(path.slice(19, -8)) }));
            if (request.method === "POST" && path.startsWith("/api/task-controls/") && path.endsWith("/bind-launch"))
                return json(200, this.service.taskControlBindLaunch({ ...bodyObject(request.body), contract_id: decodeURIComponent(path.slice(19, -12)) }));
            if (request.method === "POST" && path.startsWith("/api/task-controls/") && path.endsWith("/handoff"))
                return json(201, this.service.taskControlHandoff({ ...bodyObject(request.body), contract_id: decodeURIComponent(path.slice(19, -8)) }));
            if (request.method === "POST" && path === "/api/task-runs")
                return json(201, this.service.taskRunPrepare(bodyObject(request.body)));
            if (request.method === "GET" && path.startsWith("/api/task-runs/"))
                return json(200, this.service.taskRunGet({ task_run_id: decodeURIComponent(path.slice(15)) }));
            if (request.method === "POST" && path.startsWith("/api/task-runs/") && path.endsWith("/refresh"))
                return json(200, this.service.taskRunRefresh({ ...bodyObject(request.body), task_run_id: decodeURIComponent(path.slice(15, -8)) }));
            if (request.method === "POST" && path.startsWith("/api/task-runs/") && path.endsWith("/pause"))
                return json(200, this.service.taskRunPause({ ...bodyObject(request.body), task_run_id: decodeURIComponent(path.slice(15, -6)) }));
            if (request.method === "POST" && path.startsWith("/api/task-runs/") && path.endsWith("/resume"))
                return json(200, this.service.taskRunResume({ ...bodyObject(request.body), task_run_id: decodeURIComponent(path.slice(15, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/task-runs/") && path.endsWith("/cancel"))
                return json(200, this.service.taskRunCancel({ ...bodyObject(request.body), task_run_id: decodeURIComponent(path.slice(15, -7)) }));
            if (request.method === "POST" && path.startsWith("/api/task-runs/") && path.endsWith("/handoff"))
                return json(201, this.service.taskRunHandoff({ ...bodyObject(request.body), task_run_id: decodeURIComponent(path.slice(15, -8)) }));
            if (request.method === "POST" && path === "/api/inbox/refresh")
                return json(200, this.service.attentionRefresh(bodyObject(request.body)));
            if (request.method === "POST" && path === "/api/inbox/decide")
                return json(200, this.service.attentionDecide(bodyObject(request.body)));
            return json(404, { error: "Not found" });
        }
        catch (error) {
            return json(error instanceof SyntaxError ? 400 : error instanceof Error && error.message.includes("64 KiB") ? 413 : 422, { error: error instanceof Error ? error.message : String(error) });
        }
    }
}
export class LocalWorkbenchServer {
    ...
    constructor(service, token = randomBytes(32).toString("base64url"), options = {}) { this.service = service; this.token = token; this.acceptanceTick = options.acceptanceTick ?? (() => runBuiltinAcceptanceTicks(this.service)); }
    async start(port = 4173) {
        if (!Number.isInteger(port) || port < 0 || port > 65535)
            throw new Error("port must be an integer between 0 and 65535");
        if (this.#server)
            throw new Error("Workbench server is already running");
        const server = createServer((request, response) => {
            const chunks = [];
            let size = 0;
            request.on("data", (chunk) => { size += chunk.length; if (size <= MAX_BODY)
                chunks.push(chunk); });
            request.on("end", () => {
                const activePort = server.address().port;
                const origin = `http://127.0.0.1:${activePort}`;
                const auth = request.headers.authorization?.startsWith("Bearer ") ? request.headers.authorization.slice(7) : undefined;
                ...
                response.writeHead(result.status, { "content-type": result.contentType, "cache-control": "no-store", "content-security-policy": "default-src 'self'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'self'; frame-ancestors 'none'", "x-content-type-options": "nosniff" });
        ...
        this.#server = server;
        this.#acceptanceTimer = setInterval(() => { if (this.#acceptanceRunning)
            return; this.#acceptanceRunning = true; this.acceptanceTick().catch(() => undefined).finally(() => { this.#acceptanceRunning = false; }); }, 500);
        const activePort = server.address().port;
        return { url: `http://127.0.0.1:${activePort}/#token=${encodeURIComponent(this.token)}`, token: this.token };
    }
    async close() { const server = this.#server; if (!server)
        return; this.#server = null; if (this.#acceptanceTimer)
        clearInterval(this.#acceptanceTimer); this.#acceptanceTimer = null; await new Promise((resolve) => server.close(() => resolve())); }
}
