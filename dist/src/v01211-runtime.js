...
function positive(value, name, fallback, max) {
    const result = value === undefined ? fallback : Number(value);
    if (!Number.isInteger(result) || result < 1 || result > max)
        throw new Error(`${name} must be an integer between 1 and ${max}`);
    return result;
}
const MANIFEST_FIELDS = ["knowledge_refs", "capability_refs", "workflow_refs", "excluded_refs"];
...
export class ContextPlaneKernel {
    ...
    save(args) {
        ...
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Context Manifest idempotency conflict");
    ...
    get(args) { return { manifest: this.store.get("context_manifest", text(args.manifest_id, "manifest_id")) }; }
    audit(args) {
...
export class ReplayRunnerKernel {
    ...
    prepare(args) {
        ...
        if (!["completed", "failed", "cancelled", "blocked"].includes(String(trace.status)))
            throw new Error("Replay requires a terminal Trace");
        const events = this.store.list("trace_event", 10_000, (item) => item.trace_id === traceId && item.action_contract !== null)
            .sort((left, right) => Number(left.sequence) - Number(right.sequence));
        if (!events.length)
            throw new Error("Trace has no replayable action contracts");
        ...
        if (existing) {
            if (existing.replay_digest !== replayDigest)
                throw new Error("Replay idempotency conflict");
    ...
    async execute(args, executor) {
        ...
        if (replay.status !== "prepared")
            throw new Error("Replay is not prepared");
        if (args.approval_ref !== undefined && text(args.approval_ref, "approval_ref") !== replay.approval_ref)
            throw new Error("Replay approval does not match");
        const trace = this.store.get("trace", String(replay.trace_id));
        if (Number(trace.version) !== Number(replay.trace_version))
            throw new Error("Replay source Trace changed; revalidate first");
        const events = replay.event_ids.map((id) => this.store.get("trace_event", id));
        const results = [];
    ...
    get(args) { return { replay: this.store.get("replay_run", text(args.replay_id, "replay_id")) }; }
}
...
export class LocalRuntimeServiceKernel {
    ...
    configure(args) {
        ...
        if (existing)
            return { service: this.store.save("local_runtime_service", serviceId, { ...payload(existing), ...record }), idempotent: false };
    ...
    start(args = {}) {
        ...
        if (service.status === "running")
            return { service, idempotent: true };
    ...
    tick(args = {}) {
        ...
        const service = this.store.get("local_runtime_service", serviceId);
        if (service.status !== "running")
            return { service, processed: [], count: 0, skipped: true };
    ...
    get(args = {}) { return { service: this.store.get("local_runtime_service", String(args.service_id ?? "craft-local")) }; }
}
...
export class ProjectBundleKernel {
    ...
    export(args) {
        const projectId = text(args.project_id, "project_id");
        const max = positive(args.limit, "limit", 5000, 20_000);
        ...
        if (existing) {
            if (existing.bundle_digest !== digest(identity))
                throw new Error("Project Bundle idempotency conflict");
...
export class FeedbackLearningKernel {
    ...
    constructor(store) { this.store = store; }
    record(args) {
        const signalId = String(args.signal_id ?? `feedback_signal_${randomUUID().replaceAll("-", "")}`);
        const scope = String(args.scope ?? "project");
        if (!["task", "project", "global"].includes(scope))
            throw new Error("Unsupported feedback scope");
        ...
        if (existing) {
            if (existing.identity_digest !== digest(identity))
                throw new Error("Feedback signal idempotency conflict");
    ...
    resolve(args) {
        const signal = this.store.get("feedback_signal", text(args.signal_id, "signal_id"));
        const reusable = signal.scope === "global" || signal.scope === "project" && args.project_id !== undefined && signal.project_id === args.project_id;
...
export class DomainEvaluatorKernel {
    ...
    constructor(store) { this.store = store; }
    save(args) {
        const evaluatorId = String(args.evaluator_id ?? `domain_evaluator_${randomUUID().replaceAll("-", "")}`);
        const domain = text(args.domain, "domain");
        const name = text(args.name, "name");
        const criteria = object(args.rules ?? args.criteria, "rules");
        const record = { domain, name, criteria, description_digest: digest(args.description ?? ""), status: "active" };
        const existing = this.store.find("domain_evaluator", evaluatorId);
        if (existing)
            return { evaluator: this.store.save("domain_evaluator", evaluatorId, { ...payload(existing), ...record }), idempotent: false };
        return { evaluator: this.store.create("domain_evaluator", evaluatorId, record), idempotent: false };
    }
    evaluate(args) {
        const evaluator = this.store.get("domain_evaluator", text(args.evaluator_id, "evaluator_id"));
        const metrics = object(args.metrics, "metrics");
        const criteria = evaluator.criteria;
...
export class HandoffManifestKernel {
    ...
    get(args) { return { handoff: this.store.get("handoff_manifest", text(args.handoff_id, "handoff_id")) }; }
}
...
export class CostLedgerKernel {
    ...
    priceSave(args) {
        ...
        if (![input, output].every((value) => Number.isFinite(value) && value >= 0))
            throw new Error("prices must be non-negative finite numbers");
        const id = String(args.price_id ?? `price_${provider}_${model}`);
        const record = { provider, model, input_per_million: input, output_per_million: output, effective_at: args.effective_at === undefined ? new Date().toISOString() : text(args.effective_at, "effective_at") };
        const existing = this.store.find("provider_price", id);
        return { price: existing ? this.store.save("provider_price", id, { ...payload(existing), ...record }) : this.store.create("provider_price", id, record), idempotent: false };
    }
    usageRecord(args) {
        const provider = text(args.provider, "provider");
        const model = text(args.model, "model");
        const input = Number(args.input_tokens ?? 0);
        const output = Number(args.output_tokens ?? 0);
        if (![input, output].every((value) => Number.isInteger(value) && value >= 0))
            throw new Error("token counts must be non-negative integers");
        const price = this.store.list("provider_price", 1000, (item) => item.provider === provider && item.model === model)[0];
        if (!price)
            throw new Error("No provider price snapshot");
