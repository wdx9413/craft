...
export class HostRunKernel {
    ...
    start(args) {
        ...
        if (!driver)
            throw new Error("Host Driver is unavailable");
        const dispatchId = text(args.dispatch_id, "dispatch_id");
        const prompt = text(args.prompt, "prompt");
        const runId = String(args.run_id ?? `host_run_${randomUUID().replaceAll("-", "")}`);
        const existing = this.store.find("host_run", runId);
        if (existing) {
            if (existing.host !== host || existing.dispatch_id !== dispatchId)
    ...
    get(args) { const run = this.store.get("host_run", text(args.run_id, "run_id")); return { run, events: this.store.events(`host-run:${run.id}`), active: this.controllers.has(String(run.id)) }; }
    cancel(args) { const run = this.store.get("host_run", text(args.run_id, "run_id")); if (TERMINAL.has(String(run.status)))
        return { run, idempotent: true }; const controller = this.controllers.get(String(run.id)); if (!controller)
        throw new Error("Host run is not owned by this process; recover it only after verifying the original runner stopped"); const saved = this.store.save("host_run", String(run.id), { ...payload(run), cancel_requested: true, cancel_reason: text(args.reason, "reason") }); controller.abort(); return { run: saved, idempotent: false }; }
    recover(args) { if (args.confirmed_original_runner_stopped !== true)
        throw new Error("Recovery requires confirmed_original_runner_stopped=true"); const ownerId = text(args.owner_id, "owner_id"); const recovered = []; for (const run of this.store.list("host_run", Number.MAX_SAFE_INTEGER, (item) => new Set(["running", "cancel_requested"]).has(String(item.status)) && item.owner_id === ownerId))
        if (!this.controllers.has(String(run.id))) {
            const saved = this.store.save("host_run", String(run.id), { ...payload(run), status: "interrupted", finished_at: new Date().toISOString() });
            recovered.push(saved);
            this.notifyTerminal(saved, null);
        } return { recovered, count: recovered.length }; }
    async wait(runId) { const completion = this.completions.get(runId); await completion; if (completion)
        this.completions.delete(runId); }
}
//# sourceMappingURL=host-run.js.map