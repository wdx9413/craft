...
import { BUILTIN_HOST_PROFILES } from "./host-registry.js";
...
function isExpired(value) { return Number.isNaN(Date.parse(String(value))) || Date.parse(String(value)) < Date.now(); }
/**
...
export class HostActivationManifestKernel {
    store;
    hosts;
    constructor(store, hostProfiles = BUILTIN_HOST_PROFILES) {
        this.store = store;
    ...
    prepare(args) {
        ...
        if (args.profile_version !== undefined && Number(args.profile_version) !== Number(profile.version))
            throw new Error("Activation Profile version does not match current state");
        if (profile.task_id !== task.id)
            throw new Error("Activation Profile does not match task");
        const host = text(args.host, "host");
        if (!this.hosts.has(host))
            throw new Error("Host Activation Manifest host is unsupported");
        const requested = args.asset_ids === undefined ? [...profile.asset_ids] : values(args.asset_ids, "asset_ids");
        if (requested.some((assetId) => !profile.asset_ids.includes(assetId)))
            throw new Error("Host Activation Manifest asset is not in the Activation Profile");
        const tickets = args.connector_ticket_ids === undefined ? [] : values(args.connector_ticket_ids, "connector_ticket_ids");
        const ticketByAsset = new Map();
        for (const ticketId of tickets) {
            ...
            if (ticketByAsset.has(String(ticket.asset_id)))
                throw new Error("Connector tickets must cover each asset at most once");
        ...
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Host Activation Manifest idempotency conflict");
    ...
    validate(args) {
        ...
        if (manifest.status !== "issued")
            throw new Error("Host Activation Manifest is not issued");
        const profile = this.store.get("activation_profile", String(manifest.profile_id));
        if (Number(profile.version) !== Number(manifest.profile_version))
            throw new Error("Activation Profile changed after manifest issue");
        const assets = manifest.assets.map((reference) => this.asset(profile, String(reference.asset_id), new Map(reference.connector_ticket_id ? [[String(reference.asset_id), this.store.get("capability_connector_ticket", String(reference.connector_ticket_id))]] : [])));
        const expected = { task_id: manifest.task_id, profile_id: profile.id, profile_version: profile.version, host: manifest.host, assets };
        if (digest(expected) !== manifest.identity_digest)
            throw new Error("Host Activation Manifest facts drifted");
        return { manifest, valid: true };
    }
    consume(args) {
        const manifest = this.validate(args).manifest;
        const callId = text(args.call_id, "call_id");
        const host = text(args.host, "host");
        if (host !== manifest.host)
            throw new Error("Host Activation Manifest host does not match");
        ...
        if (existing) {
            if (existing.identity_digest !== digest(identity))
                throw new Error("Host Activation receipt idempotency conflict");
    ...
    asset(profile, assetId, tickets) {
        ...
        if (!EFFECTS.has(String(asset.effect)) || !profile.allowed_effects.includes(String(asset.effect)) ||
        ...
        if (asset.connector_id !== undefined) {
            ...
            throw new Error("Non-connector capability asset must not have a Connector ticket");
...
//# sourceMappingURL=host-activation-manifest.js.map