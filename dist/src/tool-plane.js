 ...
 */
export const SYSCALL_PASSTHROUGH = [
...
export function parseToolName(name) {
    ...
    if (cut === -1)
        return { resource: rest, operation: "info" };
    const operation = rest.slice(cut + 1);
    if (!OPERATIONS.includes(operation))
        return { resource: rest, operation: "info" };
    return { resource: rest.slice(0, cut), operation };
}
function classifyEffect(tool, operation, resource) {
    if (tool.annotations?.readOnlyHint === true)
        return "read_only";
    if (DESTRUCTIVE_OPERATIONS.has(operation))
        return "destructive";
    if (EXTERNAL_OPERATIONS.has(operation) || EXTERNAL_RESOURCES.test(resource))
        return "external_write";
    if (READ_OPERATIONS.has(operation))
        return "read_only";
    return "local_write";
}
function riskOf(effect) {
    if (effect === "destructive" || effect === "external_write")
        return "high";
    return effect === "read_only" ? "low" : "medium";
}
function auditOf(effect) {
    if (effect === "read_only")
        return "never";
    return effect === "local_write" ? "on_write" : "always";
}
function schemaParts(tool) {
...
export function buildRegistry(tools) {
    ...
    return tools.map((tool) => {
        ...
        if (clash)
            throw new Error(`Tool registry collision: ${tool.name} and ${clash} both map to ${key}`);
...
export function findEntry(entries, resource, operation) {
 ...
 */
export function resolveEntry(entries, resource, operation, fallback) {
    if (operation)
        return findEntry(entries, resource, operation);
...
export function catalogOf(entries) {
 ...
 */
export function measureSurface(tools) {
...
//# sourceMappingURL=tool-plane.js.map