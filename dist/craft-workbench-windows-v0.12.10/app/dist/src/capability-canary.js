...
function number(value, name, min = 0, max = Number.MAX_SAFE_INTEGER) { const result = Number(value); if (!Number.isFinite(result) || result < min || result > max)
    throw new Error(`${name} must be between ${min} and ${max}`); return result; }
function integer(value, name, min, max) { const result = number(value, name, min, max); if (!Number.isInteger(result))
    throw new Error(`${name} must be an integer`); return result; }
...
function digest(value) { return Number.parseInt(createHash("sha256").update(value).digest("hex").slice(0, 8), 16) % 10_000; }
export class CapabilityCanaryKernel {
    ...
    start(args) {
        ...
        if (publication.status !== "active")
            throw new Error("Canary requires an active Contract publication");
        ...
        if (baseline.effect !== candidate.effect || baseline.asset_type !== candidate.asset_type)
            throw new Error("Canary assets are not comparable");
        const thresholds = object(args.thresholds, "thresholds");
        for (const key of ["max_failure_rate_delta", "max_cost_ratio", "max_latency_ratio", "max_correction_rate_delta"])
            number(thresholds[key], `thresholds.${key}`, 0);
    ...
    route(args) {
        ...
        const key = text(args.routing_key, "routing_key");
    ...
    observe(args) {
        ...
        if (!new Set(["baseline", "candidate"]).has(arm))
            throw new Error("Canary arm is unsupported");
        const evidenceIds = args.evidence_ids;
        if (!Array.isArray(evidenceIds) || !evidenceIds.length)
            throw new Error("Canary observation requires Evidence");
        for (const id of evidenceIds)
            this.store.get("evidence", text(id, "evidence_id"));
        const outcome = text(args.outcome, "outcome");
        if (!new Set(["passed", "failed"]).has(outcome))
            throw new Error("Canary outcome is unsupported");
        const sampleId = text(args.sample_id, "sample_id");
        const fingerprint = createHash("sha256").update(JSON.stringify({ canary_id: canary.id, arm, outcome,
        ...
        if (existing) {
            if (existing.fingerprint !== fingerprint)
                throw new Error("Canary sample idempotency conflict");
            return { sample: existing, idempotent: true };
        }
        if (canary.status !== "running")
            throw new Error("Canary is not accepting samples");
    ...
    evaluate(args) {
        const canary = this.store.get("capability_canary", text(args.canary_id, "canary_id"));
        if (canary.status !== "running")
            return { canary, ready: true, idempotent: true };
        ...
        if (!ready)
            return { canary, ready: false, baseline, candidate };
        ...
        const saved = this.store.save("capability_canary", String(canary.id), { ...payload(canary),
...
//# sourceMappingURL=capability-canary.js.map