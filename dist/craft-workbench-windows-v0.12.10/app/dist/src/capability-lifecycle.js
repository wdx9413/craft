...
function state(value) {
    ...
    if (!(new Set(["draft", "installed", "active", "disabled", "retired"])).has(normalized))
        throw new Error("Unsupported capability lifecycle");
    return normalized;
}
export class CapabilityLifecycleKernel {
    ...
    register(args) {
        ...
        if (existing) {
            if (existing.manifest_digest !== manifestDigest)
                throw new Error("Capability identity already exists with different manifest");
    ...
    install(args) {
        ...
        if (current === "retired")
            throw new Error("Retired capability cannot be installed");
        if (current === "active" || current === "installed")
            return { capability, idempotent: true };
    ...
    activate(args) {
        ...
        const current = state(capability.lifecycle);
        if (current === "retired")
            throw new Error("Retired capability cannot be activated");
        if (current === "disabled")
            throw new Error("Disabled capability must be installed before activation");
        if (current === "draft")
            throw new Error("Capability must be installed before activation");
    ...
    disable(args) {
        const capability = this.store.get("capability_lifecycle", text(args.capability_id, "capability_id"));
        if (state(capability.lifecycle) === "retired")
            return { capability, idempotent: true };
    ...
    upgrade(args) {
        const capability = this.store.get("capability_lifecycle", text(args.capability_id, "capability_id"));
        if (state(capability.lifecycle) === "retired")
            throw new Error("Retired capability cannot be upgraded");
        const version = text(args.source_version, "source_version");
        const sourceDigest = text(args.source_digest, "source_digest");
        if (version === capability.source_version && sourceDigest === capability.source_digest)
            return { capability, idempotent: true };
    ...
    list() { return { capabilities: this.store.list("capability_lifecycle", 10_000) }; }
}
