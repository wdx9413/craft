...
function limits(value) {
    ...
    if (!Number.isInteger(maxSteps) || maxSteps < 1 || maxSteps > 1_000)
        throw new Error("max_steps must be an integer between 1 and 1000");
    if (!Number.isInteger(maxTokens) || maxTokens < 1 || maxTokens > 10_000_000)
        throw new Error("max_tokens must be an integer between 1 and 10000000");
...
function safeArgs(value) {
    ...
    if (/(?:["']?)(?:api[_-]?key|authorization|cookie|password|secret|token)(?:["']?)\s*[:=]/iu.test(serialized))
        throw new Error("action args must not contain secrets");
    return value;
}
export class AutonomousRuntimeKernel {
    ...
    prepare(args) {
        ...
        if (existing) {
            if (existing.request_digest !== requestDigest)
                throw new Error("Autonomous run idempotency conflict");
    ...
    current(runId) { return this.store.get("autonomous_run", runId); }
    checkpoint(args) {
        const runId = text(args.run_id, "run_id");
        const run = this.current(runId);
        if (new Set(["completed", "failed", "cancelled"]).has(String(run.status)))
            throw new Error("Cannot checkpoint a terminal autonomous run");
        const state = args.state;
        if (!state || typeof state !== "object" || Array.isArray(state))
            throw new Error("state must be an object");
        const serialized = JSON.stringify(state);
        if (serialized.length > 1_000_000)
            throw new Error("state exceeds the checkpoint limit");
        ...
        const checkpoint = this.store.create("autonomous_checkpoint", checkpointId, {
            run_id: runId, sequence, reason: String(args.reason ?? "turn"), state_digest: digest(state), state: state,
        });
    ...
    resume(args) {
        ...
        if (run.status === "completed" || run.status === "cancelled")
            return { run, checkpoint: run.checkpoint_id ? this.store.get("autonomous_checkpoint", String(run.checkpoint_id)) : null, idempotent: true };
    ...
    async run(args, model, executor) {
        const prepared = this.prepare(args);
        let run = prepared.run;
        if (run.status === "paused")
            run = this.resume({ run_id: run.id }).run;
        if (run.status === "ready")
            run = this.store.save("autonomous_run", String(run.id), { ...payload(run), status: "running" });
        ...
        while (run.status === "running") {
            ...
            if (!Number.isInteger(tokens) || tokens < 0)
                throw new Error("turn tokens must be a non-negative integer");
            const sequence = Number(run.steps) + 1;
            if (turn.kind === "final") {
            ...
            run = this.store.save("autonomous_run", String(run.id), { ...payload(savedCheckpoint.run), status: "running" });
        }
...
//# sourceMappingURL=autonomous-runtime.js.map