...
export function resolveInputs(definitions, supplied) {
    ...
    for (const [index, definition] of definitions.entries()) {
        ...
        if (!(name in result) && "default" in definition)
            result[name] = definition.default;
        if (definition.required && !(name in result))
            throw new Error(`Missing required workflow input: ${name}`);
    }
    return result;
}
export function substitute(value, inputs) {
    if (Array.isArray(value))
        return value.map((item) => substitute(item, inputs));
...
export function normalizeSteps(input) {
    ...
    return input.map((original, index) => {
        ...
        if (!SIDE_EFFECTS.has(effect))
            throw new Error(`Unsupported side effect for ${stepId}: ${effect}`);
        return { ...step, id: stepId, side_effect: effect };
    });
}
export function approvedEffects(allowExecution, effects = []) {
    const approved = new Set(["read_only"]);
    if (allowExecution)
        approved.add("local_write");
    for (const effect of effects) {
        if (!SIDE_EFFECTS.has(String(effect)))
            throw new Error(`Unsupported approved side effect: ${effect}`);
        approved.add(String(effect));
    }
    return approved;
}
export function runStep(step, root, runtimeEnv = {}, runner = spawnSync) {
    ...
    if (kind === "command") {
        ...
        if (!existsSync(cwd) || !statSync(cwd).isDirectory())
            throw new Error(`Workflow command cwd does not exist: ${cwd}`);
    ...
    if (kind === "assertion") {
        if (step.evaluator === "file_exists") {
            let actual = true;
            try {
                statSync(safePath(root, String(step.path ?? "")));
            }
            catch {
                actual = false;
            }
...
export function executeSteps(steps, root, approved, executor = runStep) {
    const results = [];
    for (const step of steps) {
        ...
        try {
            ...
            results.push(result);
            if (!result.passed && step.continue_on_failure !== true)
                break;
        }
        catch (error) {
