...
export class WorkCoordinatorKernel {
    store;
    managed;
    constructor(store, managed) { this.store = store; this.managed = managed; }
    prepare(args) {
        ...
        const coordinatorId = String(args.coordinator_id ?? `work_coordinator_${fabric.id}`);
        const existing = this.store.find("work_coordinator", coordinatorId);
        const identityDigest = digest(identity);
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Work Coordinator idempotency conflict");
            return { coordinator: existing, managed_run: managed, idempotent: true };
        }
        ...
        this.event(coordinator, "prepared", { fabric_id: fabric.id, managed_run_id: managed.id });
    ...
    attachHostRun(args) {
        ...
        const run = this.store.get("host_run", text(args.host_run_id, "host_run_id"));
        ...
        if (coordinator.lifecycle === "needs_replan")
            throw new Error("Work Coordinator requires replan before another Host Run");
        if (run.dispatch_id !== launch.dispatch_id || run.host !== launch.host)
            throw new Error("Host Run does not match Work Coordinator Fabric");
        if (coordinator.active_host_run_id && coordinator.active_host_run_id !== run.id)
            throw new Error("Work Coordinator already owns a Host Run");
        const saved = this.store.save("work_coordinator", String(coordinator.id), { ...payload(coordinator), lifecycle: "running", active_host_run_id: run.id, next_action: "wait_for_host_receipt" });
        this.event(saved, "host_attached", { host_run_id: run.id });
    ...
    observe(args) {
        ...
        const saved = this.store.save("work_coordinator", String(coordinator.id), { ...payload(coordinator), lifecycle, latest_observation_id: observation.id, next_action: nextAction, active_host_run_id: lifecycle === "observed" ? null : coordinator.active_host_run_id });
        this.event(saved, "observed", { observation_id: observation.id, lifecycle });
    ...
    handoff(args) {
        ...
        const saved = this.store.save("work_coordinator", String(coordinator.id), { ...payload(coordinator), lifecycle: "paused", next_action: "resume_from_handoff" });
        this.event(saved, "handoff", { handoff_id: handoff.handoff.id });
        return { coordinator: saved, ...handoff };
    }
    get(args) { const coordinator = this.store.get("work_coordinator", text(args.coordinator_id, "coordinator_id")); return { coordinator, managed_run: this.managed.get({ managed_run_id: coordinator.managed_run_id }), timeline: this.store.events(`work-coordinator:${coordinator.id}`) }; }
    event(coordinator, type, data) { this.store.appendEvent(`work-coordinator:${coordinator.id}`, `work_coordinator.${type}`, data); }
}
//# sourceMappingURL=work-coordinator.js.map