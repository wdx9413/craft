...
function strings(value, name, minimum = 0) {
    if (!Array.isArray(value) || value.length < minimum)
        throw new Error(`${name} must contain at least ${minimum} values`);
...
function confirmedEvidence(store, ids) {
    for (const id of ids)
        if (store.get("evidence", id).confidence !== "confirmed")
            throw new Error("Enterprise boundary verification requires confirmed Evidence");
}
/**
...
export class EnterpriseAccessKernel {
    ...
    providerRegister(args) {
        ...
        if (!PROVIDER_KINDS.has(kind))
            throw new Error("Enterprise identity provider kind is unsupported");
        const issuer = new URL(text(args.issuer, "issuer"));
        if (issuer.protocol !== "https:" || issuer.username || issuer.password)
            throw new Error("Enterprise issuer must be an HTTPS URL without credentials");
        ...
        const providerId = text(args.provider_id, "provider_id");
        ...
        if (existing) {
            if (existing.definition_digest !== definitionDigest)
                throw new Error("Enterprise identity provider idempotency conflict");
    ...
    providerVerify(args) {
        ...
        if (provider.lifecycle === "verified")
            return { provider, idempotent: true };
        if (provider.lifecycle !== "declared")
            throw new Error("Enterprise identity provider cannot be verified from its current lifecycle");
        const evidenceIds = strings(args.evidence_ids, "evidence_ids", 1);
        confirmedEvidence(this.store, evidenceIds);
        const claims = strings(args.observed_claims, "observed_claims", 4);
        const required = ["short_lived_credentials", "token_exchange", "audit_subject", "revocation"];
        if (required.some((claim) => !claims.includes(claim)))
            throw new Error("Enterprise identity provider verification is missing a required boundary claim");
    ...
    principalBind(args) {
        const provider = this.verifiedProvider(text(args.provider_id, "provider_id"));
        const task = this.store.get("task", text(args.task_id, "task_id"));
        const subjectDigest = text(args.subject_digest, "subject_digest");
        if (!/^sha256:[a-f0-9]{64}$/u.test(subjectDigest))
            throw new Error("subject_digest must be SHA-256; raw subject identifiers are not stored");
        const now = instant(args.now, "now");
        const ttl = integer(args.ttl_seconds, "ttl_seconds", Math.min(900, Number(provider.max_ttl_seconds)), 60, Number(provider.max_ttl_seconds));
        ...
        const principalId = text(args.principal_id, "principal_id");
        ...
        if (existing) {
            if (existing.identity_digest !== identityDigest)
                throw new Error("Enterprise principal idempotency conflict");
    ...
    adapterBind(args) {
        const provider = this.verifiedProvider(text(args.provider_id, "provider_id"));
        const publication = this.store.get("contract_publication", text(args.contract_publication_id, "contract_publication_id"));
        if (publication.status !== "active")
            throw new Error("Enterprise Adapter requires an active Contract Publication");
        ...
        if (asset.trust !== "verified" || asset.health !== "healthy")
            throw new Error("Enterprise Adapter requires a verified healthy Capability Asset");
        const effects = strings(args.allowed_effects, "allowed_effects", 1);
        if (effects.some((effect) => !EFFECTS.has(effect)))
            throw new Error("Enterprise Adapter effect is unsupported");
        if (!effects.includes(String(asset.effect)))
            throw new Error("Enterprise Adapter cannot widen its published Contract effect");
        ...
        if (existing) {
            if (existing.definition_digest !== definitionDigest)
                throw new Error("Enterprise Adapter binding idempotency conflict");
    ...
    ticketIssue(args) {
        const binding = this.store.get("enterprise_adapter_binding", text(args.binding_id, "binding_id"));
        if (binding.lifecycle !== "active")
            throw new Error("Enterprise Adapter binding is not active");
        const principal = this.store.get("enterprise_principal", text(args.principal_id, "principal_id"));
        const task = this.store.get("task", text(args.task_id, "task_id"));
        const now = instant(args.now, "now");
        if (principal.status !== "active" || principal.task_id !== task.id || now >= Date.parse(String(principal.expires_at)))
            throw new Error("Enterprise principal is inactive, expired, or belongs to another task");
        const lease = this.store.get("credential_lease", text(args.credential_lease_id, "credential_lease_id"));
        if (lease.status !== "active" || lease.task_id !== task.id || now >= Date.parse(String(lease.expires_at)))
            throw new Error("Enterprise Access requires an active same-task short-lived credential lease");
        const effect = text(args.effect, "effect");
        if (!binding.allowed_effects.includes(effect))
            throw new Error("Enterprise ticket effect is not allowed by its Adapter binding");
        const approvalRef = args.approval_ref === undefined ? null : text(args.approval_ref, "approval_ref");
        const consumptionId = args.autonomy_consumption_id === undefined ? null : text(args.autonomy_consumption_id, "autonomy_consumption_id");
        if (effect !== "read_only") {
            if (!approvalRef || !consumptionId)
                throw new Error("Enterprise write access requires approval and consumed autonomy authorization");
            const consumption = this.store.get("autonomy_consumption", consumptionId);
            if (consumption.task_id !== task.id || !["external_write", "destructive"].includes(String(consumption.action)))
                throw new Error("Enterprise autonomy consumption is not valid for this task or write effect");
        }
        ...
        const ticketDigest = digest(identity);
        if (existing) {
            if (existing.ticket_digest !== ticketDigest)
                throw new Error("Enterprise Access ticket idempotency conflict");
    ...
    ticketConsume(args) {
        const ticket = this.store.get("enterprise_access_ticket", text(args.ticket_id, "ticket_id"));
        const now = instant(args.now, "now");
        if (ticket.status === "consumed")
            return { ticket, idempotent: true };
        if (ticket.status !== "issued" || now >= Date.parse(String(ticket.expires_at)))
            throw new Error("Enterprise Access ticket is expired or no longer active");
        if (text(args.request_digest, "request_digest") !== ticket.request_digest)
            throw new Error("Enterprise Access ticket request digest mismatch");
    ...
    get(args) { return { ticket: this.store.get("enterprise_access_ticket", text(args.ticket_id, "ticket_id")) }; }
    verifiedProvider(id) {
        const provider = this.store.get("enterprise_identity_provider", id);
        if (provider.lifecycle !== "verified")
            throw new Error("Enterprise identity provider is not verified");
        return provider;
    }
}
//# sourceMappingURL=enterprise-access.js.map