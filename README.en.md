...
- **Trust Profile and web boundary (v0.12.25)**: `TrustProfile` turns scoped Trial/Outcome/Evidence history into expiring, revocable autonomy recommendations; a recommendation is never an authorization. File operations continue through the Action Gateway. Credential-free bounded GET/HEAD web observations are available through MCP, while browser navigation, clicks, forms, and submits remain Adapter Contracts that a host/plugin must execute and return a receipt for. `craft-mcp-http` adds a bounded `POST /mcp` entry point; stdio remains the default local transport. See [v0.12.25 Trust/Web Runtime](docs/technical/modules/v01225-trust-web-runtime.md).
