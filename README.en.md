...
> Current release: v0.12.31. Craft is a general work runtime for people and AI. The default `craft` plugin includes Knowledge, Memory, Capability discovery, Skill Quality, and Workflow Evolution behind one shared control plane; each remains available as an optional standalone component. In Codex, the current app is the embedded execution host; Craft does not start another Codex CLI by default.
...
- **Runtime Proof & Deployment (v0.12.30)**: optional OIDC/JWKS resource-server verification, tenant/principal/scope/TTL-bound remote task handles, real Host Session plus independent Outcome Observer acceptance campaigns, publisher digest attestations, and a grant-bound A2A v1 adapter. Deployment-specific TLS, OAuth login, IdP revocation, business cases, and platform isolation remain explicit adapters rather than default claims. See [Runtime Proof & Deployment](docs/technical/modules/runtime-proof-deployment.md).
