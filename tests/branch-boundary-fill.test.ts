...
import { OsSecurityKernel } from "../core/os-security.ts";
...
import { ActionGatewayKernel, AcceptanceGateKernel, ProviderRouterKernel } from "../core/runtime-completion.ts";
import { RuntimeAssuranceKernel } from "../core/runtime-assurance.ts";
...
test("service compatibility facades cover default, review and rollback branches", async () => {
  ...
  try {
    ...
    service.workbenchWorkflowSave({ workflow_id: "studio-wf", name: "Studio", steps: [{ type: "action" }] });
    service.workbenchWorkflowSave({ workflow_id: "studio-placeholder", name: "Placeholder", steps: [] });
