...
await import("./v01132.test.ts");

test("v0.11.31 keeps Wiki-derived capability candidates proposal-only", async () => {
  ...
  try {
    ...
    assert.equal((service.wikiSkillCandidateCreate({ title: "Generated", kind: "workflow", claim_ids: [a.id, b.id], instructions: "Do work.", applicability: "When applicable.", fallback_condition: "Ask." }).candidate as JsonObject).id.startsWith("wiki_skill_candidate_"), true);
