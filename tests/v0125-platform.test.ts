...
test("settings persist, validate, and support a relocatable data root", async () => {
  ...
  assert.equal(resetSettingsSync(paths, new Date("2026-09-14T00:00:00Z")).theme, "light");
  ...
  assert.equal((await loadSettings(malformed)).theme, "light");
  assert.equal(loadSettingsSync(malformed).theme, "light");
