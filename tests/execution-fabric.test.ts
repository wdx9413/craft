...
test("Execution Fabric joins the verified loop and Host activation without a second scheduler", async () => {
  ...
  try {
    ...
    assert.equal((noAsset.launch as JsonObject).status, "prepared"); assert.equal((noAsset.launch as JsonObject).run_id, undefined);
    const noAssetExecution = f.service.executionFabricExecute({ fabric_id: "no-asset", prompt: "inspect" }); assert.equal((f.service.executionFabricExecute({ fabric_id: "no-asset", prompt: "inspect" }).run as JsonObject).id, (noAssetExecution.run as JsonObject).id); await f.service.hostRuns.wait(String((noAssetExecution.run as JsonObject).id)); assert.equal(f.service.executionFabricExecute({ fabric_id: "no-asset", prompt: "inspect" }).completed, true);
    ...
    assert.equal((prepared.activation_profile as JsonObject).selection, "eligible_local_assets"); assert.equal(launch.status, "prepared");
    const execution = f.service.executionFabricExecute({ fabric_id: fabric.id, prompt: "inspect", call_id: "fabric-call" }); await f.service.hostRuns.wait(String((execution.run as JsonObject).id));
    ...
    assert.equal((f.service.executionFabricGet({ fabric_id: fabric.id }).advances as JsonObject[]).length, 3);
    ...
    const explicitExecution = f.service.executionFabricExecute({ fabric_id: (explicit.execution_fabric as JsonObject).id, prompt: "other" }); await f.service.hostRuns.wait(String((explicitExecution.run as JsonObject).id));
    ...
    const readStatus = f.service.executionFabricPrepare({ fabric_id: "read-status", manifest_id: "read-status-manifest", workspace_id: f.workspace.id, title: "Read status", goal: "read", host: "codex-cli", prompt: "read", sandbox: "read-only" }); f.store.save("work_launch", String((readStatus.launch as JsonObject).id), { ...(readStatus.launch as JsonObject), status: "awaiting_approval" }); assert.throws(() => f.service.executionFabricExecute({ fabric_id: "read-status", prompt: "read" }), /not prepared/);
    const writeStatus = f.service.executionFabricPrepare({ fabric_id: "write-status", manifest_id: "write-status-manifest", workspace_id: f.workspace.id, title: "Write status", goal: "write", host: "codex-cli", prompt: "write", sandbox: "workspace-write" }); f.store.save("work_launch", String((writeStatus.launch as JsonObject).id), { ...(writeStatus.launch as JsonObject), status: "prepared" }); assert.throws(() => f.service.executionFabricExecute({ fabric_id: "write-status", prompt: "write", approved: true, actor: "user" }), /not awaiting approval/);
    assert.throws(() => f.service.workLaunchDecide({ launch_id: (write.launch as JsonObject).id, actor: "user", approved: true, prompt: "write" }), /Fabric-managed/);
    assert.throws(() => f.service.executionFabricExecute({ fabric_id: "write", prompt: "write" }), /approved=true/);
    const writeExecution = f.service.executionFabricExecute({ fabric_id: "write", prompt: "write", approved: true, actor: "user" }); await f.service.hostRuns.wait(String((writeExecution.run as JsonObject).id));
    assert.equal((f.service.executionFabricGet({ fabric_id: "write" }).bridge_invocations as JsonObject[])[0].status, "completed");
...
test("Managed Host Bridge rejects mismatched facts and retains only exact invocation references", async () => {
  const f = await fixture();
  try {
    f.service.executionFabricWorkbenchPrepare({ fabric_id: "default-scope", manifest_id: "default-scope-manifest", workspace: f.root, title: "Default scope", goal: "inspect", host: "codex-cli", prompt: "inspect" }); assert.deepEqual(f.store.list("workspace", 10, (workspace) => workspace.root_path === f.root && workspace.id !== f.workspace.id)[0].include_paths, ["."]);
    assert.equal((f.service.executionFabricWorkbenchPrepare({ fabric_id: "explicit-scope", manifest_id: "explicit-scope-manifest", workspace_id: "explicit-workspace", workspace_name: "Explicit Workspace", workspace: f.root, include_paths: ["note.txt"], title: "Explicit scope", goal: "inspect", host: "codex-cli", prompt: "inspect" }).launch as JsonObject).status, "prepared");
    assert.equal((f.service.executionFabricWorkbenchPrepare({ fabric_id: "existing-scope", manifest_id: "existing-scope-manifest", workspace_id: "explicit-workspace", workspace: f.root, include_paths: ["note.txt"], title: "Existing scope", goal: "inspect", host: "codex-cli", prompt: "inspect" }).launch as JsonObject).status, "prepared");
    assert.throws(() => f.service.executionFabricWorkbenchPrepare({ fabric_id: "scope-conflict", manifest_id: "scope-conflict-manifest", workspace_id: "explicit-workspace", workspace: f.root, include_paths: ["."], title: "Scope conflict", goal: "inspect", host: "codex-cli", prompt: "inspect" }), /another root or observation scope/);
    assert.throws(() => f.service.executionFabricWorkbenchPrepare({ fabric_id: "root-conflict", manifest_id: "root-conflict-manifest", workspace_id: "explicit-workspace", workspace: join(f.root, "other-root"), include_paths: ["note.txt"], title: "Root conflict", goal: "inspect", host: "codex-cli", prompt: "inspect" }), /another root or observation scope/);
    const fallbackTask = f.service.taskOpen({ title: "Fallback", goal: "inspect" }).task as JsonObject; assert.equal((f.service.executionFabricWorkbenchPrepare({ fabric_id: "fallback-scope", manifest_id: "fallback-scope-manifest", workspace_id: "fallback-workspace", workspace: f.root, include_paths: ["note.txt"], task_id: fallbackTask.id, goal: "inspect", host: "codex-cli", prompt: "inspect" }).launch as JsonObject).status, "prepared");
    ...
    assert.throws(() => f.service.hostBridgeGet({ invocation_id: " " }), /must not be empty/);
    const preparedInvocation = (id: string) => f.service.hostBridge.prepare({ fabric_id: fabric.id, invocation_id: id }).invocation as JsonObject;
    const savedManifest = f.store.get("host_activation_manifest", String(manifest.id)); f.store.save("host_activation_manifest", String(manifest.id), { ...savedManifest, task_id: "other-task" }); assert.throws(() => preparedInvocation("wrong-manifest-task"), /requires one Fabric/); f.store.save("host_activation_manifest", String(manifest.id), savedManifest);
    const savedLaunch = f.store.get("work_launch", String(launch.id)); f.store.save("work_launch", String(launch.id), { ...savedLaunch, task_id: "other-task" }); assert.throws(() => preparedInvocation("wrong-launch-task"), /requires one Fabric/); f.store.save("work_launch", String(launch.id), { ...savedLaunch, host: "claude-code" }); assert.throws(() => preparedInvocation("wrong-launch-host"), /requires one Fabric/); f.store.save("work_launch", String(launch.id), { ...savedLaunch, status: "denied" }); assert.throws(() => preparedInvocation("denied-launch"), /not executable/); f.store.save("work_launch", String(launch.id), savedLaunch);
    const run = f.store.create("host_run", "bridge-run", { host: "codex-cli", dispatch_id: launch.dispatch_id, status: "running" }); const receipt = f.store.create("host_activation_receipt", "bridge-receipt", { manifest_id: manifest.id });
    const idempotencyRun = f.store.create("host_run", "idempotency-run", { host: "codex-cli", dispatch_id: launch.dispatch_id, status: "running" }); const idempotencyReceipt = f.store.create("host_activation_receipt", "idempotency-receipt", { manifest_id: manifest.id });
    assert.equal((f.service.hostBridge.start({ invocation_id: first.id, run_id: run.id, activation_receipt_id: receipt.id }).invocation as JsonObject).status, "running");
    assert.equal(f.service.hostBridge.start({ invocation_id: first.id, run_id: run.id, activation_receipt_id: receipt.id }).idempotent, true);
    assert.throws(() => f.service.hostBridge.start({ invocation_id: first.id, run_id: idempotencyRun.id, activation_receipt_id: receipt.id }), /idempotency conflict/);
    assert.throws(() => f.service.hostBridge.start({ invocation_id: first.id, run_id: run.id, activation_receipt_id: idempotencyReceipt.id }), /idempotency conflict/);
    assert.throws(() => f.service.hostBridge.finish({ invocation_id: first.id, run_id: run.id }), /terminal Host receipt/);
    const wrongHost = f.store.create("host_run", "wrong-host", { host: "claude-code", dispatch_id: launch.dispatch_id, status: "completed" }); const wrongDispatch = f.store.create("host_run", "wrong-dispatch", { host: "codex-cli", dispatch_id: "other", status: "completed" }); const wrongReceipt = f.store.create("host_activation_receipt", "wrong-receipt", { manifest_id: "other" });
    assert.throws(() => f.service.hostBridge.start({ invocation_id: preparedInvocation("wrong-host-invocation").id, run_id: wrongHost.id, activation_receipt_id: receipt.id }), /facts do not match/);
    assert.throws(() => f.service.hostBridge.start({ invocation_id: preparedInvocation("wrong-dispatch-invocation").id, run_id: wrongDispatch.id, activation_receipt_id: receipt.id }), /facts do not match/);
    assert.throws(() => f.service.hostBridge.start({ invocation_id: preparedInvocation("wrong-receipt-invocation").id, run_id: run.id, activation_receipt_id: wrongReceipt.id }), /facts do not match/);
    const terminalRun = f.store.save("host_run", String(run.id), { ...run, status: "completed", finished_at: new Date().toISOString() });
    assert.throws(() => f.service.hostBridge.finish({ invocation_id: first.id, run_id: wrongHost.id }), /does not belong/);
    assert.equal((f.service.hostBridge.finish({ invocation_id: first.id, run_id: terminalRun.id }).invocation as JsonObject).status, "completed");
    assert.equal(f.service.hostBridge.finish({ invocation_id: first.id, run_id: terminalRun.id }).idempotent, true);
    assert.throws(() => f.service.hostBridge.start({ invocation_id: first.id, run_id: terminalRun.id, activation_receipt_id: receipt.id }), /facts do not match/);
    const unstarted = preparedInvocation("unstarted"); f.store.save("host_bridge_invocation", String(unstarted.id), { ...unstarted, run_id: terminalRun.id }); assert.throws(() => f.service.hostBridge.finish({ invocation_id: unstarted.id, run_id: terminalRun.id }), /was not started/);
    const fallbackRun = f.store.create("host_run", "fallback-run", { host: "codex-cli", dispatch_id: launch.dispatch_id, status: "completed" }); const fallback = preparedInvocation("fallback"); f.store.save("host_bridge_invocation", String(fallback.id), { ...fallback, status: "running", run_id: fallbackRun.id, activation_receipt_id: receipt.id }); assert.ok((f.service.hostBridge.finish({ invocation_id: fallback.id, run_id: fallbackRun.id }).invocation as JsonObject).finished_at);
    const bridgeInternals = f.service as unknown as { finishFabricHostBridge(run: JsonObject): void; executionFabricAdvance(args: JsonObject): JsonObject };
    const originalAdvance = bridgeInternals.executionFabricAdvance; const projection = (id: string) => f.store.create("host_bridge_invocation", id, { fabric_id: fabric.id, manifest_id: manifest.id, run_id: terminalRun.id, activation_receipt_id: receipt.id, status: "running" });
    projection("projection-error"); bridgeInternals.executionFabricAdvance = () => { throw new Error("projection"); }; bridgeInternals.finishFabricHostBridge(terminalRun);
    projection("projection-unknown"); bridgeInternals.executionFabricAdvance = () => { throw "projection"; }; bridgeInternals.finishFabricHostBridge(terminalRun); bridgeInternals.executionFabricAdvance = originalAdvance;
    f.store.save("work_launch", String(launch.id), { ...f.store.get("work_launch", String(launch.id)), dispatch_id: "changed-dispatch" }); assert.throws(() => f.service.hostBridge.prepare({ fabric_id: fabric.id, invocation_id: first.id }), /idempotency conflict/); f.store.save("work_launch", String(launch.id), savedLaunch);
    const deferred = f.service.workLaunchPrepare({ launch_id: "deferred-idempotent", title: "Deferred", goal: "inspect", workspace: f.root, host: "codex-cli", sandbox: "read-only", prompt: "inspect", defer_host_start: true }); assert.equal((deferred.launch as JsonObject).status, "prepared"); assert.equal(f.service.workLaunchPrepare({ launch_id: "deferred-idempotent", title: "Deferred", goal: "inspect", workspace: f.root, host: "codex-cli", sandbox: "read-only", prompt: "inspect", defer_host_start: true }).idempotent, true); assert.throws(() => f.service.workLaunchPrepare({ launch_id: "deferred-idempotent", title: "Deferred", goal: "inspect", workspace: f.root, host: "codex-cli", sandbox: "read-only", prompt: "inspect" }), /idempotency conflict/);
    assert.equal(loop.task_id, (prepared.task as JsonObject).id);
  } finally { f.store.close(); await rm(f.root, { recursive: true, force: true }); }
});
