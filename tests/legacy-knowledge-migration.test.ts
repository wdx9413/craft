...
test("legacy discovery enforces the bounded source file count", async () => {
  ...
  // 10 001 concurrent writes exhaust the process' file handles; the file count is the point.
  for (let start = 0; start < 10_001; start += 500) {
    await Promise.all(Array.from({ length: Math.min(500, 10_001 - start) }, (_, offset) => writeFile(join(pages, `page-${start + offset}.md`), "plain")));
  }
  const store = await new CraftStore(craftPaths(root)).open();
