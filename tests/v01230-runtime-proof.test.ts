...
test("v0.12.32 binds remote task operations to one tenant, principal, receipt, scope and opaque one-time handle", async () => {
...
test("v0.12.32 verifies a real JWKS-signed OIDC resource token without retaining it", async () => {
...
test("v0.12.32 keeps publisher signatures separate from capability activation and makes drift fail closed", async () => {
  const f = await fixture();
  try {
...
test("v0.12.32 accepts only observed two-Host/two-Case five-trial evidence before default activation", async () => {
  const f = await fixture();
  try {
...
test("v0.12.32 adapts A2A v1 only through a consumed read-only delegation grant", async () => {
  const f = await fixture();
  try {
