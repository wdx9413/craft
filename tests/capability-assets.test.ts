...
test("the facade runs the internal host end to end with a permitted action", async () => {
  ...
  try {
    ...
    // Two steps: the action turn, and the completing turn, which is now charged to
    // the loop instead of being invisible to it.
    assert.equal((receipt.loop as JsonObject).steps, 2);
...
test("the internal host action whitelist is enforced by the facade", async () => {
  ...
  try {
    ...
    // Two independent fail-closed gates: a governed verb is refused by the tier
    // gate, and an unknown action is refused by dispatch. Both stay "not permitted".
    await assert.rejects(invoke("capability_delete", {}), /not permitted/);
    await assert.rejects(invoke("totally_unknown_action", {}), /not permitted/);
    await assert.rejects(invoke("contract_publish", {}), /not permitted.*governed/s);
  } finally { store.close(); await rm(root, { recursive: true, force: true }); }
});
