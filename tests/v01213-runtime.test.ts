...
test("v0.12.16 action gateway enforces workspace boundaries and approval", async () => {
...
test("v0.12.16 acceptance, worker and provider kernels form a durable local control path", async () => {
  const f = await fixture();
  try {
...
test("v0.12.16 A2A standard operations are HTTPS-only and content-free", async () => {
...
test("v0.12.16 service and MCP expose the new runtime surface", async () => {
  const f = await fixture();
  try {
