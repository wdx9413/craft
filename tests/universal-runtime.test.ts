...
test("v0.12.19 makes Knowledge Sources, Memory Ledger, receipts and retrieval selection explicit", async () => {
  ...
  try {
    ...
    assert.equal((f.sources.sourceList({ scope_kind: "project", scope_id: "project" }).sources as JsonObject[]).length, 1);
    assert.throws(() => f.sources.sourceList({ scope_kind: "project" }), /together/);
    assert.throws(() => f.sources.sourceRegister({ kind: "bad", label: "x", scope_kind: "project", scope_id: "project", locator: "x", content_digest: "x" }), /kind/);
    assert.throws(() => f.sources.sourceRegister({ kind: "readme", label: "x", scope_kind: "bad", scope_id: "project", locator: "x", content_digest: "x" }), /scope_kind/);
    ...
    assert.equal((f.ledger.compatBind({ binding_id: "legacy-bind", legacy_kind: "memory_item", legacy_id: "legacy", source_id: source.id }).binding as JsonObject).migration_performed, false);
    assert.equal((f.ledger.compatBind({ binding_id: "legacy-bind", legacy_kind: "memory_item", legacy_id: "legacy", source_id: source.id }) as JsonObject).idempotent, true);
    assert.throws(() => f.ledger.compatBind({ legacy_kind: "wrong", legacy_id: "legacy", source_id: source.id }), /unsupported/);

    const keyword = f.context.retrievalConfigure({ adapter_id: "keyword", strategy: "keyword" }).adapter as JsonObject;
    assert.equal((f.context.retrievalEvaluate({ adapter_id: keyword.id, metrics: { recall: 0, cross_project_leak_count: 0, latency_ms: 0, cost_usd: 0 } }).adapter as JsonObject).status, "eligible");
    assert.throws(() => f.context.retrievalConfigure({ strategy: "vector" }), /provider_fingerprint/);
    ...
    assert.equal((f.context.receiptGet({ receipt_id: "context" }).receipt as JsonObject).content_free, true);
    f.sources.sourceTransition({ source_id: source.id, status: "disabled", reason: "off" });
    assert.throws(() => f.ledger.remember({ source_id: source.id, kind: "working", scope_kind: "project", scope_id: "project", content: "blocked" }), /not active/);
    assert.equal((f.ledger.get({ memory_id: procedural.id }).memory as JsonObject).kind, "procedural");
...
test("v0.12.19 fails closed for untrusted, stale, restricted, malformed and drifting runtime facts", async () => {
  ...
  try {
    ...
    const untrusted = f.sources.sourceRegister({ source_id: "untrusted", kind: "custom", label: "untrusted", scope_kind: "project", scope_id: "project", locator: "custom", content_digest: "sha256:custom", trust: "untrusted", access: "proposal_only" }).source as JsonObject;
    assert.equal((f.sources.sourceList().sources as JsonObject[]).length, 2);
    assert.throws(() => f.sources.sourceRegister({ source_id: "trusted", kind: "readme", label: "changed", scope_kind: "project", scope_id: "project", locator: "README.md", content_digest: "sha256:readme", trust: "bounded", access: "read_only" }), /conflict/);
    assert.throws(() => f.sources.sourceRegister({ kind: "readme", label: "x", scope_kind: "project", scope_id: "project", locator: "x", content_digest: "x", trust: "bad" }), /trust/);
    assert.throws(() => f.sources.sourceRegister({ kind: "readme", label: "x", scope_kind: "project", scope_id: "project", locator: "x", content_digest: "x", access: "write" }), /access/);
    assert.equal((f.sources.sourceTransition({ source_id: untrusted.id, status: "revoked", reason: "withdrawn" }).source as JsonObject).status, "revoked");

    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "bad", scope_kind: "project", scope_id: "project", content: "x" }), /kind/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "x", sensitivity: "bad" }), /sensitivity/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "bad", scope_id: "project", content: "x" }), /scope_kind/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "x", confidence: "bad" }), /confidence/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "x", confidence: "confirmed" }), /requires Evidence/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "x", evidence_ids: ["missing"] }), /Unknown/);
    assert.throws(() => f.ledger.remember({ source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "x", valid_until: "never" }), /ISO/);
    const normal = f.ledger.remember({ memory_id: "normal", source_id: trusted.id, kind: "working", scope_kind: "project", scope_id: "project", content: "normal information" }).memory as JsonObject;
    const restricted = f.ledger.remember({ memory_id: "restricted", source_id: trusted.id, kind: "preference", scope_kind: "project", scope_id: "project", content: "restricted information", sensitivity: "restricted" }).memory as JsonObject;
    const otherScope = f.ledger.remember({ memory_id: "other", source_id: trusted.id, kind: "working", scope_kind: "task", scope_id: "task", content: "other" }).memory as JsonObject;
    assert.throws(() => f.ledger.transition({ memory_id: normal.id, status: "superseded", replacement_id: otherScope.id, reason: "wrong scope" }), /same scope/);
    assert.equal((f.ledger.transition({ memory_id: otherScope.id, status: "expired", reason: "finished" }).memory as JsonObject).status, "expired");
    assert.equal((f.ledger.transition({ memory_id: normal.id, status: "revoked", reason: "withdrawn" }).memory as JsonObject).status, "revoked");
    f.store.create("episodic_memory", "episode", { content: "old" }); f.store.create("semantic_memory", "semantic", { content: "old" });
    assert.ok((f.ledger.compatBind({ legacy_kind: "episodic_memory", legacy_id: "episode", source_id: trusted.id }).binding as JsonObject).id);
    assert.ok((f.ledger.compatBind({ legacy_kind: "semantic_memory", legacy_id: "semantic", source_id: trusted.id }).binding as JsonObject).id);

    assert.throws(() => f.context.retrievalConfigure({ strategy: "bad" }), /unsupported/);
    assert.throws(() => f.context.retrievalConfigure({ strategy: "keyword", configuration: [] }), /object/);
    assert.throws(() => f.context.retrievalConfigure({ strategy: "keyword", configuration: { token: "abcdefgh" } }), /credentials/);
    const keyword = f.context.retrievalConfigure({ adapter_id: "keyword-2", strategy: "keyword" }).adapter as JsonObject;
    assert.equal((f.context.retrievalConfigure({ adapter_id: "keyword-2", strategy: "keyword" }) as JsonObject).idempotent, true);
    assert.throws(() => f.context.retrievalConfigure({ adapter_id: "keyword-2", strategy: "vector", provider_fingerprint: "x" }), /conflict/);
    assert.throws(() => f.context.retrievalEvaluate({ adapter_id: keyword.id, metrics: {} }), /invalid/);
    assert.throws(() => f.context.retrievalEvaluate({ adapter_id: keyword.id, metrics: { recall: 0, cross_project_leak_count: 0, latency_ms: 0, cost_usd: 0 }, minimum_recall: 2 }), /thresholds/);
    assert.equal((f.context.retrievalEvaluate({ evaluation_id: "keyword-eval", adapter_id: keyword.id, metrics: { recall: 0, cross_project_leak_count: 0, latency_ms: 0, cost_usd: 0 } }).evaluation as JsonObject).verdict, "eligible");
    assert.equal((f.context.retrievalEvaluate({ evaluation_id: "keyword-eval", adapter_id: keyword.id, metrics: { recall: 0, cross_project_leak_count: 0, latency_ms: 0, cost_usd: 0 } }) as JsonObject).idempotent, true);
    assert.throws(() => f.context.retrievalEvaluate({ evaluation_id: "keyword-eval", adapter_id: keyword.id, metrics: { recall: 1, cross_project_leak_count: 0, latency_ms: 0, cost_usd: 0 } }), /conflict/);

    await assert.rejects(() => f.context.resolve({ query: "token=abcdefgh", scope_kind: "project", scope_id: "project" }), /credentials/);
    await assert.rejects(() => f.context.resolve({ query: "x", scope_kind: "project", scope_id: "project", now: "never" }), /ISO/);
    await assert.rejects(() => f.context.resolve({ query: "x", scope_kind: "project", scope_id: "project", max_items: 0 }), /budget/);
    await assert.rejects(() => f.context.resolve({ query: "x", scope_kind: "project", scope_id: "project", source_ids: ["untrusted"] }), /Source/);
    await assert.rejects(() => f.context.resolve({ query: "x", scope_kind: "project", scope_id: "project", memory_ids: [restricted.id] }), /unavailable/);
    const allowed = await f.context.resolve({ receipt_id: "restricted-ok", query: "restricted", scope_kind: "project", scope_id: "project", source_ids: [trusted.id], memory_ids: [restricted.id], allow_restricted: true, max_items: 1, max_chars: 100 }) as JsonObject;
    assert.equal((allowed.items as JsonObject[])[0].sensitivity, "restricted");
    await assert.rejects(() => f.context.resolve({ receipt_id: "restricted-ok", query: "different", scope_kind: "project", scope_id: "project", source_ids: [trusted.id], memory_ids: [restricted.id], allow_restricted: true, max_items: 1, max_chars: 100 }), /conflict/);
    f.sources.sourceTransition({ source_id: trusted.id, status: "disabled", reason: "maintenance" });
    await assert.rejects(() => f.context.resolve({ query: "normal", scope_kind: "project", scope_id: "project", source_ids: [trusted.id] }), /Source/);
    ...
    const generatedSource = f.sources.sourceRegister({ kind: "project_note", label: "generated", scope_kind: "project", scope_id: "project", locator: "note.md", content_digest: "sha256:note", trust: "verified", access: "read_only" }).source as JsonObject;
    const generatedMemory = f.ledger.remember({ source_id: generatedSource.id, kind: "episodic", scope_kind: "project", scope_id: "project", content: "alpha alpha alpha alpha alpha alpha alpha alpha alpha alpha" }).memory as JsonObject;
    f.ledger.remember({ memory_id: "alpha-two", source_id: generatedSource.id, kind: "episodic", scope_kind: "project", scope_id: "project", content: "alpha alpha alpha alpha alpha alpha alpha alpha alpha alpha" });
    assert.match(String(generatedSource.id), /^knowledge_source_/); assert.match(String(generatedMemory.id), /^memory_ledger_/);
    assert.throws(() => f.ledger.remember({ memory_id: generatedMemory.id, source_id: generatedSource.id, kind: "episodic", scope_kind: "project", scope_id: "project", content: "changed" }), /conflict/);
    assert.throws(() => f.ledger.compatBind({ binding_id: "compat-conflict", legacy_kind: "memory_item", legacy_id: "missing", source_id: generatedSource.id }), /Unknown/);
    f.store.create("memory_item", "compat", { content: "compat" }); f.ledger.compatBind({ binding_id: "compat-conflict", legacy_kind: "memory_item", legacy_id: "compat", source_id: generatedSource.id });
    assert.throws(() => f.ledger.compatBind({ binding_id: "compat-conflict", legacy_kind: "memory_item", legacy_id: "compat", source_id: trusted.id }), /conflict/);
    const nested = f.context.retrievalConfigure({ strategy: "vector", provider_fingerprint: "nested", configuration: { nested: ["safe"] } }).adapter as JsonObject;
