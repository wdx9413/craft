...
test("v0.11.57 closes a local write with a scoped transaction and explicit rollback", async () => {
...
test("v0.11.57 reports a repeated held-out Campaign and routes only a reviewed canary candidate", async () => {
  const f = await fixture();
  try {
...
test("v0.11.57 keeps recovery, report, and routing rejection branches explicit", async () => {
  const f = await fixture();
  try {
