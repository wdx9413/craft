...
async function fixture() {
  ...
  const memory = new KnowledgeMemoryRuntime(store); const runtime = new TurnCognitiveRuntime(store, memory);
  const source = memory.sourceRegister({ source_id: "source", kind: "project_note", label: "project notes", scope_kind: "project", scope_id: "project", locator: "notes.md", content_digest: "sha256:notes", trust: "verified", access: "proposal_only" }).source as JsonObject;
  return { root, store, memory, runtime, source };
}
...
test("v0.12.28 covers conservative policy branches, candidate lifecycle, and replay guards", async () => {
  ...
  try {
    ...
    f.memory.sourceTransition({ source_id: f.source.id, status: "disabled", reason: "disabled" });
...
test("v0.12.28 exhausts default, generated-id, and negative decision paths", async () => {
  ...
  try {
    ...
    const source = f.memory.sourceRegister({ source_id: "untrusted", kind: "custom", label: "untrusted", scope_kind: "project", scope_id: "project", locator: "u", content_digest: "sha256:u", trust: "untrusted", access: "read_only" }).source as JsonObject;
