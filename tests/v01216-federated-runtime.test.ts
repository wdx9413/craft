...
test("v0.12.16 grants only healthy, scoped, one-time remote read authority and records bound receipts", async () => {
...
test("v0.12.16 closes unhealthy, revoked, and expired remote delegation paths without assuming success", async () => {
  const f = await fixture();
  try {
...
test("v0.12.16 keeps multi-Agent topology shadowed until paired evidence and makes deployment gaps explicit", async () => {
  const f = await fixture();
  try {
...
test("v0.12.16 fails closed at every newly introduced authorization and topology boundary", async () => {
  const f = await fixture();
  try {
...
test("v0.12.16 records every remote delegation failure as an explicit boundary", async () => {
  const f = await fixture();
  try {
