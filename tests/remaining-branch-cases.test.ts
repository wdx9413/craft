...
test("project knowledge rejects escaped and symlinked memory paths", async () => {
  ...
  try {
    ...
    const linked = new ProjectKnowledgeKernel(f.store, (() => ({ isSymbolicLink: () => true })) as never);
    assert.throws(() => linked.resolve({ discovery_id: "d", memory_ids: [String(descriptor[0].memory_id)] }), /symbolic/);
  } finally { f.store.close(); await rm(f.root, { recursive: true, force: true }); }
});
...
test("memory governance covers conflict resolution, expiry and consolidation idempotency", async () => {
  ...
  try {
    ...
    const memory = new MemoryGovernanceKernel(f.store, new MemoryLedgerKernel(f.store));
...
test("legacy migration source-state and metadata guards fail closed", async () => {
  ...
  try {
    ...
    const outsideDirectory = join(root, "outside-directory"); await mkdir(outsideDirectory, { recursive: true }); await symlink(outsideDirectory, join(pages, "link.md"), "junction");
    ...
    const escape = await mkdtemp(join(tmpdir(), "legacy-state-escape-")); const escapedRoot = await mkdtemp(join(tmpdir(), "legacy-state-root-")); await mkdir(join(escapedRoot, "data"), { recursive: true }); await symlink(escape, join(escapedRoot, "data", "pages"), process.platform === "win32" ? "junction" : undefined);
