...
test("Workbench Home provides one bounded UI-safe projection without copying authoritative state", async () => {
  ...
  f.store.create("task_run", "task-run", { contract_id: "control", launch_id: "linked", lifecycle: "active" }); f.store.create("task_run_state", "task_run_state_task-run", { status: "ready_for_delivery", action: "deliver", actor: "human" });
  f.store.create("task_run", "unrefreshed-run", { contract_id: "unrefreshed", launch_id: "pending", lifecycle: "paused" });
  const view = f.home.view({ now: "2030-01-01T00:00:00Z", limit: 10 }); const summary = view.summary as Record<string, unknown>;
  assert.deepEqual(summary, { active_tasks: 1, workspaces: 1, attention: 1, active_runs: 1, active_budgets: 1, active_task_controls: 2, active_task_runs: 1, health: "healthy" });
  assert.deepEqual((view.task_runs as Record<string, unknown>[]).map((item) => [item.id, item.status, item.action, item.actor]), [["unrefreshed-run", "not_refreshed", "refresh_task_run", "host"], ["task-run", "ready_for_delivery", "deliver", "human"]]);
...
test("task detail joins exact task evidence and lineage into a bounded safe view", async () => {
  ...
  f.store.create("host_run", "task-host", { task_id: "task", host: "codex-cli", status: "completed", event_count: 1 });
  f.store.appendEvent("host-run:task-host", "host.output", { status: "running" }); f.store.appendEvent("host-run:task-host", "host.finished", { status: "completed" });
  f.store.create("task_message", "task-message", { task_id: "task", role: "user", content: "bounded note", created_at: "2030-01-01T00:00:00Z" });
  f.store.create("task_message", "task-message-2", { task_id: "task", role: "assistant", content: "bounded reply", created_at: "2030-01-01T00:00:01Z" });
  f.store.create("memory_item", "task-memory", { task_id: "task", kind: "decision", content: "Keep the blue frame", scope: "task", source: "human", status: "active" });
  f.store.create("memory_item", "other-memory", { task_id: "other", kind: "decision", content: "hidden other task", scope: "task", status: "active" });
  f.store.create("knowledge_claim", "task-claim", { task_id: "task", kind: "rule", content: "Frames need a title", status: "reviewed", scope: "task:task" });
  f.store.create("workflow", "task-workflow", { name: "Storyboard", description: "Create frames", status: "active" });
  ...
  f.store.create("context_manifest", "manifest", { task_id: "task", summary: "bounded context", status: "active" });
  ...
  // The fixture set holds both thread shapes: a message bound to a model and one that is not.
  const detail = f.home.task({ task_id: "task", limit: 20 }); assert.equal((detail.trials as object[]).length, 2); assert.equal((detail.evidence as object[]).length, 2); assert.equal((detail.artifacts as object[]).length, 1); assert.equal((detail.runs as object[]).length, 3); assert.equal((detail.attention as object[]).length, 1); assert.equal((detail.messages as object[]).length, 4); assert.equal((detail.host_runs as object[]).length, 2); assert.ok((detail.activity as object[]).length > 0); assert.equal(((detail.context as Record<string, unknown>).memories as object[]).length, 1); assert.equal(((detail.context as Record<string, unknown>).knowledge as object[]).length, 1); assert.equal(((detail.context as Record<string, unknown>).workflow_runs as object[]).length, 1); assert.equal(((detail.context as Record<string, unknown>).manifests as object[]).length, 1); assert.equal(JSON.stringify(detail).includes("hidden"), false);
