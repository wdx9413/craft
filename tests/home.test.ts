...
test("task detail joins exact task evidence and lineage into a bounded safe view", async () => {
  ...
  const detail = f.home.task({ task_id: "task", limit: 20 }); assert.equal((detail.trials as object[]).length, 2); assert.equal((detail.evidence as object[]).length, 2); assert.equal((detail.artifacts as object[]).length, 1); assert.equal((detail.runs as object[]).length, 3); assert.equal((detail.attention as object[]).length, 1); assert.equal((detail.messages as object[]).length, 2); assert.equal(((detail.context as Record<string, unknown>).memories as object[]).length, 1); assert.equal(((detail.context as Record<string, unknown>).knowledge as object[]).length, 1); assert.equal(((detail.context as Record<string, unknown>).workflow_runs as object[]).length, 1); assert.equal(JSON.stringify(detail).includes("hidden"), false);
