...
test("knowledge retrieval evaluation does not change routing", async () => {
  ...
  try {
    ...
    const evidence = service.evidenceRecord({ source_type: "program", claim: "Observed.", confidence: "bounded" }); const claim = service.knowledgeClaimSave({ claim_id: "alpha", kind: "fact", content: "alpha rule", evidence_ids: [evidence.id] }).claim as JsonObject; service.knowledgeClaimReview({ claim_id: claim.id, status: "reviewed", reviewer: "h", reason: "checked" });
