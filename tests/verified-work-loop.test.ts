...
test("Project Knowledge reads only trusted Serena project memories, pins selection, and proposes but never writes updates", async () => {
  ...
  try {
    ...
    const full = new McpServer(f.service, "full"); const core = new McpServer(f.service, "core"); assert.ok(full.tools.some((tool) => tool.name === "craft_verified_work_loop_prepare")); assert.ok(core.tools.some((tool) => tool.name === "craft_verified_work_loop_get")); assert.equal(core.tools.some((tool) => tool.name === "craft_verified_work_loop_prepare"), false); assert.equal(((await full.handlers.craft_project_knowledge_discover({ project_root: f.root, trusted: true, discovery_id: "mcp" })).discovery as JsonObject).id, "mcp"); await full.handlers.craft_project_knowledge_resolve({ discovery_id: "mcp", memory_ids: ["serena:style"] }); await full.handlers.craft_project_knowledge_propose_update({ discovery_id: "mcp", memory_id: "serena:style", evidence_ids: [evidence.id], summary: "mcp" });
  } finally { f.store.close(); await rm(f.root, { recursive: true, force: true }); }
});
...
test("v0.11.51 closes the remaining deterministic rejection and idempotency branches", async () => {
  const f = await fixture();
  let directHostRunId = "";
  try {
