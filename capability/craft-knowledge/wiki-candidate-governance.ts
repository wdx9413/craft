...
const WORKFLOW_TARGETS = new Set(["craft-workflow-host"]);
...
export class WikiCandidateGovernanceKernel {
  ...
  attest(args: JsonObject): JsonObject {
    ...
    const attestation = this.store.create("wiki_candidate_evaluation_attestation", attestationId, { ...identity, identity_digest: digestJson(identity), status: "passed" });
  ...
  authorize(args: JsonObject): JsonObject {
    ...
    const identity = { ...proof, attestation_id: attestation.id, reviewer, reason_digest: digestJson(reason) };
    const authorization = this.store.create("wiki_candidate_publication_authorization", authorizationId, { ...identity, identity_digest: digestJson(identity), publication_allowed: true, execution_authority: false });
  ...
  packagePrepare(args: JsonObject): JsonObject {
    ...
    const packageId = String(args.package_id ?? id("wiki_candidate_package")); const identity = { ...proof, authorization_id: authorization.id, target_host: targetHost, package_format: `craft.portable-${candidate.kind}.v1`, content_digest: digestJson(content) };
    const existing = this.store.find("wiki_candidate_publication_package", packageId);
    if (existing) { if (existing.identity_digest !== digestJson(identity)) throw new Error("Wiki candidate publication package idempotency conflict"); return { package: existing, idempotent: true }; }
    const packageRecord = this.store.create("wiki_candidate_publication_package", packageId, { ...identity, identity_digest: digestJson(identity), content, manual_import_required: true, execution_authority: false, status: "prepared" });
