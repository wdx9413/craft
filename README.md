...
从 v0.12.15 起，插件发行物固定在 `plugins/craft/`；Codex 把该轻量目录复制到缓存后无需再执行 `npm install`，也不会依赖源码仓库的 `node_modules`。当前暂不构建或发布桌面 `.app`、Windows ZIP、运行时二进制等平台安装包；它们不会进入插件目录或 Git 历史。升级后请重新安装插件，并在新会话中验证 `craft_info`。
