...
Craft 是独立于具体 AI 产品的本地优先 Agent Harness。它把散落的 Skill、任务状态、验证证据和可复用流程组织成用户自己的能力资产，并通过 MCP、插件和 CLI 提供给不同客户端使用。Codex、Claude 只是目前可以接入 Craft 的客户端，Craft 本身不从属于它们，也不要求用户必须使用其中任何一个。
