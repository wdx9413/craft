...
- For long or expensive work, create a host-neutral budget, call `craft_budget_check` before dispatch with any known predicted usage, then call `craft_budget_record` using measured usage and a stable idempotency key. Honor `warn` and `stop`; do not let a model override a hard limit. Provider-specific token and price conversion belongs in the host adapter.
