...
export class CapabilityFederationKernel {
  ...
  propose(args: JsonObject): JsonObject {
    ...
    const evidenceIds = strings(args.evidence_ids, "evidence_ids", 1); for (const id of evidenceIds) this.store.get("evidence", id);
  ...
  review(args: JsonObject): JsonObject {
    const bundle = this.store.get("capability_bundle", text(args.bundle_id, "bundle_id")); if (bundle.status !== "proposed") throw new Error("Capability bundle is not awaiting review");
    const decision = text(args.decision, "decision"); if (!new Set(["approve", "reject"]).has(decision)) throw new Error("Federation review decision is unsupported");
    const reviewedManifest = args.reviewed_manifest === undefined ? object(bundle.manifest, "manifest") : object(args.reviewed_manifest, "reviewed_manifest"); safe(reviewedManifest);
  ...
  publish(args: JsonObject): JsonObject {
    const bundle = this.store.get("capability_bundle", text(args.bundle_id, "bundle_id"), Number(args.bundle_version)); if (bundle.status !== "reviewed") throw new Error("Only a reviewed Capability bundle can be published");
  ...
  resolve(args: JsonObject): JsonObject {
    const subscription = this.store.get("capability_subscription", text(args.subscription_id, "subscription_id")); if (subscription.status !== "pinned") throw new Error("Capability subscription is not pinned");
    const release = this.store.get("capability_release", String(subscription.release_id), Number(subscription.release_version)); const currentRelease = this.store.get("capability_release", String(subscription.release_id));
  ...
  revoke(args: JsonObject): JsonObject {
    const release = this.store.get("capability_release", text(args.release_id, "release_id")); if (release.status === "revoked") return { release, idempotent: true };
