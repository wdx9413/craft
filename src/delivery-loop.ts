...
function nextAction(status: string): JsonObject {
  ...
  if (status === "awaiting_acceptance") return { phase: "waiting", action: "collect_acceptance" };
  if (status === "blocked") return { phase: "waiting", action: "human_handoff" };
  if (status === "host_failed" || status === "rejected") return { phase: "recovery", action: "retry_or_handoff" };
...
export class DeliveryLoopKernel {
  readonly store: CraftStore; readonly deliveries: WorkDeliveryKernel;
  constructor(store: CraftStore, deliveries: WorkDeliveryKernel) { this.store = store; this.deliveries = deliveries; }
  ...
  get(args: JsonObject): JsonObject { return { loop: this.store.get("delivery_loop", text(args.loop_id, "loop_id")) }; }
}
