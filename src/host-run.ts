...
export class HostRunKernel {
  ...
  constructor(store: CraftStore, drivers: HostDriver[], ownerId = `runner_${randomUUID().replaceAll("-", "")}`, terminalObserver?: (run: JsonObject, receipt: JsonObject | null) => void) { this.store = store; this.drivers = new Map(drivers.map((driver) => [driver.host, driver])); this.ownerId = ownerId; this.terminalObserver = terminalObserver; }
  ...
  start(args: JsonObject): JsonObject {
    ...
    const completion = driver.execute({ ...args, dispatch_id: dispatchId, prompt }, { signal: controller.signal, observe }).then((result) => { const current = this.store.get("host_run", runId); const receipt = result.receipt as JsonObject; const status = current.cancel_requested || receipt.cancelled ? "cancelled" : receipt.status === "completed" ? "completed" : "failed"; const saved = this.store.save("host_run", runId, { ...payload(current), status, receipt_id: receipt.id, finished_at: new Date().toISOString() }); this.store.appendEvent(`host-run:${runId}`, "host.finished", { status, receipt_id: receipt.id }); this.notifyTerminal(saved, receipt); }).catch((error) => { const current = this.store.get("host_run", runId); const saved = this.store.save("host_run", runId, { ...payload(current), status: current.cancel_requested ? "cancelled" : "failed", error_class: error instanceof Error ? error.name : "UnknownError", finished_at: new Date().toISOString() }); this.notifyTerminal(saved, null); }).finally(() => { this.controllers.delete(runId); });
