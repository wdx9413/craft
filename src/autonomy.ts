...
export class AutonomyKernel {
  ...
  policySave(args: JsonObject): JsonObject {
    ...
    const supplied = object(args.rules, "rules"); const rules: JsonObject = {};
    for (const [action, value] of Object.entries(supplied)) {
      ...
      const rule = object(value, `rules.${action}`); const level = text(rule.level, `rules.${action}.level`);
      if (!LEVELS.has(level)) throw new Error(`Unsupported autonomy level: ${level}`);
      if (HIGH_RISK.has(action) && new Set(["automatic", "notify_only"]).has(level)) throw new Error(`${action} cannot bypass explicit approval`);
    ...
    const status = String(args.status ?? "active"); if (!new Set(["active", "paused"]).has(status)) throw new Error("Autonomy policy status is unsupported");
  ...
  request(args: JsonObject): JsonObject {
    ...
    const taskId = text(args.task_id, "task_id"); if (taskId !== policy.task_id) throw new Error("Autonomy request belongs to another task");
    const action = text(args.action, "action"); const rule = (policy.rules as JsonObject)[action] as JsonObject | undefined;
    if (!rule) throw new Error("Autonomy action is not declared by the policy");
    const requestDigest = text(args.request_digest, "request_digest"); if (!/^sha256:[a-f0-9]{64}$/u.test(requestDigest)) throw new Error("request_digest must be sha256 hex");
    ...
    const level = String(rule.level); const status = new Set(["automatic", "notify_only"]).has(level) ? "authorized" : "pending_approval";
  ...
  decide(args: JsonObject): JsonObject {
    ...
    if (request.status !== "pending_approval") throw new Error("Autonomy request is not awaiting approval");
    const now = instant(args.now, "now"); if (now > Date.parse(String(request.expires_at))) throw new Error("Autonomy request expired");
    const actor = text(args.actor, "actor"); const decision = text(args.decision, "decision");
    if (!new Set(["approve", "deny"]).has(decision)) throw new Error("Autonomy decision is unsupported");
    const approvalRef = text(args.approval_ref, "approval_ref"); const approvals = request.approvals as JsonObject[];
  ...
  consumptionPlan(args: JsonObject): { request: JsonObject; existing: JsonObject | null; entries: Array<{ kind: string; id: string; version?: number; payload: JsonObject }> } {
    const request = this.store.get("autonomy_request", text(args.request_id, "request_id")); const now = instant(args.now, "now");
    const consumptionId = `autonomy_use_${request.id}`; const existing = this.store.find("autonomy_consumption", consumptionId);
    if (existing) {
    ...
    if (request.status !== "authorized") throw new Error("Autonomy request is not authorized");
    if (now > Date.parse(String(request.expires_at))) throw new Error("Autonomy authorization expired");
    for (const field of ["task_id", "action", "target", "request_digest"] as const) if (args[field] !== request[field]) throw new Error(`Autonomy ${field} mismatch`);
