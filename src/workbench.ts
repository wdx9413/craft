...
function matchesScope(memory: JsonObject, taskId?: string, workspaceId?: string): boolean {
...
function profileSelector(value: unknown, name: string, allowed: Set<string>): string[] {
  const values = strings(value, name);
  if (values.some((item) => !allowed.has(item))) throw new Error(`${name} contains an unsupported value`);
  return values;
}

function graphNodes(value: unknown): JsonObject[] {
  if (!Array.isArray(value) || !value.length) throw new Error("nodes must contain at least one node");
  ...
  const nodes = value.map((raw, index) => {
    ...
    if (!TASK_GRAPH_NODE_KINDS.has(kind)) throw new Error(`Unsupported task graph node kind: ${kind}`);
  ...
  for (const node of nodes) for (const dependency of node.depends_on as string[]) {
    if (!seen.has(dependency)) throw new Error(`Task graph node ${node.id} has an unknown dependency: ${dependency}`);
  }
  const byId = new Map(nodes.map((node) => [String(node.id), node])); const visiting = new Set<string>(); const visited = new Set<string>();
  const visit = (nodeId: string): void => {
    if (visiting.has(nodeId)) throw new Error(`Task graph contains a dependency cycle at: ${nodeId}`);
    if (visited.has(nodeId)) return;
    visiting.add(nodeId);
    for (const dependency of byId.get(nodeId)!.depends_on as string[]) visit(dependency);
...
export class WorkbenchKernel {
  ...
  objectPut(args: JsonObject): JsonObject {
    ...
    for (const dependencyId of dependencies) {
      ...
      const pending = [dependency]; const visited = new Set<string>();
      while (pending.length) {
        const candidate = pending.pop()!; const candidateId = String(candidate.id);
        if (candidateId === objectId) throw new Error("Work object dependencies must not form a cycle");
        if (visited.has(candidateId)) continue;
        visited.add(candidateId);
        for (const parentId of candidate.depends_on as string[]) pending.push(this.store.get("work_object", parentId));
      }
    }
    ...
    const entries: SaveEntry[] = [{ kind: "work_object", id: objectId, payload: record },
  ...
  impact(args: JsonObject): JsonObject {
    ...
    for (const object of objects) {
      ...
      if (paths.some((path) => objectPaths.includes(path))) roots.add(String(object.id));
    }
    for (const root of roots) {
      const object = this.store.get("work_object", root);
      if (object.workspace_id !== workspaceId) throw new Error("Affected work objects must belong to the workspace");
    }
    ...
    while (changed) {
      changed = false;
      for (const object of objects) {
        if (!impacted.has(String(object.id)) && (object.depends_on as string[]).some((dependency) => impacted.has(dependency))) {
          impacted.add(String(object.id)); changed = true;
        }
      }
    }
  ...
  changeApply(args: JsonObject): JsonObject {
    ...
    const impactedIds = impact.impacted_object_ids as string[];
    const nextRevision = expected + 1;
    const changeId = id(args.change_id, "change_id", "workspace_change");
    const entries: SaveEntry[] = impactedIds.map((objectId) => {
  ...
  remember(args: JsonObject): JsonObject {
    const kind = String(args.kind); const scope = String(args.scope);
    if (!MEMORY_KINDS.has(kind)) throw new Error("Memory kind is unsupported");
    if (!new Set(["user", "workspace", "task"]).has(scope)) throw new Error("Memory scope is unsupported");
    if (scope === "workspace") this.store.get("workspace", text(args.workspace_id, "workspace_id"));
    if (scope === "task") this.store.get("task", text(args.task_id, "task_id"));
  ...
  contextProfileSave(args: JsonObject): JsonObject {
    ...
    if (taskId !== null) this.store.get("task", String(taskId));
    ...
    const maxItems = Number(args.max_items ?? previous?.max_items ?? 12); const maxChars = Number(args.max_chars ?? previous?.max_chars ?? 12_000);
    if (!Number.isInteger(maxItems) || maxItems < 1 || !Number.isInteger(maxChars) || maxChars < 1) throw new Error("Context profile limits must be positive integers");
  ...
  contextProfileAssemble(args: JsonObject): JsonObject {
    ...
    if (args.task_id !== undefined && args.task_id !== profile.task_id) throw new Error("Task does not match the context profile");
    if (args.workspace_id !== undefined && args.workspace_id !== profile.workspace_id) throw new Error("Workspace does not match the context profile");
  ...
  contextAssemble(args: JsonObject): JsonObject {
    ...
    const maxItems = Number(args.limit ?? 12); const maxChars = Number(args.max_chars ?? 12_000);
    ...
    for (const memoryId of requiredMemoryIds) if (!memories.some((item) => item.id === memoryId)) throw new Error("Required memory is unavailable in this context");
    for (const objectId of requiredObjectIds) if (!objects.some((item) => item.id === objectId)) throw new Error("Required work object is unavailable in this context");
    ...
    const selected: typeof candidates = []; let usedChars = 0;
    for (const candidate of candidates) {
      const size = String(candidate.content).length;
      if (selected.length >= maxItems) break;
      if (usedChars + size > maxChars) {
        if (candidate.required) throw new Error("Required context exceeds the context budget");
        continue;
      }
      selected.push(candidate); usedChars += size;
    }
  ...
  taskGraphCreate(args: JsonObject): JsonObject {
    const taskId = args.task_id === undefined ? null : text(args.task_id, "task_id"); const workspaceId = args.workspace_id === undefined ? null : text(args.workspace_id, "workspace_id");
    if (taskId !== null) this.store.get("task", taskId); if (workspaceId !== null) this.store.get("workspace", workspaceId);
    const nodes = graphNodes(args.nodes);
    for (const node of nodes) {
      if (node.context_profile_id === null) {
        if (node.context_profile_version !== null) throw new Error("A context profile version requires a context profile id");
        continue;
      }
      if (!Number.isInteger(node.context_profile_version) || Number(node.context_profile_version) < 1) throw new Error("context_profile_version must be a positive integer");
      const profile = this.store.get("context_profile", String(node.context_profile_id), Number(node.context_profile_version));
      if (profile.task_id !== null && profile.task_id !== taskId) throw new Error("Task graph node profile does not match the task");
    ...
    return { graph, ready_node_ids: (nodes as JsonObject[]).filter((node) => !(node.depends_on as string[]).length).map((node) => node.id) };
  }

  taskGraphAdvance(args: JsonObject): JsonObject {
    const graph = this.store.get("task_graph", text(args.graph_id, "graph_id")); const nodeId = text(args.node_id, "node_id"); const nextStatus = text(args.status, "status");
    if (!TASK_GRAPH_NODE_STATUS.has(nextStatus) || nextStatus === "pending") throw new Error("Task graph node status is unsupported");
    const nodes = (graph.nodes as JsonObject[]).map((node) => ({ ...node })); const node = nodes.find((item) => item.id === nodeId);
    ...
    if (nextStatus === "active" && !(node.depends_on as string[]).every((dependency) => nodes.find((item) => item.id === dependency)?.status === "done" || nodes.find((item) => item.id === dependency)?.status === "skipped")) {
      throw new Error("Task graph dependencies are not complete");
    }
    node.status = nextStatus;
    const blocked = new Set(nodes.filter((item) => item.status === "blocked").map((item) => String(item.id))); let changed = true;
    while (changed) {
      changed = false;
      for (const candidate of nodes) if (candidate.status === "pending" && (candidate.depends_on as string[]).some((dependency) => blocked.has(dependency))) {
        candidate.status = "blocked"; blocked.add(String(candidate.id)); changed = true;
      }
    }
    ...
    return { graph: saved, ready_node_ids: nodes.filter((item) => item.status === "pending" && (item.depends_on as string[]).every((dependency) => ["done", "skipped"].includes(String(nodes.find((candidate) => candidate.id === dependency)?.status)))).map((item) => item.id) };
  }
}
