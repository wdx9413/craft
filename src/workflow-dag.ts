...
const EDGE_KINDS = new Set(["success", "failure", "condition", "retry", "compensation", "human_resume"]);
const JOIN_POLICIES = new Set(["all", "any", "quorum", "partial"]);
...
export type WorkflowEdge = JsonObject & { id: string; from: string; to: string; kind: string };
export class WorkflowDagKernel {
  ...
  validate(args: JsonObject): JsonObject {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (type === "parallel" && n.join_policy !== undefined && !JOIN_POLICIES.has(text(n.join_policy, `nodes[${i}].join_policy`))) throw new Error(`parallel node ${nodeId} has unsupported join_policy`);
      normalized.push({ ...n, id: nodeId, type, depends_on: [...new Set(deps as string[])].sort(), side_effect: sideEffect, ...(type === "parallel" ? { join_policy: n.join_policy ?? "all" } : {}) });
    }
    ...
    // `depends_on` remains a DAG: it is the deterministic scheduling/merge relation.  Runtime
    // feedback belongs in explicit edges so a graph can express a bounded retry or human pause
    // without turning dependency resolution into an unbounded loop.
    const rawEdges = args.edges === undefined ? [] : args.edges;
    if (!Array.isArray(rawEdges)) throw new Error("edges must be an array");
    const edgeIds = new Set<string>(); const edges: WorkflowEdge[] = [];
    for (let i = 0; i < rawEdges.length; i++) {
      const raw = rawEdges[i]; if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error(`edges[${i}] must be an object`);
      const edge = raw as JsonObject; const edgeId = text(edge.id ?? `edge_${i + 1}`, `edges[${i}].id`);
      if (edgeIds.has(edgeId)) throw new Error(`duplicate edge id: ${edgeId}`); edgeIds.add(edgeId);
      const from = text(edge.from, `edges[${i}].from`); const to = text(edge.to, `edges[${i}].to`);
      if (!seen.has(from) || !seen.has(to)) throw new Error(`edge ${edgeId} references an unknown node`);
      const kind = text(edge.kind, `edges[${i}].kind`); if (!EDGE_KINDS.has(kind)) throw new Error(`edge ${edgeId} has unsupported kind`);
      if (kind === "condition") text(edge.predicate_ref, `edges[${i}].predicate_ref`);
      if (kind === "retry") {
        ...
        if (edge.on_exhausted !== undefined && !new Set(["needs_replan", "blocked", "handoff"]).has(text(edge.on_exhausted, `edges[${i}].on_exhausted`))) throw new Error(`retry edge ${edgeId} has unsupported on_exhausted`);
      }
      if (kind === "compensation") text(edge.compensation_ref, `edges[${i}].compensation_ref`);
      if (kind === "human_resume") text(edge.approval_ref, `edges[${i}].approval_ref`);
      edges.push({ ...edge, id: edgeId, from, to, kind });
    }
    const outgoing = new Map<string, WorkflowEdge[]>(); for (const edge of edges) outgoing.set(edge.from, [...(outgoing.get(edge.from) ?? []), edge]);
    const active = new Set<string>(); const path: WorkflowEdge[] = []; const checked = new Set<string>();
    const walk = (nodeId: string): void => {
      active.add(nodeId);
      for (const edge of outgoing.get(nodeId) ?? []) {
        if (active.has(edge.to)) {
          const cycle = [...path.slice(path.findIndex((item) => item.from === edge.to)), edge];
        ...
        if (!checked.has(edge.to)) { path.push(edge); walk(edge.to); path.pop(); }
      }
      active.delete(nodeId); checked.add(nodeId);
    };
    for (const node of normalized) if (!checked.has(node.id)) walk(node.id);
  ...
  save(args: JsonObject): JsonObject {
    ...
    if (SECRET.test(JSON.stringify({ graph, derived_from: args.derived_from ?? null })) || SECRET.test(String(args.name)) || SECRET.test(String(args.description ?? ""))) throw new Error("Workflow definition must not contain credentials or secrets");
    const derivedFrom = args.derived_from === undefined ? null : args.derived_from;
    if (derivedFrom !== null && (typeof derivedFrom !== "object" || Array.isArray(derivedFrom))) throw new Error("derived_from must be an object");
    const identity = { workflow_id: workflowId, name: text(args.name, "name"), description: String(args.description ?? ""), graph, graph_digest: digest(graph), derived_from: derivedFrom }; const existing = this.store.find("workflow_dag", workflowId);
    ...
    mkdirSync(join(this.store.paths.root, "workflows"), { recursive: true }); const filePath = join(this.store.paths.root, "workflows", `${workflowId}.workflow.json`); const document = { workflow_id: workflowId, name: identity.name, description: identity.description, ...graph, graph_digest: identity.graph_digest, derived_from: derivedFrom }; writeFileSync(filePath, `${JSON.stringify(document, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  ...
  get(args: JsonObject): JsonObject { return { workflow: this.store.get("workflow_dag", text(args.workflow_id, "workflow_id"), args.version === undefined ? undefined : Number(args.version)) }; }
