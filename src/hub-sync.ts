...
function entry(value: unknown): JsonObject {
  ...
  const result: JsonObject = { entry_id: text(item.entry_id, "entry.entry_id"), status, content_digest: text(item.content_digest, "entry.content_digest") };
  if (status === "active") Object.assign(result, { release_id: text(item.release_id, "entry.release_id"), release_version: integer(item.release_version, "entry.release_version", 1, Number.MAX_SAFE_INTEGER),
...
export class HubSyncKernel {
  ...
  ingest(args: JsonObject): JsonObject {
    ...
    const saved = this.store.transaction((database) => {
      const now = new Date().toISOString(); const insert = (kind: string, id: string, data: JsonObject, version: number) => { database.prepare("INSERT INTO records(kind,id,version,payload_json,created_at,updated_at) VALUES(?,?,?,?,?,?)").run(kind, id, version, JSON.stringify(data), now, now); return { ...data, id, version, created_at: now, updated_at: now }; };
      for (const item of entries) { const id = `${source.id}:${item.entry_id}`; const row = database.prepare("SELECT COALESCE(MAX(version),0)+1 version FROM records WHERE kind='hub_catalog_entry' AND id=?").get(id) as { version: number };
      ...
      const receipt = insert("hub_sync_receipt", receiptId, { source_id: source.id, revision, previous_digest: previousDigest, page_digest: pageDigest,
        entry_count: entries.length, signature_verified: true, issued_at: issuedAt }, 1); return { source: nextSource, receipt };
    });
    return { ...saved, idempotent: false };
  }

  search(args: JsonObject): JsonObject {
    const terms = text(args.query, "query").toLowerCase().split(/\s+/u); const limit = integer(args.limit ?? 10, "limit", 1, 50);
    const entries = this.store.list("hub_catalog_entry", 100_000, (item) => item.status === "active").map((item) => { const haystack = `${item.name} ${item.description} ${(item.tags as string[]).join(" ")}`.toLowerCase();
      return { ...item, score: terms.reduce((sum, term) => sum + Number(haystack.includes(term)), 0) }; }).filter((item) => Number(item.score) > 0)
      .sort((left, right) => Number(right.score) - Number(left.score) || String((left as JsonObject).id).localeCompare(String((right as JsonObject).id))).slice(0, limit);
    return { entries, retrieval: "local_signed_catalog", scanned_remote: false };
  }

  sourceDisable(args: JsonObject): JsonObject {
    const source = this.store.get("hub_source", text(args.source_id, "source_id")); if (source.status === "disabled") return { source, idempotent: true };
