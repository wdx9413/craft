...
export class A2ADiscoveryKernel {
  ...
  async discover(args: JsonObject): Promise<JsonObject> {
    ...
    const identity = { discovery_url: url.toString(), name, endpoint: endpoint.toString(), protocol_version: typeof card.protocolVersion === "string" ? card.protocolVersion : null, skills: strings(card.skills).length ? strings(card.skills) : Array.isArray(card.skills) ? (card.skills as unknown[]).map((skill) => object(skill, "A2A Agent Card.skill")).map((skill) => text(skill.id, "A2A Agent Card.skill.id")) : [], card_digest: digestJson(card) }; const cardId = String(args.card_id ?? `a2a_agent_card_${randomUUID().replaceAll("-", "")}`); const existing = this.store.find("a2a_agent_card", cardId); if (existing) { if (existing.identity_digest !== digestJson(identity)) throw new Error("A2A Agent Card idempotency conflict or drift"); return { card: existing, idempotent: true }; }
    const record = this.store.create("a2a_agent_card", cardId, { ...identity, identity_digest: digestJson(identity), discovered_at: new Date().toISOString(), trusted: false, execution_authority: false, status: "discovered" }); return { card: record, idempotent: false };
  }
