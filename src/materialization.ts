...
function files(value: unknown): { path: string; content: Buffer; size: number; digest: string }[] {
  ...
  const result = value.map((raw) => { if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("Materialized file must be an object"); const item = raw as JsonObject; const filePath = safeRelative(item.path); const content = decode(item.content_base64); if (content.length > 1_048_576) throw new Error("Materialized file exceeds 1 MiB"); return { path: filePath, content, size: content.length, digest: `sha256:${createHash("sha256").update(content).digest("hex")}` }; });
  ...
  if (result.reduce((sum, item) => sum + item.size, 0) > 5_242_880) throw new Error("Materialized package exceeds 5 MiB"); return result;
}
function findings(items: { path: string; content: Buffer }[]): JsonObject[] {
  const result: JsonObject[] = []; for (const item of items) { const extension = path.posix.extname(item.path).toLowerCase(); const body = item.content.toString("utf8");
    if (new Set([".exe", ".dll", ".so", ".dylib", ".wasm", ".node"]).has(extension)) result.push({ severity: "critical", code: "native_executable", path: item.path });
    if (/(?:bearer\s+[a-z0-9._-]{8,}|sk-[a-z0-9_-]{8,}|-----BEGIN [A-Z ]*PRIVATE KEY-----)/iu.test(body)) result.push({ severity: "high", code: "secret_like_content", path: item.path });
    if (path.posix.basename(item.path).toLowerCase() === "package.json") { try { const parsed = JSON.parse(body) as JsonObject; if (parsed.scripts && typeof parsed.scripts === "object" && Object.keys(parsed.scripts as JsonObject).length) result.push({ severity: "high", code: "package_lifecycle_scripts", path: item.path }); } catch { result.push({ severity: "critical", code: "invalid_package_json", path: item.path }); } }
  } return result;
}
...
export class MaterializationKernel {
  ...
  async stage(args: JsonObject): Promise<JsonObject> {
  ...
  review(args: JsonObject): JsonObject {
    const item = this.store.get("capability_materialization", text(args.materialization_id, "materialization_id")); if (item.status !== "quarantined") throw new Error("Materialization is not awaiting review");
    const decision = text(args.decision, "decision"); if (!new Set(["approve", "reject"]).has(decision)) throw new Error("Materialization review decision is unsupported");
    ...
    for (const id of evidenceIds) this.store.get("evidence", id); if (decision === "approve" && item.risk_level === "high" && !args.security_approval_ref) throw new Error("High-risk materialization requires security approval");
  ...
  activate(args: JsonObject): JsonObject {
    const item = this.store.get("capability_materialization", text(args.materialization_id, "materialization_id")); if (item.status !== "approved") throw new Error("Only an approved materialization can become a Capability candidate");
    const source = this.store.get("hub_source", String(item.source_id)); if (source.status !== "active") throw new Error("Materialization source is not active");
    const entry = this.store.get("hub_catalog_entry", String(item.entry_record_id)); if (entry.status !== "active" || entry.content_digest !== item.content_digest) throw new Error("Materialized catalog entry was withdrawn or changed");
    const assetType = text(args.asset_type, "asset_type"); const effect = text(args.effect, "effect"); if (!TYPES.has(assetType) || !EFFECTS.has(effect)) throw new Error("Materialized Capability type or effect is unsupported");
    const assetId = text(args.asset_id, "asset_id"); const current = this.store.find("capability_asset", assetId); const asset = this.store.save("capability_asset", assetId, { name: text(args.name ?? entry.name, "name"), asset_type: assetType,
