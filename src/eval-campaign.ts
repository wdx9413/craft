...
export class EvalCampaignKernel {
  readonly store: CraftStore; readonly benchmarks: TaskBenchmarkKernel;
  constructor(store: CraftStore, benchmarks: TaskBenchmarkKernel) { this.store = store; this.benchmarks = benchmarks; }
  ...
  bind(args: JsonObject): JsonObject {
    const slot = this.store.get("eval_campaign_slot", text(args.slot_id, "slot_id")); const campaign = this.store.get("eval_campaign", String(slot.campaign_id)); const run = this.store.get("task_run", text(args.task_run_id, "task_run_id"));
    if (slot.status === "bound" && slot.task_run_id === run.id) return { slot, idempotent: true };
    if (slot.status !== "pending") throw new Error("Eval Campaign Slot is already bound"); if (run.environment_digest !== campaign.environment_digest || run.budget_digest !== campaign.budget_digest) throw new Error("Eval Campaign Run does not meet the campaign environment or budget");
  ...
  advance(args: JsonObject): JsonObject {
    const campaign = this.store.get("eval_campaign", text(args.campaign_id, "campaign_id")); const slots = this.slots(String(campaign.id)); if (slots.some((slot) => slot.status !== "bound")) return { campaign, status: "collecting", benchmarks: [], evaluation: null };
    const pairs = new Map<string, JsonObject[]>(); for (const slot of slots) { const key = `${slot.case_id}:${slot.trial}`; pairs.set(key, [...(pairs.get(key) ?? []), slot]); }
    const benchmarks = [...pairs.entries()].map(([key, pair]) => { const baseline = pair.find((slot) => slot.arm === "baseline"); const candidate = pair.find((slot) => slot.arm === "candidate"); if (!baseline || !candidate) throw new Error("Eval Campaign pair is incomplete"); const benchmark = this.benchmarks.create({ benchmark_id: `eval_campaign_benchmark_${campaign.id}_${key.replace(':', '_')}`, case_id: baseline.case_id, baseline_task_run_id: baseline.task_run_id, candidate_task_run_id: candidate.task_run_id }).benchmark as JsonObject; return this.benchmarks.evaluate({ benchmark_id: benchmark.id }).benchmark as JsonObject; });
    if (benchmarks.some((item) => item.status === "awaiting_delivery")) return { campaign, status: "awaiting_delivery", benchmarks, evaluation: null };
    if (benchmarks.some((item) => item.status !== "ready_for_aggregate")) return { campaign: this.store.save("eval_campaign", String(campaign.id), { ...payload(campaign), lifecycle: "inconclusive" }), status: "inconclusive", benchmarks, evaluation: null };
    const evaluation = this.benchmarks.aggregate({ benchmark_ids: benchmarks.map((item) => item.id), run_id: `eval_campaign_run_${campaign.id}`, min_trials: campaign.trials_per_case }).evaluation as JsonObject;
  ...
  get(args: JsonObject): JsonObject { const campaign = this.store.get("eval_campaign", text(args.campaign_id, "campaign_id")); return { campaign, slots: this.slots(String(campaign.id)) }; }
  private slots(campaignId: string): JsonObject[] { return this.store.list("eval_campaign_slot", 10_000, (item) => item.campaign_id === campaignId); }
}
