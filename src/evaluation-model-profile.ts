...
import { credentialStatus, selectModel, type ModelProviderSpec, type ModelTier } from "./model-gateway.ts";
...
function canonical(value: unknown): string {
  ...
  if (value && typeof value === "object") return `{${Object.entries(value as JsonObject).sort(([left], [right]) => left.localeCompare(right)).map(([key, child]) => `${JSON.stringify(key)}:${canonical(child)}`).join(",")}}`;
...
export class EvaluationModelProfileKernel {
  ...
  save(args: JsonObject): JsonObject {
    ...
    const profileId = String(args.profile_id ?? `evaluation_model_profile_${randomUUID().replaceAll("-", "")}`); const existing = this.store.find("evaluation_model_profile", profileId); const identityDigest = digest(identity);
  ...
  issue(args: JsonObject): JsonObject {
    ...
    const ticketId = String(args.ticket_id ?? `evaluation_model_ticket_${digest(identity).slice(-20)}`); const existing = this.store.find("evaluation_model_ticket", ticketId); const identityDigest = digest(identity);
