...
function ref(store: CraftStore, kind: string, idValue: unknown, versionValue: unknown): JsonObject | null {
  ...
  if (versionValue !== undefined && Number(versionValue) !== Number(item.version)) throw new Error(`${kind} version does not match current state`);
  return item;
}
function launchEffect(launch: JsonObject): string { return launch.sandbox === "read-only" ? "read_only" : launch.sandbox === "workspace-write" ? "local_write" : "unsupported"; }
function controlState(launch: JsonObject | null, run: JsonObject | null, loop: JsonObject | null): JsonObject {
  if (!launch) return { status: "prepared", action: "prepare_work_launch", actor: "human_or_host" };
  if (launch.status === "awaiting_approval") return { status: "awaiting_approval", action: "review_work_launch", actor: "human" };
  if (launch.status === "denied") return { status: "blocked", action: "revise_contract_or_open_new_launch", actor: "human" };
  if (!run || !["completed", "failed", "cancelled", "interrupted"].includes(String(run.status))) return { status: "running", action: "wait_for_host", actor: "host" };
  ...
  if (action === "human_handoff") return { status: "blocked", action, actor: "human" };
...
export class TaskControlKernel {
  readonly store: CraftStore; readonly delivery: DeliveryLoopKernel;
  constructor(store: CraftStore, delivery: DeliveryLoopKernel) { this.store = store; this.delivery = delivery; }

  save(args: JsonObject): JsonObject {
    const task = this.store.get("task", text(args.task_id, "task_id")); const allowedEffects = values(args.allowed_effects); const workspace = resolve(text(args.workspace, "workspace"));
    ...
    if (profile && Array.isArray(profile.allowed_effects) && (profile.allowed_effects as unknown[]).some((item) => !allowedEffects.includes(String(item)))) throw new Error("Activation profile effect exceeds task contract");
  ...
  bindLaunch(args: JsonObject): JsonObject {
    const contract = this.store.get("task_control_contract", text(args.contract_id, "contract_id")); const launch = this.store.get("work_launch", text(args.launch_id, "launch_id"));
    ...
    if (resolve(String(launch.workspace)) !== contract.workspace) throw new Error("Work Launch workspace does not match task control contract");
    if (!(contract.allowed_effects as string[]).includes(launchEffect(launch))) throw new Error("Work Launch effect is not allowed by task control contract");
    ...
    if (contract.launch_id !== null) { if (contract.launch_id !== launch.id) throw new Error("Task control contract is already bound to another Work Launch"); return { contract, state: this.refresh({ contract_id: contract.id }).state, idempotent: true }; }
  ...
  refresh(args: JsonObject): JsonObject {
    const contract = this.store.get("task_control_contract", text(args.contract_id, "contract_id")); const launch = contract.launch_id ? this.store.get("work_launch", String(contract.launch_id)) : null;
    const run = launch?.run_id ? this.store.find("host_run", String(launch.run_id)) : null; const loop = launch ? this.delivery.refresh({ launch_id: launch.id }).loop as JsonObject : null;
  ...
  get(args: JsonObject): JsonObject {
    const contract = this.store.get("task_control_contract", text(args.contract_id, "contract_id")); const state = this.store.find("task_control_state", `task_control_state_${contract.id}`);
