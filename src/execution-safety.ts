...
export class ExecutionSafetyKernel {
  readonly store: CraftStore; readonly sandbox: SandboxKernel;
  ...
  validate(args: JsonObject): JsonObject {
    ...
    if (preflight.status !== "passed") throw new Error("Safety preflight is not passed"); const profile = this.store.get("sandbox_profile", String(preflight.profile_id), Number(preflight.profile_version));
