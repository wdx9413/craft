...
export class ProviderRouterKernel {
  ...
  plan(args: JsonObject): JsonObject {
    ...
    const routeId = String(args.route_id ?? `provider_route_${randomUUID().replaceAll("-", "")}`); const identity = { providers, preferred, fallback: providers.filter((item) => item !== preferred), task_id: args.task_id ?? null, budget_digest: digest(args.budget ?? {}) };
