...
export class ActionGatewayKernel {
  ...
  prepare(args: JsonObject): JsonObject {
    ...
    const actionDigest = digest(contract); const existing = this.store.find("action_gateway", actionId);
...
export class AcceptanceGateKernel {
  ...
  prepare(args: JsonObject): JsonObject {
    ...
    const gateDigest = digest(identity); const existing = this.store.find("acceptance_gate", gateId);
...
export class DurableWorkerKernel {
  ...
  enqueue(args: JsonObject): JsonObject { const workerId = String(fallback(args.worker_id, "craft-worker")); const jobId = String(fallback(args.job_id, `job_${randomUUID().replaceAll("-", "")}`)); const existing = this.store.find("worker_job", jobId); if (existing) return { job: existing, idempotent: true }; return { job: this.store.create("worker_job", jobId, { worker_id: workerId, task_id: text(args.task_id, "task_id"), action: text(args.action, "action"), status: "pending", payload_digest: digest(fallback(args.payload, {})) }), idempotent: false }; }
...
export class ProviderRouterKernel {
  ...
  plan(args: JsonObject): JsonObject {
    ...
    const routeId = String(fallback(args.route_id, `provider_route_${randomUUID().replaceAll("-", "")}`)); const identity = { providers, preferred, fallback: providers.filter((item) => item !== preferred), task_id: args.task_id ?? null, budget_digest: digest(fallback(args.budget, {})) };
    const existing = this.store.find("provider_route", routeId); if (existing) { if (existing.route_digest !== digest(identity)) throw new Error("Provider route idempotency conflict"); return { route: existing, idempotent: true }; }
