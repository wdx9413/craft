...
export class ExecutionFabricKernel {
  ...
  create(args: JsonObject): JsonObject {
    ...
    const fabricId = String(args.fabric_id ?? `execution_fabric_${loop.id}`); const existing = this.store.find("execution_fabric", fabricId); const identityDigest = digestJson(identity);
  ...
  advance(args: JsonObject): JsonObject {
    ...
    const advanceId = String(args.advance_id ?? `execution_fabric_advance_${fabric.id}_${receipt.id}_${receipt.version}_${activationReceipt?.id ?? "none"}`); const existing = this.store.find("execution_fabric_advance", advanceId); const identityDigest = digestJson(identity);
