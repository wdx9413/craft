...
export class HostSessionEventKernel {
  ...
  open(args: JsonObject): JsonObject {
    ...
    const identityDigest = digest(identity); const existing = this.store.find("host_session", sessionId);
  ...
  append(args: JsonObject): JsonObject {
    ...
    const eventId = requestedEventId ?? `${session.id}:${sequence}`; const eventDigest = digest(identity);
