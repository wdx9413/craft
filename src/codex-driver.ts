...
export class CodexHostKernel implements HostDriver {
  ...
  prepare(args: JsonObject): JsonObject {
    ...
    const model = args.model === undefined ? null : text(args.model, "model"); const dispatchId = String(args.dispatch_id ?? `codex_dispatch_${randomUUID().replaceAll("-", "")}`);
    const identity = { task_id: task.id, task_version: task.version, workspace, sandbox, model, prompt_digest: digest(prompt) }; const requestDigest = digest(identity);
  ...
  async execute(args: JsonObject, options: { signal?: AbortSignal; observe?: HostOutputObserver } = {}): Promise<JsonObject> {
    ...
    if (dispatch.status !== "prepared") throw new Error("Codex dispatch is not executable"); if (digest(prompt) !== dispatch.prompt_digest) throw new Error("Codex prompt does not match the prepared digest");
    ...
    const argv = ["exec", "--json", "--ephemeral", "--sandbox", String(dispatch.sandbox), "--cd", String(dispatch.workspace)]; if (dispatch.model) argv.push("--model", String(dispatch.model)); argv.push("-");
