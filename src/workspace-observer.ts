...
export class WorkspaceObserverKernel {
  readonly store: CraftStore; readonly states: StateWorkspaceKernel;
  constructor(store: CraftStore, states: StateWorkspaceKernel) { this.store = store; this.states = states; }

  observe(args: JsonObject): JsonObject {
    const workspaceId = text(args.workspace_id, "workspace_id"); this.store.get("workspace", workspaceId);
    const snapshot = this.states.observe({ workspace_id: workspaceId, adapter: args.adapter, paths: args.paths, artifact_ids: args.artifact_ids, snapshot_id: args.snapshot_id }).snapshot as JsonObject;
    const priorId = args.previous_snapshot_id === undefined ? null : text(args.previous_snapshot_id, "previous_snapshot_id"); const prior = priorId ? this.store.get("state_snapshot", priorId) : null;
    if (prior && prior.workspace_id !== workspaceId) throw new Error("Workspace observation snapshots must belong to one Workspace");
    const difference = prior ? this.states.compare({ before_snapshot_id: prior.id, after_snapshot_id: snapshot.id }).difference as JsonObject : { added_paths: [], deleted_paths: [], modified_paths: [], changed: false };
  ...
  get(args: JsonObject): JsonObject { const observation = this.store.get("workspace_observation", text(args.observation_id, "observation_id")); return { observation, timeline: this.store.events(`workspace:${observation.workspace_id}`) }; }
}
