...
export interface RemoteTransport {
  dispatch(endpoint: string, envelope: JsonObject): Promise<{ remote_id: string; status: "accepted" | "completed" | "failed"; result_digest?: string }>;
}
...
export class RemoteInteropKernel {
  ...
  async dispatch(args: JsonObject, transport: RemoteTransport): Promise<JsonObject> {
    const request = this.store.get("remote_request", text(args.request_id, "request_id"));
    if (request.status === "completed" || request.status === "failed") return { request, idempotent: true };
    if (request.status !== "prepared" && request.status !== "accepted") throw new Error("Remote request is not dispatchable");
  ...
  report(args: JsonObject): JsonObject {
    ...
    if (!(new Set(["completed", "failed", "cancelled"]).has(status))) throw new Error("Unsupported remote report status");
    const resultDigest = args.result_digest === undefined ? null : text(args.result_digest, "result_digest");
    if (request.status === status && request.result_digest === resultDigest) return { request, idempotent: true };
    if (request.status === "completed" || request.status === "failed") throw new Error("Remote request is already terminal");
  ...
  get(args: JsonObject): JsonObject { const request = this.store.get("remote_request", text(args.request_id, "request_id")); return { request, receipts: this.store.list("remote_receipt", 100, (item) => item.request_id === request.id) }; }
}
