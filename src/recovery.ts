...
type Candidate = { id: string; task_id: string; action: string; priority: number; subject_kind: string;
  subject_id: string; subject_version: number; subject_status: string; details: JsonObject };
...
export class RecoveryQueueKernel {
  ...
  refresh(args: JsonObject = {}): JsonObject {
    const now = instant(args.now, "now"); const limit = positiveInteger(args.limit, "limit", 100, 1000);
    this.recoverExpired({ now: new Date(now).toISOString(), limit });
    const candidates = this.candidates(now).slice(0, limit); const active = new Set(candidates.map((item) => item.id));
  ...
  claim(args: JsonObject): JsonObject {
    const now = instant(args.now, "now"); this.recoverExpired({ now: new Date(now).toISOString(), limit: 1000 });
    const actions = stringArray(args.actions, "actions"); const workerId = text(args.worker_id, "worker_id");
    ...
    const token = randomUUID(); const leased = this.store.updateIfVersion("recovery_item", String(item.id), Number(item.version), {
  ...
  report(args: JsonObject): JsonObject {
    ...
    const lease = item.lease as JsonObject; const now = instant(args.now, "now");
    ...
    if (!new Set(["completed", "failed", "deferred"]).has(outcome)) throw new Error("Unsupported recovery outcome");
  ...
  recoverExpired(args: JsonObject = {}): JsonObject {
    const now = instant(args.now, "now"); const limit = positiveInteger(args.limit, "limit", 100, 1000); let recovered = 0;
    for (const item of this.store.list("recovery_item", 10_000, (entry) => entry.status === "leased").slice(0, limit)) {
      const lease = item.lease as JsonObject;
      if (now >= Date.parse(String(lease.expires_at))) {
        this.store.updateIfVersion("recovery_item", String(item.id), Number(item.version), {
          ...payload(item), status: "open", lease: null, recovered_at: new Date(now).toISOString() }); recovered += 1;
      }
    }
    return { recovered };
  }

  private candidates(now: number): Candidate[] {
    ...
    for (const saga of this.store.list("effect_saga", 10_000)) {
      const effects = (saga.effect_ids as string[]).map((id) => this.store.get("external_effect", id));
    ...
    for (const certification of this.store.list("capability_certification", 10_000, (item) => item.status === "invalidated")) {
      candidates.push(this.candidate(certification, "capability_certification", "recertify_capability", 85,
        { asset_id: certification.asset_id, materialization_id: certification.materialization_id,
  ...
  private candidate(subject: JsonObject, kind: string, action: string, priority: number, details: JsonObject): Candidate {
