...
export class PlatformExecutionKernel {
  ...
  conformanceRecord(args: JsonObject): JsonObject {
    const platform = text(args.platform, "platform"); const verifier = text(args.verifier, "verifier"); const checks = args.checks;
  ...
  validate(args: JsonObject): JsonObject {
    ...
    if (observedBoundary !== requiredBoundary) throw new Error("Platform preflight is no longer valid"); if (profile.conformance) this.conformanceValidate(profile.conformance as JsonObject, String(preflight.platform));
  ...
  private conformanceValidate(reference: JsonObject, platform: string): void { const record = this.store.get("platform_execution_conformance", text(reference.id, "conformance.id"), Number(reference.version)); if (record.status !== "verified" || record.platform !== platform) throw new Error("Platform conformance is no longer valid"); }
}
