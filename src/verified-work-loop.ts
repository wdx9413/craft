...
export class VerifiedWorkLoopKernel {
  ...
  advance(args: JsonObject): JsonObject {
    ...
    const receiptId = String(args.receipt_id ?? `work_loop_receipt_${saved.id}_${snapshot.version}_${state.version}`); const identity = { work_loop_id: saved.id, work_loop_version: saved.version, task_run_state_id: state.id, task_run_state_version: state.version, snapshot_id: snapshot.id, snapshot_version: snapshot.version, status, reason };
  ...
  decide(args: JsonObject): JsonObject {
    ...
    if (["human_change", "replan", "reject"].includes(decision)) this.store.save("verified_work_loop", String(loop.id), { ...payload(loop), lifecycle: "needs_replan", needs_replan_reason: decision });
    return { decision: saved, idempotent: false };
  }

  get(args: JsonObject): JsonObject { const loop = this.store.get("verified_work_loop", text(args.work_loop_id, "work_loop_id")); return { loop, receipts: this.store.list("verified_work_loop_receipt", 1000, (item) => item.work_loop_id === loop.id), decisions: this.store.list("verified_work_loop_decision", 1000, (item) => item.work_loop_id === loop.id) }; }
}
