...
export class WorkbenchExperienceKernel {
  ...
  query(args: JsonObject = {}): JsonObject {
    ...
    return { ...projection, projection_digest: digestJson(projection), content_free: true };
  }
  ...
  replayPlan(args: JsonObject): JsonObject {
    ...
    const steps = events.filter((event) => event.action_contract !== null).map((event) => ({ sequence: event.sequence, event_kind: event.event_kind, action_digest: digestJson(event.action_contract), input_refs: event.input_refs, output_refs: event.output_refs }));
  ...
  review(args: JsonObject): JsonObject { const result = this.query(args); return { ...result, review_digest: digestJson(result), recommendations: [result.next_action], evidence_required: true }; }
