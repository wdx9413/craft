...
export class AgentEvalLabKernel {
  ...
  attach(args: JsonObject): JsonObject {
    const lab = this.store.get("agent_eval_lab", text(args.lab_id, "lab_id")); const dispatch = this.store.get("campaign_runner_dispatch", text(args.dispatch_id, "dispatch_id")); const taskRun = this.store.get("task_run", text(args.task_run_id, "task_run_id")); const hostRun = this.store.get("host_run", text(args.host_run_id, "host_run_id")); const coordinator = this.store.get("work_coordinator", text(args.coordinator_id, "coordinator_id"));
  ...
  observe(args: JsonObject): JsonObject {
    const attempt = this.store.get("agent_eval_attempt", text(args.attempt_id, "attempt_id")); const coordinator = this.store.get("work_coordinator", String(attempt.coordinator_id)); const run = this.store.get("host_run", String(attempt.host_run_id));
    if (coordinator.latest_observation_id !== text(args.observation_id, "observation_id")) throw new Error("Agent Eval outcome requires the Coordinator's latest observation");
    const observation = this.store.get("managed_run_observation", String(coordinator.latest_observation_id)); const terminal = ["needs_replan", "paused", "observed"].includes(String(coordinator.lifecycle));
    if (!terminal || !["completed", "failed", "cancelled", "interrupted"].includes(String(run.status))) throw new Error("Agent Eval attempt requires a terminal Host Run and re-observation");
    const lifecycle = observation.status === "needs_replan" ? "inconclusive" : "observed"; const saved = this.store.save("agent_eval_attempt", String(attempt.id), { ...attempt, lifecycle, observation_id: observation.id, observation_version: observation.version, host_status: run.status });
    return { attempt: saved, eligible_for_campaign: lifecycle === "observed" };
  }

  get(args: JsonObject): JsonObject { const lab = this.store.get("agent_eval_lab", text(args.lab_id, "lab_id")); return { lab, attempts: this.store.list("agent_eval_attempt", 10_000, (item) => item.lab_id === lab.id) }; }
}
