...
function uniqueTextArray(value: unknown, name: string): string[] {
  ...
  if (!values.length) throw new Error(`${name} must contain at least 1 unique values`);
  return values;
}
...
export class CapabilityAccessKernel {
  ...
  assetSave(args: JsonObject): JsonObject {
    ...
    const dependencies = optionalTextArray(args.dependencies, "dependencies"); const aliases = optionalTextArray(args.aliases, "aliases");
  ...
  accessPlan(args: JsonObject): JsonObject {
    ...
    const candidates = this.store.list("capability_asset", 10_000).map((asset) => {
      ...
      const costHint = asset.cost_hint as JsonObject; const historical = Number(asset.historical_success_rate);
  ...
  private saveVersioned(kind: string, prefix: string, args: JsonObject, required: string[]): JsonObject {
    for (const key of required) text(args[key], key);
    const recordId = String(args[`${prefix}_id`] ?? id(prefix)); const payload = { ...args };
