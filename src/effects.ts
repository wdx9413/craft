...
function statusCodes(value: unknown, name: string): number[] {
  if (!Array.isArray(value) || !value.length) throw new Error(`${name} must be a non-empty array`);
...
function generated(prefix: string): string { return `${prefix}_${randomUUID().replaceAll("-", "")}`; }
...
export class ExternalEffectKernel {
  ...
  prepare(args: JsonObject): JsonObject {
    ...
    if (!new Set(["external_write", "destructive"]).has(effect)) throw new Error("External effect must be external_write or destructive");
  ...
  startPlan(args: JsonObject): { operation: JsonObject; payload: JsonObject; dispatch: JsonObject; idempotent: boolean } {
    const operation = this.store.get("external_effect", text(args.effect_id, "effect_id"));
    if (operation.status === "executing") return { operation, payload: payload(operation), dispatch: {}, idempotent: true };
    if (operation.status !== "prepared") throw new Error("Only a prepared external effect can start");
    if (text(args.approval_ref, "approval_ref") !== operation.approval_ref) throw new Error("External effect approval does not match");
    const next = {
  ...
  report(args: JsonObject): JsonObject {
    const operation = this.store.get("external_effect", text(args.effect_id, "effect_id"));
    const receiptId = text(args.receipt_id, "receipt_id"); const status = text(args.status, "status");
    ...
    if (existing) {
      if (existing.fingerprint !== fingerprint) throw new Error("External effect receipt idempotency conflict");
  ...
  resolve(args: JsonObject): JsonObject {
    ...
    if (!new Set(["succeeded", "failed"]).has(resolution)) throw new Error("resolution must be succeeded or failed");
    const evidenceIds = strings(args.evidence_ids, "evidence_ids"); for (const evidenceId of evidenceIds) this.store.get("evidence", evidenceId);
    ...
    const updated = this.store.updateIfVersion("external_effect", String(operation.id), Number(operation.version), {
      ...
      evidence_ids: [...new Set([...(operation.evidence_ids as string[]), ...evidenceIds])],
  ...
  reconcileIssue(args: JsonObject): JsonObject {
    ...
    if (existing) {
      if (existing.fingerprint !== fingerprint) throw new Error("Effect reconciliation idempotency conflict");
  ...
  reconcileReport(args: JsonObject): JsonObject {
    ...
    const result = (reconciliation.succeeded_http_statuses as number[]).includes(httpStatus) ? "succeeded"
      : (reconciliation.failed_http_statuses as number[]).includes(httpStatus) ? "failed" : "indeterminate";
    ...
    const updated = this.store.updateIfVersion("external_effect", String(operation.id), Number(operation.version), {
      ...
      evidence_ids: [...new Set([...(operation.evidence_ids as string[]), String(execution.evidence_id)])],
  ...
  compensateIssue(args: JsonObject): JsonObject {
    const operation = this.store.get("external_effect", text(args.effect_id, "effect_id"));
    const compensation = operation.compensation as JsonObject;
    ...
    if (existing) {
      if (existing.fingerprint !== fingerprint) throw new Error("Effect compensation idempotency conflict");
  ...
  compensateFromExecution(args: JsonObject): JsonObject {
    ...
    if (!compensation.authorization_id || execution.authorization_id !== compensation.authorization_id ||
      execution.status !== "completed" || !execution.evidence_id) throw new Error("Compensation requires its completed authorized egress execution");
    const succeeded = compensation.succeeded_http_statuses as number[];
    const failed = compensation.failed_http_statuses as number[];
    const httpStatus = Number(execution.http_status); const status = succeeded.includes(httpStatus) ? "succeeded"
  ...
  compensateReport(args: JsonObject): JsonObject {
    ...
    const status = text(args.status, "status"); if (!RESULTS.has(status)) throw new Error("Compensation result is unsupported");
    const evidenceIds = strings(args.evidence_ids, "evidence_ids"); for (const evidenceId of evidenceIds) this.store.get("evidence", evidenceId);
  ...
  sagaCreate(args: JsonObject): JsonObject {
    const task = this.store.get("task", text(args.task_id, "task_id")); const effectIds = strings(args.effect_ids, "effect_ids");
  ...
  sagaGet(args: JsonObject): JsonObject {
    ...
    return { saga, ...this.sagaState(saga, (saga.effect_ids as string[]).map((id) => this.store.get("external_effect", id))) };
  }

  private sagaState(saga: JsonObject, effects: JsonObject[]): JsonObject {
