...
export type IsolatedExecution = {
  ...
  effect: string; cwd?: string; requires_credential?: boolean; compensation?: JsonObject | null;
};
...
function helperFor(platform: NodeJS.Platform): string | null {
...
export class LocalIsolatedAdapter {
  readonly platform: NodeJS.Platform; readonly helperAvailable: (path: string) => boolean; readonly runner: Runner;
  constructor(options: Options = {}) { this.platform = options.platform ?? process.platform; this.helperAvailable = options.helperAvailable ?? existsSync; this.runner = options.runner ?? runLocalProcess; }

  async execute(input: IsolatedExecution): Promise<JsonObject> {
    const helper = helperFor(this.platform); if (!helper || !this.helperAvailable(helper)) throw new Error("Local isolated execution is unavailable on this platform");
    ...
    const relativeCwd = input.cwd ?? "."; if (!pathAllowed(relativeCwd, input.path_allowlist)) throw new Error("Local isolated path is not in the allowlist");
    const workspace = resolve(input.runtime_root, input.run_id); await mkdir(workspace, { recursive: true });
