...
function render(name: string, operations: ScriptOperation[]): string {
...
export class TrajectoryCompiler {
  ...
  compile(args: JsonObject): JsonObject {
    const taskId = id(args.task_id, "task_id", "task"); this.store.get("task", taskId);
    const trialIds = this.trialIds(args.trial_ids, taskId);
    const operations = this.operations(args.operations);
    const name = text(args.name, "name"); const typescript = render(name, operations);
  ...
  authorize(args: JsonObject): JsonObject {
    ...
    if (signoff.decision !== "passed" || signoff.subject_type !== "trajectory_script_proposal" || signoff.subject_id !== proposal.id ||
      Number(signoff.subject_version) !== Number(proposal.version)) throw new Error("trajectory script proposal requires an exact passed Signoff");
  ...
  private trialIds(value: unknown, taskId: string): string[] {
    ...
    for (const trialId of trialIds) {
      const trial = this.store.get("trial", trialId); const outcome = this.store.find("outcome", `outcome_${trialId}`);
  ...
  private operations(value: unknown): ScriptOperation[] {
    ...
    const operations = value.map((raw, index) => {
      const operation = object(raw, `operations[${index}]`); const operation_id = id(operation.operation_id, `operations[${index}].operation_id`, "operation");
      const kind = text(operation.kind, `operations[${index}].kind`);
      if (kind === "checkpoint") return { operation_id, kind, label: text(operation.label, `operations[${index}].label`) } as ScriptOperation;
      ...
      return { operation_id, kind: "workflow", workflow_id, workflow_version, inputs: object(operation.inputs ?? {}, `operations[${index}].inputs`) } as ScriptOperation;
    });
