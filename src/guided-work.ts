...
export class GuidedWorkKernel {
  ...
  bindLaunch(args: JsonObject): JsonObject {
    const brief = this.store.get("guided_work_brief", text(args.brief_id, "brief_id")); const launch = this.store.get("work_launch", text(args.launch_id, "launch_id"));
    if (launch.task_id !== brief.task_id) throw new Error("Guided work launch does not belong to the brief task");
    if (brief.launch_id !== null && brief.launch_id !== undefined && brief.launch_id !== launch.id) throw new Error("Guided work brief is already bound to another launch");
