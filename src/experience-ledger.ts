...
function canonical(value: unknown): string { if (Array.isArray(value)) return `[${value.map(canonical).join(",")}]`; if (value && typeof value === "object") return `{${Object.entries(value as JsonObject).sort(([a], [b]) => a.localeCompare(b)).map(([key, item]) => `${JSON.stringify(key)}:${canonical(item)}`).join(",")}}`; return JSON.stringify(value); }
...
export class ExperienceLedgerKernel {
  ...
  observe(args: JsonObject): JsonObject {
    ...
    const evidenceIds = this.evidenceIds(args.evidence_ids); const identity = { source, kind, scenario_key: text(args.scenario_key, "scenario_key"), finding_digest: digest(text(args.finding, "finding")), evidence_ids: evidenceIds, content_free: true };
    const observationId = String(args.observation_id ?? `experience_observation_${digest(identity).slice(-20)}`); const existing = this.store.find("experience_observation", observationId); const identityDigest = digest(identity);
  ...
  compile(args: JsonObject): JsonObject {
    ...
    const identity = { scenario_key: scenarioKey, kind, observation_refs: observations.map((item) => ({ id: item.id, version: item.version })), hypothesis_digest: digest(text(args.hypothesis, "hypothesis")), applicability_digest: digest(text(args.applicability, "applicability")), counterexample_digest: digest(text(args.counterexample, "counterexample")), evidence_ids: [...new Set(observations.flatMap((item) => item.evidence_ids as string[]))].sort(), content_free: true };
    const patternId = String(args.pattern_id ?? `experience_pattern_${digest(identity).slice(-20)}`); const existing = this.store.find("experience_pattern", patternId); const identityDigest = digest(identity);
  ...
  propose(args: JsonObject): JsonObject {
    ...
    const identity = { pattern_refs: patterns.map((item) => ({ id: item.id, version: item.version })), subject, design_axes: axes, diff_digest: digest(text(args.diff_summary, "diff_summary")), hypothesis_digest: digest(text(args.hypothesis, "hypothesis")), lifecycle: "draft", execution_visible: false };
    const interventionId = String(args.intervention_id ?? `experience_intervention_${randomUUID().replaceAll("-", "")}`); const existing = this.store.find("experience_intervention", interventionId); const identityDigest = digest(identity);
  ...
  decide(args: JsonObject): JsonObject {
    ...
    const saved = this.store.save("experience_intervention", String(intervention.id), { ...payload(intervention), lifecycle, publication_allowed: false, decision_reason_digest: digest(text(args.reason, "reason")), signoff_id: decision === "accept" ? text(args.signoff_id, "signoff_id") : null, retry_condition_digest: decision === "reject" ? digest(text(args.retry_condition, "retry_condition")) : intervention.retry_condition_digest ?? null });
