...
export class SupplyChainKernel {
  ...
  reconcile(args: JsonObject): JsonObject {
    const source = this.store.get("hub_source", text(args.source_id, "source_id")); const invalidated: JsonObject[] = [];
    ...
    for (const certification of certifications) {
      ...
      if (!reason) continue;
      const evaluation = this.store.get("evaluation_run", String(certification.evaluation_run_id)); const trial = this.store.get("trial", String((evaluation.trial_ids as string[])[0]));
      ...
      for (const profile of this.store.list("activation_profile", 100_000, (item) => item.status === "recommended" && (item.asset_ids as string[]).includes(String(asset.id)))) {
        const versions = profile.asset_versions as JsonObject;
        const certifiedVersion = Number(certification.asset_version) + 1;
        if (Number(versions[String(asset.id)]) === certifiedVersion) entries.push({ kind: "activation_profile", id: String(profile.id), payload: { ...payload(profile), status: "invalidated", invalidation_reason: reason, invalidated_asset_id: asset.id } });
      }
      const saved = this.store.saveBatch(entries); invalidated.push({ certification: saved[2], asset: saved[0], materialization: saved[1], profiles: saved.slice(3), reason });
    }
