...
function brainId(project: string): string { return `project_brain:${digest(project).slice(-24)}`; }
...
export class ProjectBrainKernel {
  ...
  experienceRecord(args: JsonObject): JsonObject {
    const project = projectId(args.project_id); this.require(project); const id = String(args.experience_id ?? `experience_${randomUUID().replaceAll("-", "")}`); const experience = { project_id: project, name: text(args.name, "name"), pattern_digest: digest(text(args.pattern, "pattern")), source_outcome_id: args.source_outcome_id === undefined ? null : text(args.source_outcome_id, "source_outcome_id"), status: "candidate" };
