...
export class AutonomyLadderKernel {
  ...
  decide(args: JsonObject): JsonObject {
    const effect = text(args.effect, "effect"); const unattended = args.unattended === true;
    if (!new Set(["read_only", "local_write", "external_write", "destructive"]).has(effect)) throw new Error("Autonomy effect is unsupported");
    ...
    if (effect === "local_write") return this.isolated(args, effect, workspaceId, "unattended_local_write_requires_verified_boundary");
    if (effect === "external_write") {
      if (args.authorization_ref === undefined || args.reobserve_required !== true) throw new Error("External write requires authorization_ref and reobserve_required=true");
      return this.isolated(args, effect, workspaceId, "external_gateway_requires_verified_boundary");
    }
    if (args.authorization_ref === undefined || args.compensation_or_handoff !== true) throw new Error("Destructive effect requires authorization_ref and compensation_or_handoff=true");
    return this.isolated(args, effect, workspaceId, "destructive_effect_requires_verified_boundary");
  }

  get(args: JsonObject): JsonObject { return { decision: this.store.get("autonomy_ladder_decision", text(args.decision_id, "decision_id")) }; }

  private isolated(args: JsonObject, effect: string, workspaceId: string | null, reason: string): JsonObject {
    const profileId = text(args.platform_profile_id, "platform_profile_id"); const profile = this.store.get("platform_execution_profile", profileId);
