...
function noSecret(value: string): void { if (/(?:api[_-]?key|password|secret|token)\s*[:=]\s*[^\s]{8,}/i.test(value)) throw new Error("Project Knowledge content appears to contain a secret"); }
...
export class ProjectKnowledgeKernel {
  ...
  discover(args: JsonObject): JsonObject {
    ...
    const projectRoot = resolve(text(args.project_root, "project_root")); const memoriesRoot = safeChild(projectRoot, ".serena/memories");
    ...
    const discoveryId = String(args.discovery_id ?? `project_knowledge_${recordDigest(identity).slice(-16)}`); const existing = this.store.find("project_knowledge_discovery", discoveryId); const discoveryDigest = recordDigest(identity);
    if (existing) { if (existing.discovery_digest !== discoveryDigest) throw new Error("Project Knowledge discovery idempotency conflict"); return { discovery: existing, idempotent: true }; }
  ...
  resolve(args: JsonObject): JsonObject {
    const discovery = this.store.get("project_knowledge_discovery", text(args.discovery_id, "discovery_id")); const requested = Array.isArray(args.memory_ids) ? args.memory_ids.map((item) => text(item, "memory_ids")) : [];
  ...
  proposeUpdate(args: JsonObject): JsonObject {
    const discovery = this.store.get("project_knowledge_discovery", text(args.discovery_id, "discovery_id")); const evidenceIds = Array.isArray(args.evidence_ids) ? args.evidence_ids.map((item) => text(item, "evidence_ids")) : [];
    if (!evidenceIds.length) throw new Error("Project Knowledge update proposal requires evidence_ids"); for (const evidenceId of evidenceIds) this.store.get("evidence", evidenceId);
    ...
    if (!(discovery.descriptors as MemoryDescriptor[]).some((item) => item.memory_id === identity.memory_id)) throw new Error("Project Knowledge update target is not in discovery");
    const proposalId = String(args.proposal_id ?? `project_knowledge_proposal_${recordDigest(identity).slice(-16)}`); const existing = this.store.find("project_knowledge_proposal", proposalId); const proposalDigest = recordDigest(identity);
    if (existing) { if (existing.proposal_digest !== proposalDigest) throw new Error("Project Knowledge proposal idempotency conflict"); return { proposal: existing, idempotent: true }; }
