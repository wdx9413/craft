...
export class ManagedWriteKernel {
  readonly store: CraftStore; readonly transactions: TransactionCoordinator; readonly workspace: WorkspaceState;
  constructor(store: CraftStore, transactions: TransactionCoordinator, workspace: WorkspaceState) { this.store = store; this.transactions = transactions; this.workspace = workspace; }
  ...
  settleForRun(run: JsonObject): JsonObject | null {
    ...
    if (!new Set(["completed", "failed", "cancelled", "interrupted"]).has(String(run.status))) throw new Error("Managed write settlement requires terminal Host receipt");
    const existing = this.store.find("managed_write_settlement", `managed_write_settlement_${guard.id}_${run.id}`);
    if (existing) return { guard: this.store.get("managed_write_guard", String(guard.id)), settlement: existing, idempotent: true };
    const committed = run.status === "completed";
    const checkpoint = committed ? this.workspace.checkpoint({ workspace_id: guard.workspace_id, checkpoint_id: `managed_write_commit_${guard.id}_${run.id}`, label: `Fabric ${guard.id} completed`, artifact_ids: [], evidence_ids: [] }).checkpoint as JsonObject : null;
  ...
  rollback(args: JsonObject): JsonObject {
    ...
    if (!new Set(["committed", "rollback_pending"]).has(String(guard.status))) throw new Error(`Managed write cannot roll back from ${guard.status}`);
  ...
  private workspaceId(fabric: JsonObject): string { return String(this.store.get("verified_work_loop", String(fabric.work_loop_id)).workspace_id); }
  private scope(fabric: JsonObject): string[] { return [...this.store.get("workspace", this.workspaceId(fabric)).include_paths as string[]].sort(); }
  private launch(fabric: JsonObject): JsonObject { const loop = this.store.get("verified_work_loop", String(fabric.work_loop_id)); const run = this.store.get("task_run", String(loop.task_run_id)); return this.store.get("work_launch", String(run.launch_id)); }
  private assertRun(guard: JsonObject, runId: string): void { const run = this.store.get("host_run", runId); const fabric = this.store.get("execution_fabric", String(guard.fabric_id)); const launch = this.launch(fabric); if (run.host !== launch.host || run.dispatch_id !== launch.dispatch_id) throw new Error("Managed write Host Run does not match Fabric"); }
}
