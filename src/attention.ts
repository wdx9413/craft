...
export class AttentionKernel {
  ...
  private candidates(): Candidate[] {
    ...
    for (const state of this.store.list("task_control_state", 10_000, (item) => item.actor === "human" && ["awaiting_approval", "awaiting_acceptance", "recovery", "blocked"].includes(String(item.status)))) add(state, "task_control_state", "human", state.status === "blocked" ? 98 : state.status === "recovery" ? 92 : 88, "A controlled task needs an explicit next decision", String(state.action), { contract_id: state.contract_id, launch_id: state.launch_id, delivery_loop_id: state.delivery_loop_id });
