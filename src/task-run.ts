...
export class TaskRunKernel {
  ...
  create(args: JsonObject): JsonObject {
    ...
    if (digest([contract.status, contract.launch_id, contract.task_id]) !== digest(["active", launch.id, launch.task_id])) throw new Error("Task Run requires an active Task Control contract bound to the Work Launch");
    ...
    const runId = String(args.task_run_id ?? `task_run_${launch.id}`); const existing = this.store.find("task_run", runId); const identityDigest = digest(identity);
  ...
  refresh(args: JsonObject): JsonObject {
    ...
    const stateId = `task_run_state_${run.id}`; const existing = this.store.find("task_run_state", stateId); const stateDigest = digest(identity);
  ...
  handoff(args: JsonObject): JsonObject {
    ...
    const handoffId = String(args.handoff_id ?? `task_run_handoff_${run.id}_${state.version}`); const existing = this.store.find("task_run_handoff", handoffId); const identityDigest = digest(identity);
