 ...
 */
export const HOOK_POINTS: readonly string[] = ["before_step", "after_step", "before_effect", "after_receipt", "on_failure"];
/** Built-in hook targets Craft can always satisfy without an external process. */
export const BUILTIN_HOOK_TARGETS: readonly string[] = ["builtin:audit-log", "builtin:token-meter", "builtin:receipt-check"];
 ...
 * put it.
 */
export function defaultHooks(): HookSpec[] {
  return defineHooks([
...
export interface HookRun { outcomes: HookOutcome[]; blocked: boolean }
...
export function defineHook(input: JsonObject): HookSpec {
  ...
  if (!HOOK_ID.test(id)) throw new Error(`Unsupported hook id: ${id}`);
  ...
  if (!["builtin", "command"].includes(kind)) throw new Error(`Unsupported hook kind: ${kind}`);
  ...
  return { id, point, kind: kind as HookSpec["kind"], target, fail_policy: failPolicy as HookFailPolicy, timeout_ms: timeoutMs };
}

/** Validate a hook list, refusing duplicate ids so one hook cannot silently replace another. */
export function defineHooks(entries: unknown): HookSpec[] {
  if (entries === undefined || entries === null) return [];
  if (!Array.isArray(entries)) throw new Error("Hooks must be an array");
  const hooks = entries.map((entry) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) throw new Error("Each hook must be an object");
    return defineHook(entry as JsonObject);
  });
  if (new Set(hooks.map((hook) => hook.id)).size !== hooks.length) throw new Error("Hooks must not repeat a hook id");
  return hooks;
}

/** Hooks for one point, in declaration order, so an audit trail reads the same way twice. */
export function planHooks(hooks: readonly HookSpec[], point: string): HookSpec[] {
  if (!HOOK_POINTS.includes(point)) throw new Error(`Unsupported hook point: ${point}`);
 ...
 */
export function runHooksSync(hooks: readonly HookSpec[], point: string,
  payload: JsonObject, options: { invoke: (hook: HookSpec, payload: JsonObject) => JsonObject }): HookRun {
 ...
 * it observes.
 */
export async function runHooks(hooks: readonly HookSpec[], point: string,
  payload: JsonObject, options: { invoke: (hook: HookSpec, payload: JsonObject) => JsonObject | Promise<JsonObject> }): Promise<HookRun> {
  const outcomes: HookOutcome[] = [];
  for (const hook of planHooks(hooks, point)) {
    let outcome: HookOutcome;
    try {
