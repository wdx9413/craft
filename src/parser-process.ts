...
export function resolveParserWorkerPath(entry = process.argv[1] ?? "", cwd = process.cwd(),
  fileExists: (path: string) => boolean = existsSync): string {
...
export function scrubParserEnvironment(environment: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  ...
  return { ...Object.fromEntries(names.flatMap((name) => environment[name] === undefined ? [] : [[name, environment[name]]])),
    NODE_V8_COVERAGE: "" } as NodeJS.ProcessEnv;
}

export function runParserWorker(request: JsonObject, options: WorkerOptions = {}): Promise<JsonObject> {
  const path = options.workerPath ?? resolveParserWorkerPath(); const timeoutMs = options.timeoutMs ?? 5_000;
  if (!Number.isInteger(timeoutMs) || timeoutMs < 10 || timeoutMs > 30_000) throw new Error("timeout_ms must be an integer between 10 and 30000");
  return new Promise((resolve, reject) => {
    ...
    const finish = (error?: Error, result?: JsonObject) => {
      settled = true; clearTimeout(timer); error ? reject(error) : resolve(result as JsonObject);
    };
    const timer = setTimeout(() => { child.kill(); finish(new Error("Parser worker timed out")); }, timeoutMs);
    child.on("error", (error) => finish(error));
    child.stdout.on("data", (chunk: Buffer) => {
      stdout += chunk.toString("utf8");
      if (Buffer.byteLength(stdout) > MAX_OUTPUT_BYTES) { child.kill(); finish(new Error("Parser worker output exceeded 2 MiB")); }
    });
    child.stderr.on("data", (chunk: Buffer) => { if (stderr.length < 4096) stderr += chunk.toString("utf8"); });
    child.on("close", (code) => {
      if (settled) return;
      let response: JsonObject;
      try { response = JSON.parse(stdout) as JsonObject; } catch { finish(new Error(`Parser worker returned invalid JSON${stderr ? ": " + stderr.trim() : ""}`)); return; }
      if (code !== 0 || response.ok !== true) { finish(new Error(String(response.error ?? `Parser worker exited with code ${code}`))); return; }
      finish(undefined, response.analysis as JsonObject);
    });
...
export class ParserProcessAdapter {
  ...
  async parse(args: JsonObject): Promise<JsonObject> {
    const contentId = text(args.content_id, "content_id").trim(); const raw = text(args.raw_content, "raw_content");
    const content = this.security.store.get("untrusted_content", contentId);
    const digest = `sha256:${createHash("sha256").update(raw).digest("hex")}`;
    if (content.content_digest !== digest) throw new Error("raw_content digest does not match the registered content envelope");
    ...
    try {
      const analysis = await runParserWorker({ raw_content: raw, format: args.format, selectors: args.selectors },
      ...
      const fieldSources = Object.fromEntries(Object.entries(parserSelectors).map(([field, selector]) =>
        [field, { selector, citations: [`content:${contentId}${selector}`] }]));
      ...
      const receipt = this.security.store.create("parser_process_receipt", receiptId, { content_id: contentId,
        ...
        extraction_id: (result.extraction as JsonObject).id, raw_content_stored: false });
