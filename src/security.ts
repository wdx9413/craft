...
export class SecurityBrokerKernel {
  ...
  egressAuthorize(args: JsonObject): JsonObject {
    ...
    const host = target.hostname.toLowerCase(); const action = text(args.action, "action");
    if (!(lease.allowed_hosts as string[]).includes(host)) throw new Error("Egress host is not allowed by the credential lease");
    if (!(lease.allowed_actions as string[]).includes(action)) throw new Error("Egress action is not allowed by the credential lease");
    if ((lease.approval_required_actions as string[]).includes(action) && !args.approval_ref) throw new Error("Egress action requires an approval reference");
  ...
  extractionRecord(args: JsonObject): JsonObject {
    ...
    const fieldSources = Object.fromEntries(Object.keys(structuredData).map((field) => {
      ...
      if (supplied === undefined) return [field, { selector: null, citations }];
  ...
  projectionRelease(args: JsonObject): JsonObject {
    ...
    if (!new Set(["program", "human"]).has(reviewerType)) throw new Error("reviewer_type must be program or human");
    ...
    const fields = strings(args.allowed_fields, "allowed_fields"); const source = extraction.structured_data as JsonObject;
    ...
    const extractionSources = extraction.field_sources as JsonObject;
  ...
  projectionExplain(args: JsonObject): JsonObject {
    ...
    const allowedFields = projection.allowed_fields as string[];
    if (requestedField !== undefined && !allowedFields.includes(requestedField)) throw new Error("field is not present in the released projection");
    const selectedFields = requestedField === undefined ? allowedFields : [requestedField];
    const data = projection.data as JsonObject; const lineage = projection.field_lineage as JsonObject;
