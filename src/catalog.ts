...
function declarationKey(item: JsonObject): string {
...
export class Catalog {
  ...
  async addSource(path: string, label?: string, scan = true, priority = 0): Promise<JsonObject> {
    ...
    if (!Number.isInteger(priority) || priority < -1000 || priority > 1000) throw new Error("Capability source priority must be an integer between -1000 and 1000.");
  ...
  listLogicalCapabilities(): JsonObject[] {
    return this.store.list("logical_capability", Number.MAX_SAFE_INTEGER).map((logical) => ({ ...logical,
      conflicts: this.store.list("capability_conflict", Number.MAX_SAFE_INTEGER, (conflict) => (conflict.logical_capability_ids as string[]).includes(String(logical.id))) }));
  }
  ...
  updateSource(id: string, enabled?: boolean, label?: string, priority?: number): JsonObject {
    ...
    if (priority !== undefined && (!Number.isInteger(priority) || priority < -1000 || priority > 1000)) throw new Error("Capability source priority must be an integer between -1000 and 1000.");
  ...
  private rebuildLogicalCapabilities(): void {
    ...
    const byDigest = new Map<string, JsonObject[]>();
    for (const item of observations) { const digest = String(item.digest); byDigest.set(digest, [...(byDigest.get(digest) ?? []), item]); }
    const current = new Set<string>(); const logicalByDigest = new Map<string, string>();
    for (const [digest, group] of byDigest) {
      const logicalId = stableId("logical_capability", digest); current.add(logicalId); logicalByDigest.set(digest, logicalId);
      ...
      const selected = ordered[0]; const instances = ordered.map((item) => ({ capability_id: item.id, source_id: item.source_id, source_priority: Number(sources.get(String(item.source_id))?.priority ?? 0), path: item.path }));
    ...
    for (const item of observations) { const key = declarationKey(item); byDeclaration.set(key, new Set([...(byDeclaration.get(key) ?? []), String(item.digest)])); }
    const conflictIds = new Set<string>();
    for (const [key, digests] of byDeclaration) if (digests.size > 1) {
      const conflictId = stableId("capability_conflict", key); conflictIds.add(conflictId); const logicalIds = [...digests].map((digest) => logicalByDigest.get(digest)!).sort();
  ...
  search(query: string, limit = 6): JsonObject[] {
    ...
    const sources = new Map(this.listSources().map((source) => [String(source.id), source])); const selected = new Map<string, JsonObject>();
    for (const item of candidates) { const key = String(item.digest); const previous = selected.get(key); const priority = Number(sources.get(String(item.source_id))?.priority ?? 0); const previousPriority = previous ? Number(sources.get(String(previous.source_id))?.priority ?? 0) : -Infinity; if (!previous || priority > previousPriority || (priority === previousPriority && String(item.id) < String(previous.id))) selected.set(key, item); }
    return [...selected.values()]
      .sort((left, right) => Number(right.score) - Number(left.score) || String(left.id).localeCompare(String(right.id)))
      .slice(0, Math.min(Math.max(1, limit), 20)).map((item) => {
      const { body: _body, metadata: _metadata, search_text: _searchText, ...summary } = item; const logical = this.store.find("logical_capability", stableId("logical_capability", String(item.digest)));
      const conflicts = logical ? this.store.list("capability_conflict", Number.MAX_SAFE_INTEGER, (conflict) => (conflict.logical_capability_ids as string[]).includes(String(logical.id))) : [];
  ...
  private async capabilityVectors(provider: EmbeddingProvider): Promise<Array<{ capability: JsonObject; vector: number[] }>> {
    ...
    const cached: Array<{ capability: JsonObject; vector: number[] }> = [];
    ...
    for (const capability of capabilities) {
      ...
      if (record && record.capability_digest === capability.digest && record.provider_fingerprint === provider.fingerprint && Array.isArray(record.vector)) {
        cached.push({ capability, vector: record.vector as number[] });
...
function embeddingId(capabilityId: string, fingerprint: string): string { return stableId("embedding", `${capabilityId}:${fingerprint}`); }
function embeddingText(capability: JsonObject): string {
  return sanitizeEmbeddingText([capability.name, capability.description, metadataTerms(capability.metadata as JsonObject).join("\n")]
    .filter((value) => typeof value === "string" && value.trim()).join("\n"));
}
function summary(item: JsonObject): JsonObject {
