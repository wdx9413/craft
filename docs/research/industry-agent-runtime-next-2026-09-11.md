...
> 历史读取基线：`docs/product/roadmap.zh-CN.md` 当时标注 v0.11.41；当前实现基线为 v0.12.1。本记录保留为调研基线，不修改代码或产品配置。

> **状态更新（2026-09-12，基线已到 v0.11.63）**：本文的差距分析与排序结论仍然有效，但 N1–N4 的实际进展必须按下表读，避免从错误前提出发。
>
> | 切片 | 真实状态 |
> | --- | --- |
> | **N1** | **六项验收标准已全部通过（40/40 断言）**，走的是真实入口（`dist/plugin/craft-mcp-full.cjs` + `dist/src/cli.js worker tick`）。研发闭环、文件成果确定性验收、人工修改、Host 中断、审批过期、输入漂移逐项有真实 receipt。**但 Host 是替身二进制、视频域未跑真实任务**，故属"治理机制已验收、端到端生产力未验收"。详见 [`n1-acceptance-report-2026-09-12.md`](n1-acceptance-report-2026-09-12.md)。 |
> | **N2** | 未开始。无脱敏真实 Case/Suite。 |
> | **N3** | 未开始。沙箱 Conformance 未做。 |
> | **N4** | 未开始（按本文结论应最后做）。 |
>
> 期间已落地的相关进展：**v0.11.63 工具面分片**——把 485 个工具按域切成 8 个可独立挂载的面（core 69 默认不变，最大分片 ≈10.6k tokens vs full 的 57.3k），正是本文第 62 行“仅向当前 Host 交付有预算、理由、版本和 scope 的少量材料”的兑现。**同期修复 2 个回执完整性缺陷**：回执 ID 原先用记录版本号做身份，版本号在多次观测间会重复，导致审计回执与 loop 实际状态脱钩；以及人工修改的 `needs_replan_reason` 被派生原因覆盖。详见 [`craft-next-version-plan-2026-09-12.md`](craft-next-version-plan-2026-09-12.md)。
