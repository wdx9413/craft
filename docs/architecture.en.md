...
Version 0.9.0 includes Experience Patterns, Skill Proposals, controlled publishing, default route continuation, lease TTL/renewal, idempotent submissions, and budget blocking alongside immutable same-benchmark comparisons and automatic Orchestration Trial capture. Repeated sampling and confidence estimates, automatic host drivers, automatic model and operational grading, automated search, Agent IR compilation, Capability Kit registries, and case-adaptive assembly remain future work. Codex and Claude plugin distributions use a dependency-free single-file MCP bundle.
