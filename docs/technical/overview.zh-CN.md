...
v0.11.20 固定第一条能力平面边界：**Source Mount 是 provenance，逻辑 Capability 是可发现实体**。不同目录、软链接或镜像可以共存；同内容由摘要归并为一个逻辑能力，搜索按 Source 优先级选择一个实例并保留全部来源。同一声明身份的不同内容形成 Conflict，不允许静默覆盖。Activation Profile、显式 Workflow scope、可撤销 Context Profile 和只读 Integrity Audit 仍是下一阶段目标，不能作为当前能力宣称。
