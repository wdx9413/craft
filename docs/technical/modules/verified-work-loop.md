...
> 状态：v0.11.52 提供主链机制和脱敏 fixture；它不宣称任意业务 Harness 已经带来质量收益。
...
读任务不要求 OS 级沙箱；它仍受 Workspace 范围、只读 effect 和 Receipt 限制。`workspace-write` 继续经过既有审批/Platform Profile。外部写入仍由现有 effect/egress/compensation 机制处理，v0.11.52 不将其声称为通用事务回滚。
