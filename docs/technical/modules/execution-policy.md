...
沙箱属于“执行与保障”的核心能力。`Execution Decision` 根据 effect、平台和声明信息返回执行层级；它是策略决定，不是运行环境已经隔离的证明。以下区分代码现状和目标：隔离实现现状的描述以 v0.10.2 为基线，其后版本只新增预算、等待与 Fallback 等控制面协议，没有扩大沙箱安全承诺；最新能力清单以 [产品路线](../../product/roadmap.zh-CN.md) 为准。
