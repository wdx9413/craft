...
- 会话超过窗口时保留系统约束和最近工作，旧回合被替换成带 Digest 的压缩笔记。**恢复读回压缩结果是 v0.12.43 才补上的**：此前 `context_compaction` 与 `work_note` 只能写不能读，恢复中的会话只能知道"曾经压缩过一次"，无法知道压缩后是什么。现在记录 id 就是 session id，因此只知道自己名字就能恢复，对应 `craft_runtime_truth_compaction_get` / `_list` / `craft_runtime_truth_work_note_get`。
