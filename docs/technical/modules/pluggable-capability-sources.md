...
模型和宿主会变，复杂的“超强 Prompt”很容易从增益变成上下文污染。默认只内置 `craft-route`：它提供稳定的最小路由与澄清边界，不绑定某一家模型、代码流程或行业术语。`craft` 和 `craft-clarify` 保持可单独安装；默认不内置长篇角色 Skill、自动多 Agent 编排、网页抓取器或第三方 MCP。
