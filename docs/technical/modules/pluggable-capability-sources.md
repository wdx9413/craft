...
> 状态：架构与接入准则。基线 v0.11.34。当前已实现本地 Source Mount、签名 Hub 目录同步、隔离落地、候选认证和 Codex/Claude/DeepSeek Harness/通用 MCP 接入；下述远程 Registry Adapter、A2A Adapter 和 Source 连接器尚未实现。
