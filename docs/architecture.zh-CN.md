...
v0.3.1 已实现六维 Harness Configuration、不可变 Trial/Outcome、只追加 Trace、评测分区、Workflow 晋级/回滚门禁，以及 Workflow 执行的自动取证闭环。自动 Harness 搜索、双层经验库、Grader 和按 Case 自适应装配仍是路线设计。
