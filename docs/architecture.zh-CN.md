...
当前版本已提供 Provider 模式的完整 MCP 路径、能力目录、任务/证据、确定性 Workflow、可续接默认编排、可恢复多 Agent 路由、受控确定性 Driver、Runtime Adapter 契约、程序化晋级评测和 Agent IR 编译/Lowering。Codex 与 Claude 插件使用不依赖 `node_modules` 的单文件 MCP bundle。独立 Agent 的通用模型循环、真实容器/网络沙箱、短期凭据代理、自动模型/业务 Grader、统计显著性、Kit Registry、远程 Hub、桌面端、预算预估和补偿事务是后续增量，不应在文档中被描述成已完成。
