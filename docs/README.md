...
- [Runtime Truth（v0.12.9）](technical/modules/runtime-truth.zh-CN.md)：统一 Trace、Tool Call、SSE、OTLP、压缩和结构化工作笔记。
- [Runtime Boundaries（v0.12.9）](technical/modules/v0128-runtime-boundaries.zh-CN.md)：OS 安全计划、MCP Registry、A2A Transport 与组织同步。
- [Project Brain 与 Work Session（v0.12.9）](technical/modules/project-brain-work-session.zh-CN.md)：把目标、资料、决策、任务、成果和可复核上下文固定到项目。
- [Workbench Experience（v0.12.9）](technical/modules/workbench-experience.zh-CN.md)：Trace、成果、Evidence 和下一步的只读体验投影。
- [Long Task Worker（v0.12.9）](technical/modules/long-task-worker.zh-CN.md)：释放进程、事件唤醒、重新验证和新 Host 派发。
...
文档区分“已实现接口/机制”“真实链路已验收”和“目标/待实现”。v0.12.9 的本地接口与测试已实现；真实平台 OS 沙箱、Secret Broker、OTLP 部署、远程 A2A 和多人同步仍须由对应 Adapter 提供独立证明。有代码、100% 单元覆盖率、真实平台安全与业务质量是不同证据。
